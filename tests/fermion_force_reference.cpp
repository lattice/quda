...
template <class su3_matrix, typename half_wilson_vector,
...
template<class su3_matrix, typename Real, typename half_wilson_vector, typename anti_hermitmat>
static void 
		   ...
		   half_wilson_vector *Path_numu, anti_hermitmat* mom) 
{
  ...
  if(GOES_FORWARDS(mu)){
    if(GOES_FORWARDS(nu)){
      add_3f_force_to_mom<su3_matrix>(Path_numu, Path, mu, coeff, mom) ;
    }else{
      add_3f_force_to_mom<su3_matrix>(Path,Path_numu,OPP_DIR(mu),m_coeff, mom);
    }
  }
  else{
    if(GOES_FORWARDS(nu))
      add_3f_force_to_mom<su3_matrix>(Path_nu, Path_mu, mu, m_coeff, mom);
    else
      add_3f_force_to_mom<su3_matrix>(Path_mu, Path_nu, OPP_DIR(mu), coeff, mom) ;
  }
}
			   ...
			   su3_matrix* sitelink, anti_hermitmat* mom)    
{
  ...
  for(sig=0; sig < 8; sig++){
	...
	add_3f_force_to_mom<su3_matrix>(P3, Pmu, sig, mThreeSt, mom);
      }	    
      for(nu=0; nu < 8; nu++){
	...
	if (GOES_FORWARDS(sig)){
	  add_3f_force_to_mom<su3_matrix>(P5, Pnumu, sig, FiveSt, mom);
	}
	...
	for(rho =0; rho < 8; rho++){
	  ...
	  if(GOES_FORWARDS(sig)){
	    add_3f_force_to_mom<su3_matrix>(P7, Prhonumu, sig, mSevenSt, mom) ;	
	  }
	  u_shift_hw(P7, P7rho, rho, sitelink);
	  side_link_3f_force<su3_matrix>(rho,sig,SevenSt, Pnumu, P7, Prhonumu, P7rho, mom);		    
	...
	side_link_3f_force<su3_matrix>(nu,sig,mFiveSt,Pmu,P5, Pnumu,P5nu, mom);
	...
	for(i=0; i < V; i++){
      ...
      u_shift_hw(Pnumu, P5, sig, sitelink);
      if(GOES_FORWARDS(sig)){
	add_3f_force_to_mom<su3_matrix>(P5, Pnumu, sig, Lepage, mom);
      }
	    
      u_shift_hw(P5,P5nu, mu, sitelink);
      side_link_3f_force<su3_matrix>(mu, sig, mLepage, Pmu, P5, Pnumu, P5nu, mom);
	...
	u_shift_hw(P3,P3mu, mu, sitelink);
      ...
      side_link_3f_force<su3_matrix>(mu, sig, ThreeSt, temp_x, P3, Pmu, P3mu, mom);   
	...
	if(GOES_BACKWARDS(mu)){
	  //one link term
	  add_3f_force_to_mom<su3_matrix>(Pmu, temp_x, OPP_DIR(mu), OneLink, mom);
	  u_shift_hw(temp_x, Popmu, mu, sitelink);
	  add_3f_force_to_mom<su3_matrix>(Pnumu, Popmu, OPP_DIR(mu), mNaik, mom);
	  u_shift_hw(Pnumu, Pmumumu, OPP_DIR(mu), sitelink);
	  add_3f_force_to_mom<su3_matrix>(Pmumumu, temp_x, OPP_DIR(mu), Naik, mom);
	}else{
	  u_shift_hw(temp_x, Popmu, mu, sitelink);
	  add_3f_force_to_mom<su3_matrix>(Popmu, Pnumu, mu, Naik, mom);
	}		
      }//if
    }//mu
  }//sig
