...
cpuColorSpinorField *spinorInArray[QUDA_MAX_ARRAY_SIZE];
cpuColorSpinorField *spinorOutArray[QUDA_MAX_ARRAY_SIZE];
...
int invert_test()
{
  ...
  rng->Init();
  for(int i=0; i < Nsrc; i++){
   construct_spinor_source(spinorInArray[i]->V(), 1, 3, inv_param.cpu_prec, csParam.x, *rng);
}
  rng->Release();
  delete rng;
  ...
  case 2:
      void* outArray[QUDA_MAX_ARRAY_SIZE];
      void* inArray[QUDA_MAX_ARRAY_SIZE];
