...
double verifyWilsonTypeEigenvector(void *spinor, double _Complex lambda, int i, QudaGaugeParam &gauge_param, QudaEigParam &eig_param, void **gauge, void *clover, void *clover_inv)
{
  ...
  case QUDA_TWISTED_MASS_DSLASH: {
	...
	tm_ndeg_matpc(spinorTmp, gauge, spinor, kappa, mu, epsilon,
		      matpc_type, dagger, cpu_prec, gauge_param);
	if(normop) tm_ndeg_matpc(spinorTmp2, gauge, spinorTmp, kappa, mu, epsilon,
				 matpc_type, dagger_opposite, cpu_prec, gauge_param);
      } else {
	tm_ndeg_mat(spinorTmp, gauge, spinor, kappa, mu, epsilon, dagger,
		    cpu_prec, gauge_param);
	if(normop) tm_ndeg_mat(spinorTmp, gauge, spinor, kappa, mu, epsilon, dagger_opposite,
			       cpu_prec, gauge_param);
  ...
  case QUDA_WILSON_DSLASH: {
    if(use_pc) {
      wil_matpc(spinorTmp, gauge, spinor, kappa, matpc_type, dagger, cpu_prec,
		gauge_param);
      if(normop) wil_matpc(spinorTmp2, gauge, spinorTmp, kappa, matpc_type, dagger_opposite, cpu_prec,
			   gauge_param);
    } else {
  ...
  case QUDA_CLOVER_WILSON_DSLASH: {
    if(use_pc) {
      clover_matpc(spinorTmp, gauge, clover, clover_inv, spinor, kappa, matpc_type, dagger,
		   cpu_prec, gauge_param);
      if(normop) clover_matpc(spinorTmp2, gauge, clover, clover_inv, spinorTmp, kappa, matpc_type, dagger_opposite,
			      cpu_prec, gauge_param);
    } else {
      clover_mat(spinorTmp, gauge, clover, spinor, kappa, dagger, cpu_prec, gauge_param);
      if(normop) clover_mat(spinorTmp2, gauge, clover, spinorTmp, kappa, dagger_opposite,
			    cpu_prec, gauge_param);
    }
    break;
  }
...
double verifyWilsonTypeSingularVector(void *spinor_left, void *spinor_right, double _Complex sigma, int i, QudaGaugeParam &gauge_param, QudaEigParam &eig_param, void **gauge, void *clover, void *clover_inv)
{
  bool use_pc = (eig_param.use_pc == QUDA_BOOLEAN_TRUE ? true : false);
  int vol = (use_pc ? Vh : V);
  ...
  switch(dslash_type) {
  case QUDA_DOMAIN_WALL_DSLASH: {
    if(use_pc) dw_matpc(spinorTmp, gauge, spinor_left, kappa5, matpc_type, dagger, cpu_prec, gauge_param, mass);
    else dw_mat(spinorTmp, gauge, spinor_left, kappa5, dagger, cpu_prec, gauge_param, mass);
    break;
  }
  case QUDA_DOMAIN_WALL_4D_DSLASH: {
    if(use_pc) dw_4d_matpc(spinorTmp, gauge, spinor_left, kappa5, matpc_type, dagger, cpu_prec, gauge_param, mass);
    else dw_4d_mat(spinorTmp, gauge, spinor_left, kappa5, dagger, cpu_prec, gauge_param, mass);
    break;
  }
  case QUDA_MOBIUS_DWF_DSLASH: {
    double _Complex *kappa_b = (double _Complex *)safe_malloc(Lsdim * sizeof(double _Complex));
    ...
    if(use_pc) mdw_matpc(spinorTmp, gauge, spinor_left, kappa_b, kappa_c, matpc_type, dagger, cpu_prec, gauge_param, mass, inv_param.b_5, inv_param.c_5);
    else mdw_mat(spinorTmp, gauge, spinor_left, kappa_b, kappa_c, dagger, cpu_prec, gauge_param, mass, inv_param.b_5, inv_param.c_5);
    host_free(kappa_b);
    host_free(kappa_c);
    break;
  }
  case QUDA_MOBIUS_DWF_EOFA_DSLASH: {
    if(use_pc) mdw_eofa_matpc(spinorTmp, gauge, spinor_left, matpc_type, dagger, cpu_prec, gauge_param, mass,
			      inv_param.m5, (__real__ inv_param.b_5[0]), (__real__ inv_param.c_5[0]), inv_param.mq1, inv_param.mq2,
			      inv_param.mq3, inv_param.eofa_pm, inv_param.eofa_shift);
    else mdw_eofa_mat(spinorTmp, gauge, spinor_left, dagger, cpu_prec, gauge_param, mass,
		      inv_param.m5, (__real__ inv_param.b_5[0]), (__real__ inv_param.c_5[0]), inv_param.mq1, inv_param.mq2,
		      inv_param.mq3, inv_param.eofa_pm, inv_param.eofa_shift);
    break;
  }
  case QUDA_TWISTED_MASS_DSLASH: {
    if (twist_flavor != QUDA_TWIST_SINGLET) {
      if(use_pc) tm_ndeg_matpc(spinorTmp, gauge, spinor_left, kappa, mu, epsilon,
			       matpc_type, dagger, cpu_prec, gauge_param);
      else tm_ndeg_mat(spinorTmp, gauge, spinor_left, kappa, mu, epsilon, dagger,
		       cpu_prec, gauge_param);
    } else {
      if(use_pc) tm_matpc(spinorTmp, gauge, spinor_left, kappa, mu, twist_flavor,
			  matpc_type, dagger, cpu_prec, gauge_param);
      else tm_mat(spinorTmp, gauge, spinor_left, kappa, mu, twist_flavor,
		  dagger, cpu_prec, gauge_param);
    }
    break;
  }
  case QUDA_TWISTED_CLOVER_DSLASH: {
    if (twist_flavor != QUDA_TWIST_SINGLET) {
      if(use_pc) tmc_ndeg_matpc(spinorTmp, gauge, spinor_left, clover, clover_inv, kappa, mu,
				epsilon, matpc_type, dagger, cpu_prec, gauge_param);
      else tmc_ndeg_mat(spinorTmp, gauge, spinor_left, clover, kappa, mu,
			  epsilon, dagger, cpu_prec, gauge_param);
    } else {
      if(use_pc) tmc_matpc(spinorTmp, gauge, spinor_left, clover, clover_inv, kappa, mu,
			   twist_flavor, matpc_type, dagger, cpu_prec, gauge_param);
      else tmc_mat(spinorTmp, gauge, spinor_left, clover, kappa, mu,
		   twist_flavor, dagger, cpu_prec, gauge_param);
    }
    break;
  }
  case QUDA_WILSON_DSLASH: {
    if(use_pc) wil_matpc(spinorTmp, gauge, spinor_left, kappa, matpc_type, dagger, cpu_prec,
			 gauge_param);
    else wil_mat(spinorTmp, gauge, spinor_left, kappa, dagger, cpu_prec, gauge_param);
    break;
  }
  case QUDA_CLOVER_WILSON_DSLASH: {
    if(use_pc) clover_matpc(spinorTmp, gauge, clover, clover_inv, spinor_left, kappa, matpc_type, dagger,
			    cpu_prec, gauge_param);
    else clover_mat(spinorTmp, gauge, clover, spinor_left, kappa, dagger, cpu_prec, gauge_param);
    break;
  }
  default : errorQuda("Action not supported");
  }
