	  ...
	  for (int s2 = 0; s2 < nSpin; s2++)
	      ...
	      for (int c1 = 0; c1 < src_colors; c1++)
		{
		  ...
		  contractColors<Float>(h_prop_array_flavor_1[s1*src_colors+c1] + off,
					h_prop_array_flavor_2[s2*src_colors+c1] + off,
					nSpin, M);
		  ...
		  for (int mom_idx=0; mom_idx<n_mom; ++mom_idx)
		      ...
		      for(size_t m_idx=0; m_idx<num_out_results; ++m_idx)
			{
			  ...
			  size_t g_idx = global_reduct_slices * num_out_results * mom_idx +
			    num_out_results * red_coord + m_idx;
...
template<typename Float>
int contractionFT_reference(Float **spinorX, Float **spinorY,
			    const double *const d_result, const QudaContractType cType,
			    const int src_colors, const int *X, 
			    ...
			    const int n_mom, const int *const mom_modes, const QudaFFTSymmType *const fft_type)
{
  ...
  contractFTHost<Float>(spinorX, spinorY, h_result,
			cType, src_colors,
			X, source_position,
  ...
  std::array<double,ntol> tolerance{1.0e-5*fact, 1.0e-4*fact, 1.0e-3*fact, 1.0e-2*fact, 1.0e-1*fact, 1.0e+0*fact, 1.0e+1*fact};
  int check_tol = 4;
