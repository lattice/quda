 ...
 */
template <typename real_t, QudaPCType type> struct DslashReference4D {
 ...
 * @param[in] parity The parity of the dslash (0 for even, 1 for odd).
 * @param[in] dagger Whether to apply the original or the Hermitian conjugate operator
 * @param[in] precision The floating-point type used for the vectors
 * @param[in] type Domain wall preconditioning type (4 or 5 dimensions)
 */
void dslash_reference_4d(void *out, const void *const *gauge, const void *const *ghostGauge, const void *in,
                         ...
                         QudaPrecision precision, QudaPCType type)
{
  if (is_enabled_domain_wall()) {
    if (type == QUDA_4D_PC)
      ...
      instantiate_host<DslashReference4D, QUDA_5D_PC>(precision, out, gauge, ghostGauge, in, fwdSpinor, backSpinor,
                                                      parity, dagger);
  } else {
    errorQuda("Domain wall fermions are not instantiated");
  }
}
 ...
 */
template <typename real_t, bool plus> // plus = true -> gamma_+; plus = false -> gamma_-
 ...
 */
template <typename real_t> struct MdwEofaM5 {
...
#pragma omp parallel for
    for (int idx_cb_4d = 0; idx_cb_4d < Vh; idx_cb_4d++) {
      for (int s = 0; s < Ls; s++) {
        if (dagger == 0) {
          if (eofa_pm) {
            axpby_ssp_project<real_t, true>(out, (real_t)1., out, shift_coeffs[s], in, idx_cb_4d, s, Ls - 1);
          } else {
            axpby_ssp_project<real_t, false>(out, (real_t)1., out, shift_coeffs[s], in, idx_cb_4d, s, 0);
          }
        } else {
          if (eofa_pm) {
            axpby_ssp_project<real_t, true>(out, (real_t)1., out, shift_coeffs[s], in, idx_cb_4d, Ls - 1, s);
          } else {
            axpby_ssp_project<real_t, false>(out, (real_t)1., out, shift_coeffs[s], in, idx_cb_4d, 0, s);
          }
        }
      }
    }
...
{
  if constexpr (is_enabled_domain_wall())
    instantiate_host<MdwEofaM5>(precision, out, in, parity, dagger, mferm, m5, b, c, mq1, mq2, mq3, eofa_pm, eofa_shift);
  else
    errorQuda("Domain wall fermions are not enabled");
}
 ...
 * @param[in] zero_initialize Whether or not to zero initialize or accumulate into the output rhs
 */
void dslash_reference_5th(void *out, const void *in, int parity, int dagger, double mferm, QudaPrecision precision,
                          QudaPCType type, bool zero_initialize = false)
{
  if constexpr (is_enabled_domain_wall()) {
    if (type == QUDA_4D_PC)
      instantiate_host<DslashReference5th, QUDA_4D_PC>(precision, out, in, parity, dagger, mferm, zero_initialize);
    else
      instantiate_host<DslashReference5th, QUDA_5D_PC>(precision, out, in, parity, dagger, mferm, zero_initialize);
  } else {
    errorQuda("Domain wall fermions are not enabled");
  }
}
 ...
 *
 * @tparam real_t The floating-point type used for the vectors
 * @param[out] out_ Host output rhs
 * @param[in] in_ Host input spinor
 * @param[in] parity 0 for D_eo, 1 for D_oe (unused)
 * @param[in] dagger 0 for the regular operator, 1 for the dagger operator
 * @param[in] mferm Domain wall fermion mass
 * @param[in] kappa_ Kappa values for each 5th dimension slice
 */
template <typename real_t> struct MDslashReference5thInv {
  void operator()(void *out_, const void *in_, int, int dagger, real_t mferm,
                  const std::vector<std::complex<double>> &kappa_)
  {
    ...
    for (int xs = 0; xs < Ls; xs++) {
      kappa[xs] = kappa_[xs]; // precision change
      inv_Ftr[xs] = one / (one + std::pow(two * kappa[xs], Ls) * mferm);
      Ftr[xs] = -two * kappa[xs] * mferm * inv_Ftr[xs];
      for (int i = 0; i < Vh; i++) { memcpy(&out[12 * (i + Vh * xs)], &in[12 * (i + Vh * xs)], 12 * sizeof(complex)); }
    }
    if (dagger == 0) {
      // s = 0
#pragma omp parallel for
      for (int i = 0; i < Vh; i++) {
        ax(&out[6 + 12 * (i + Vh * (Ls - 1))], inv_Ftr[0], &in[6 + 12 * (i + Vh * (Ls - 1))], 6);
      }
...
#pragma omp parallel for
        for (int i = 0; i < Vh; i++) {
          axpy(two * kappa[xs], &out[12 * (i + Vh * xs)], &out[12 * (i + Vh * (xs + 1))], 6);
          axpy(Ftr[xs], &out[6 + 12 * (i + Vh * xs)], &out[6 + 12 * (i + Vh * (Ls - 1))], 6);
        }
        for (int tmp_s = 0; tmp_s < Ls; tmp_s++) Ftr[tmp_s] *= two * kappa[tmp_s];
      }
      for (int xs = 0; xs < Ls; xs++) Ftr[xs] = -std::pow(two * kappa[xs], Ls - 1) * mferm * inv_Ftr[xs];

      // s = ls-2 ... 0
      for (int xs = Ls - 2; xs >= 0; --xs) {
#pragma omp parallel for
        for (int i = 0; i < Vh; i++) {
          axpy(Ftr[xs], &out[12 * (i + Vh * (Ls - 1))], &out[12 * (i + Vh * xs)], 6);
          axpy(two * kappa[xs], &out[6 + 12 * (i + Vh * (xs + 1))], &out[6 + 12 * (i + Vh * xs)], 6);
        }
        for (int tmp_s = 0; tmp_s < Ls; tmp_s++) Ftr[tmp_s] /= two * kappa[tmp_s];
      }
      // s = ls -1
#pragma omp parallel for
      for (int i = 0; i < Vh; i++) {
        ax(&out[12 * (i + Vh * (Ls - 1))], inv_Ftr[Ls - 1], &out[12 * (i + Vh * (Ls - 1))], 6);
      }
    } else {
      // s = 0
#pragma omp parallel for
      for (int i = 0; i < Vh; i++) { ax(&out[12 * (i + Vh * (Ls - 1))], inv_Ftr[0], &in[12 * (i + Vh * (Ls - 1))], 6); }

      // s = 1 ... ls-2
      for (int xs = 0; xs <= Ls - 2; ++xs) {
#pragma omp parallel for
        for (int i = 0; i < Vh; i++) {
          axpy(Ftr[xs], &out[12 * (i + Vh * xs)], &out[12 * (i + Vh * (Ls - 1))], 6);
          axpy(two * kappa[xs], &out[6 + 12 * (i + Vh * xs)], &out[6 + 12 * (i + Vh * (xs + 1))], 6);
        }
        for (int tmp_s = 0; tmp_s < Ls; tmp_s++) Ftr[tmp_s] *= two * kappa[tmp_s];
      }
      for (int xs = 0; xs < Ls; xs++) Ftr[xs] = -std::pow(two * kappa[xs], Ls - 1) * mferm * inv_Ftr[xs];
...
#pragma omp parallel for
        for (int i = 0; i < Vh; i++) {
          axpy(two * kappa[xs], &out[12 * (i + Vh * (xs + 1))], &out[12 * (i + Vh * xs)], 6);
          axpy(Ftr[xs], &out[6 + 12 * (i + Vh * (Ls - 1))], &out[6 + 12 * (i + Vh * xs)], 6);
        }
        for (int tmp_s = 0; tmp_s < Ls; tmp_s++) Ftr[tmp_s] /= two * kappa[tmp_s];
      }
      // s = ls -1
#pragma omp parallel for
      for (int i = 0; i < Vh; i++) {
        ax(&out[6 + 12 * (i + Vh * (Ls - 1))], inv_Ftr[Ls - 1], &out[6 + 12 * (i + Vh * (Ls - 1))], 6);
      }
    }
  }
};
 ...
 * @param[in] eofa_shift EOFA parameter eofa_shift
 */
template <typename real_t> struct MdwEofaM5Inv {
  void operator()(void *out_, const void *in_, int parity, int dagger, real_t mferm, real_t m5, real_t b, real_t c,
                  real_t mq1, real_t mq2, real_t mq3, int eofa_pm, real_t eofa_shift)
  {
    auto out = reinterpret_cast<real_t *>(out_);
    ...
    std::vector<std::complex<double>> kappa_array(Ls, -0.5 * kappa5);
    std::vector<real_t> eofa_u(Ls), eofa_x(Ls), eofa_y(Ls);

    {
      MDslashReference5thInv<real_t> mdr5thinv;
      mdr5thinv(out_, in_, parity, dagger, mferm, kappa_array);
    }
      ...
      for (int s = 0; s < Ls; s++) {
        for (int sp = 0; sp < Ls; sp++) {
          ...
          if (dagger == 0) {
            ...
            if (eofa_pm) {
              axpby_ssp_project<real_t, true>(out, (real_t)1., out, t, in, idx_cb_4d, s, sp);
            } else {
              axpby_ssp_project<real_t, false>(out, (real_t)1., out, t, in, idx_cb_4d, s, sp);
            }
          } else {
...
{
  if constexpr (is_enabled_domain_wall())
    instantiate_host<MdwEofaM5Inv>(precision, out, in, parity, dagger, mferm, m5, b, c, mq1, mq2, mq3, eofa_pm,
                                   eofa_shift);
  else
    errorQuda("Domain wall fermions are not enabled");
}
               ...
               const QudaGaugeParam &gauge_param, double mferm)
{
  ...
  dslash_reference_4d(out, gauge, ghostGauge, in, fwd_nbr_spinor, back_nbr_spinor, parity, dagger, precision, QUDA_5D_PC);
  dslash_reference_5th(out, in, parity, dagger, mferm, precision, QUDA_5D_PC);
}
                 ...
                 const QudaGaugeParam &gauge_param, double)
{
  ...
  void **fwd_nbr_spinor = inField.fwdGhostFaceBuffer;
  void **back_nbr_spinor = inField.backGhostFaceBuffer;
  dslash_reference_4d(out, gauge, ghostGauge, in, fwd_nbr_spinor, back_nbr_spinor, parity, dagger, precision, QUDA_4D_PC);
}
...
void dslash_5_inv(void *out, const void *const *, const void *in, int parity, int dagger, QudaPrecision precision,
                  const QudaGaugeParam &, double mferm, const std::vector<double> &kappa)
{
  if constexpr (is_enabled_domain_wall())
    instantiate_host<DslashReference5thInv>(precision, out, in, parity, dagger, mferm, kappa);
  else
    errorQuda("Domain wall fermions are not enabled");
}

void mdw_dslash_5_inv(void *out, const void *const *, const void *in, int parity, int dagger, QudaPrecision precision,
                      const QudaGaugeParam &, double mferm, const std::vector<std::complex<double>> &kappa)
{
  if constexpr (is_enabled_domain_wall())
    instantiate_host<MDslashReference5thInv>(precision, out, in, parity, dagger, mferm, kappa);
  else
    errorQuda("Domain wall fermions are not enabled");
}
...
void mdw_dslash_4_pre(void *out, const void *const *, const void *in, int parity, int dagger, QudaPrecision precision,
                      const QudaGaugeParam &, double mferm, const std::vector<std::complex<double>> &b5,
                      const std::vector<std::complex<double>> &c5, bool zero_initialize)
{
  dslash_reference_5th(out, in, parity, dagger, mferm, precision, QUDA_4D_PC, zero_initialize);
  if (precision == QUDA_DOUBLE_PRECISION) {
    for (int xs = 0; xs < Ls; xs++) {
      axpby(b5[xs], reinterpret_cast<const std::complex<double> *>(in) + Vh * spinor_site_size / 2 * xs, 0.5 * c5[xs],
            reinterpret_cast<std::complex<double> *>(out) + Vh * spinor_site_size / 2 * xs, Vh * spinor_site_size / 2);
    }
  } else {
    for (int xs = 0; xs < Ls; xs++) {
      axpby(std::complex<float>(b5[xs]),
            reinterpret_cast<const std::complex<float> *>(in) + Vh * (spinor_site_size / 2) * xs,
            std::complex<float>(0.5 * c5[xs]),
            reinterpret_cast<std::complex<float> *>(out) + Vh * (spinor_site_size / 2) * xs, Vh * spinor_site_size / 2);
    }
  }
}
...
void dw_4dpv_mat(void *out, const void *const *gauge, const void *in, double kappa, int dagger, QudaPrecision precision,
  const QudaGaugeParam &gauge_param, double mferm)
{
                  ...
                  double mq2, double mq3, int eofa_pm, double eofa_shift)
{
  ...
  std::vector<complex> b5(Ls, b);
  std::vector<complex> c5(Ls, c);
               ...
               const std::vector<std::complex<double>> &b5, const std::vector<std::complex<double>> &c5)
{
  using complex = std::complex<double>;
  void *tmp = safe_malloc(V5h * spinor_site_size * precision);
  std::vector<complex> kappa5(Ls), kappa2(Ls), kappa_mdwf(Ls);
  for (int xs = 0; xs < Ls; xs++) {
                    ...
                    double c, double mq1, double mq2, double mq3, int eofa_pm, double eofa_shift)
{
  ...
  std::vector<complex> kappa2(Ls, -0.25 / (b * (4. + m5) + 1.) / (b * (4. + m5) + 1.));
  std::vector<complex> b5(Ls, b), c5(Ls, c);
