...
std::shared_ptr<QUDAApp> make_app(std::string app_description, std::string app_name)
{
  ...
  // quda_app->add_option("--dim", dim, "Set space-time dimension (X Y Z T)");
  ...
  CLI::TransformPairs<QudaTwistFlavorType> twist_flavor_map {{"singlet", QUDA_TWIST_SINGLET},
  ...
  //  quda_app->add_option("--gridsize", gridsize, "Set the grid size in all four dimension (default 1 1 1 1)");
  ...
  CLI::TransformPairs<QudaInverterType> inv_type_map {{"cg", QUDA_CG_INVERTER},
  ...
  CLI::TransformPairs<QudaMassNormalization> mass_normalization_type_map {
    {"kappa", QUDA_KAPPA_NORMALIZATION},
  ...
  // quda_app->add_option(
  // "--partition", partition, "Set the communication topology (X=1, Y=2, Z=4, T=8, and combinations of these)");
  quda_app->add_option("--pipeline", pipeline,
                       "The pipeline length for fused operations in GCR, BiCGstab-l (default 0, no pipelining)");

  // // precision options
  CLI::TransformPairs<QudaPrecision> prec_map(
  ...
  CLI::TransformPairs<QudaReconstructType> recon_type_map {{"18", QUDA_RECONSTRUCT_NO},
  ...
  quda_app->add_option("--ydim", ydim, "Set X dimension size(default 24)")->check(CLI::Range(1, 512));
  quda_app->add_option("--zdim", zdim, "Set X dimension size(default 24)")->check(CLI::Range(1, 512));
  quda_app->add_option("--tdim", tdim, "Set T dimension size(default 24)")->check(CLI::Range(1, 512));
  ...
  // quda_app->add_option("--xgridsize", xgridsize, "Set grid size in X dimension (default 1)");
  // quda_app->add_option("--ygridsize", ygridsize, "Set grid size in Y dimension (default 1)");
  // quda_app->add_option("--zgridsize", zgridsize, "Set grid size in Z dimension (default 1)");
  // quda_app->add_option("--tgridsize", tgridsize, "Set grid size in T dimension (default 1)");
...
void add_eigen_option_group(std::shared_ptr<QUDAApp> quda_app)
{
  ...
  CLI::TransformPairs<QudaEigSpectrumType> spectrum_map {{"SR", QUDA_SPECTRUM_SR_EIG}, {"LR", QUDA_SPECTRUM_LR_EIG},
                                                           {"SM", QUDA_SPECTRUM_SM_EIG}, {"LM", QUDA_SPECTRUM_LM_EIG},
                                                           {"SI", QUDA_SPECTRUM_SI_EIG}, {"LI", QUDA_SPECTRUM_LI_EIG}};

  opgroup
    ->add_option("--eig-spectrum", eig_spectrum,
                 "The spectrum part to be calulated. S=smallest L=largest R=real M=modulus I=imaginary")
    ->transform(CLI::QUDACheckedTransformer(spectrum_map));
  opgroup->add_option("--eig-tol", eig_tol, "The tolerance to use in the eigensolver");

  CLI::TransformPairs<QudaEigType> type_map {
  ...
  opgroup->add_option("--eig-type", eig_type, "The type of eigensolver to use (default trlm)")
    ->transform(CLI::QUDACheckedTransformer(type_map));
...
void add_multigrid_option_group(std::shared_ptr<QUDAApp> quda_app)
{
  ...
  // TODO
  // auto geo_block_size_opt = opgroup->add_mgoption("--mg-block-size", geo_block_size, geo_block_size_transform, "Set
  // the geometric block size for the each multigrid level's transfer operator (default 4 4 4 4)");
  ...
  auto solver_trans = CLI::QUDACheckedTransformer(inv_type_map);
  ...
  quda_app->add_mgoption(
    opgroup, "--mg-eig-spectrum", mg_eig_spectrum, CLI::QUDACheckedTransformer(spectrum_map),
  ...
  CLI::TransformPairs<QudaEigType> type_map {
    {"trlm", QUDA_EIG_TR_LANCZOS}, {"irlm", QUDA_EIG_IR_LANCZOS}, {"iram", QUDA_EIG_IR_ARNOLDI}};
  quda_app->add_mgoption(opgroup, "--mg-eig-type", mg_eig_type, CLI::QUDACheckedTransformer(type_map),
  ...
  // TODO
  // auto mg_vec_infile_opt = opgroup->add_mgoption("--mg-load-vec", mg_vec_infile, mg_vec_infile_transform, "Load the
  // vectors "file" for the multigrid_test (requires QIO)"); auto mg_vec_outfile_opt = opgroup->add_mgoption("--mg-save-vec",
  // mg_vec_outfile, mg_vec_outfile_transform, "Save the generated null-space vectors "file" from the multigrid_test (requires QIO)");
  ...
  CLI::TransformPairs<QudaFieldLocation> location_map {{"cpu", QUDA_CPU_FIELD_LOCATION},
                                                       {"host", QUDA_CPU_FIELD_LOCATION},
                                                       {"gpu", QUDA_CUDA_FIELD_LOCATION},
                                                       {"device", QUDA_CUDA_FIELD_LOCATION}};
  quda_app->add_mgoption(opgroup, "--mg-setup-location", setup_location, CLI::QUDACheckedTransformer(location_map),
  ...
  quda_app->add_mgoption(opgroup, "--mg-solve-location", solver_location, CLI::QUDACheckedTransformer(location_map),
