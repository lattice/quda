...
const double _2link_omega   = -mass;//0.5;//0.1;
...
void mg_dslash_test(int argc, char** argv)
{
...
*/
  ...
  cudaColorSpinorField *cudaSpinor = new cudaColorSpinorField(csParam);

  printfQuda("Creating cudaSpinorOut\n");
  cudaColorSpinorField *cudaSpinorOut = new cudaColorSpinorField(csParam);
  ...
  *cudaSpinor = *in;
  ...
  dirac->setOmega(_2link_omega);
  //dirac->setOmega(0.0002);

  csParam.extendDimensionality();

  printfQuda("Creating 5d tmp5\n");
  cudaColorSpinorField *tmp5 = new cudaColorSpinorField(csParam);

  printfQuda("Creating 5d tmp6\n");
  cudaColorSpinorField *tmp6 = new cudaColorSpinorField(csParam);
  ...
  *out = *cudaSpinorOut;
  ...
  double kappa = 0.5;
//  mat(ref->V(), qdp_fatlink, qdp_longlink, in->V(), inv_param.mass, 0, inv_param.cpu_prec, gauge_param.cpu_prec);
  matomega(ref->V(), qdp_fatlink, qdp_longlink, in->V(), inv_param.mass, _2link_omega, 0, inv_param.cpu_prec, gauge_param.cpu_prec);
//#endif
  ax( kappa, out->V(), V*mySpinorSiteSize, inv_param.cpu_prec );
//Let's check output:
  for (int i = 0; i < 2*Vh; i++) {
    genericPrintVector(*out, i);
    genericPrintVector(*ref, i);
  }
//////
  mxpy(out->V(), ref->V(), V*mySpinorSiteSize, inv_param.cpu_prec);
  nrm2 = norm_2(ref->V(), V*mySpinorSiteSize, inv_param.cpu_prec);
  src2 = norm_2(out->V(), V*mySpinorSiteSize, inv_param.cpu_prec);
