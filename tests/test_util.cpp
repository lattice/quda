...
std::shared_ptr<CLI::App> make_app(std::string app_description, std::string app_name)
{
  auto quda_app = std::make_shared<CLI::App>(app_description, app_name);
  ...
  std::map<std::string, QudaContractType> contract_type_map {{"open", QUDA_CONTRACT_TYPE_OPEN},
  ...
  // std::map<std::string,QudaDslashType>
...
void add_eigen_option_group(std::shared_ptr<CLI::App> quda_app)
{
  ...
  opgroup->add_option("--eig-ARPACK-logfile", eig_arpack_logfile, "...");
  opgroup->add_option("--eig-QUDA-logfile", eig_QUDA_logfile, "The filename storing the stdout from the QUDA eigensolver");
 ...
 opgroup->add_option(
  ...
  opgroup->add_option("--eig-max-restarts", eig_max_restarts,
  ...
  opgroup->add_option("--eig-load-vec", eig_vec_infile, "Load eigenvectors to <file> (requires QIO)")->check(CLI::ExistingFile);
...
void add_deflation_option_group(std::shared_ptr<CLI::App> quda_app){
  auto opgroup = quda_app->add_option_group("Deflation", "Options controlling deflation");
   ...
   opgroup->add_option(
    "--df-deflation-grid", deflation_grid, "Set maximum number of cycles needed to compute eigenvectors(default 1)")->check(CLI::PositiveNumber);
  opgroup->add_option(
    "--df-eigcg-max-restarts",
    eigcg_max_restarts, "Set how many iterative refinement cycles will be solved with eigCG within a single physical right hand site solve (default 4)")->check(CLI::PositiveNumber);;
  opgroup->add_option(
    "--df-ext-lib-type", deflation_ext_lib, "Set external library for the deflation methods  (default Eigen library)");
  opgroup->add_option(
    "--df-location-ritz", location_ritz, "Set memory location for the ritz vectors  (default cuda memory location)");
  ...
  opgroup->add_option("--df-max-search-dim", max_search_dim,
  ...
  opgroup->add_option(
    "--df-tol-inc", inc_tol, "Set tolerance for the subsequent restarts in the initCG solver  (default 1e-2)");
