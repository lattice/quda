...
private:
  // these are pointers to the host fields -- use shared as they may alias
  std::shared_ptr<ColorSpinorField> xH, yH, zH, wH, vH, xoH, yoH, zoH;
  ...
  ColorSpinorFieldVectorU xmD, ymD, zmD;
  ...
  ColorSpinorFieldVectorU xmoD, ymoD, zmoD;

  // these are pointers to the host multi-fields that have "this precision"
  ColorSpinorFieldVectorU xmH, ymH, zmH;
...
public:
  double test(Kernel kernel)
  {
    ...
    double error = 0;
    auto A = std::make_unique<quda::Complex[]>(Nsrc * Msrc);
    auto B = std::make_unique<quda::Complex[]>(Nsrc * Msrc);
    auto C = std::make_unique<quda::Complex[]>(Nsrc * Msrc);
    auto A2 = std::make_unique<quda::Complex[]>(Nsrc * Nsrc); // for the block cDotProductNorm test
    auto B2 = std::make_unique<quda::Complex[]>(Nsrc * Nsrc); // for the block cDotProductNorm test
    auto Ar = std::make_unique<double[]>(Nsrc * Msrc);;
    ...
    case Kernel::axpy_block:
      ...
      blas::axpy(Ar.get(), xmD.get(), ymoD.get());
    ...
    case Kernel::caxpy_block:
      ...
      blas::caxpy(A.get(), xmD.get(), ymoD.get());
      for (int i = 0; i < Nsrc; i++) {
    ...
    case Kernel::axpyBzpcx_block:
      ...
      *yD = *yH;
      ...
      blas::axpyBzpcx((double *)A.get(), xmD.get(), zmoD.get(), (double *)B.get(), *yD, (const double *)C.get());

      for (int i = 0; i < Nsrc; i++) {
        blas::axpyBzpcx(((double *)A.get())[i], *xmH[i], *zmH[i], ((double *)B.get())[i], *yH, ((double *)C.get())[i]);
      }
    ...
    case Kernel::reDotProductNorm_block:
      for (int i = 0; i < Nsrc; i++) xmD.Component(i) = *(xmH[i]);
      blas::reDotProduct((double *)A2.get(), xmD.get(), xmD.get());
      error = 0.0;
      for (int i = 0; i < Nsrc; i++) {
        for (int j = 0; j < Nsrc; j++) {
          ((double *)B2.get())[i * Nsrc + j] = blas::reDotProduct(xmD.Component(i), xmD.Component(j));
          error += std::abs(((double *)A2.get())[i * Nsrc + j] - ((double *)B2.get())[i * Nsrc + j])
            / std::abs(((double *)B2.get())[i * Nsrc + j]);
        }
      }
      error /= Nsrc * Nsrc;
      break;
    ...
    case Kernel::reDotProduct_block:
      ...
      blas::reDotProduct((double *)A.get(), xmD.get(), ymoD.get());
      error = 0.0;
      for (int i = 0; i < Nsrc; i++) {
        for (int j = 0; j < Msrc; j++) {
          ((double *)B.get())[i * Msrc + j] = blas::reDotProduct(xmD.Component(i), ymD.Component(j));
          error += std::abs(((double *)A.get())[i * Msrc + j] - ((double *)B.get())[i * Msrc + j])
            / std::abs(((double *)B.get())[i * Msrc + j]);
        }
      }
      error /= Nsrc * Msrc;
      break;
    ...
    case Kernel::cDotProductNorm_block:
      for (int i = 0; i < Nsrc; i++) xmD.Component(i) = *(xmH[i]);
      blas::cDotProduct(A2.get(), xmD.Components(), xmD.Components());
      error = 0.0;
      for (int i = 0; i < Nsrc; i++) {
        for (int j = 0; j < Nsrc; j++) {
          B2[i * Nsrc + j] = blas::cDotProduct(xmD.Component(i), xmD.Component(j));
          error += std::abs(A2.get()[i * Nsrc + j] - B2[i * Nsrc + j]) / std::abs(B2.get()[i * Nsrc + j]);
        }
      }
      error /= Nsrc * Nsrc;
      break;
    ...
    case Kernel::cDotProduct_block:
      ...
      for (int i = 0; i < Msrc; i++) ymoD.Component(i) = *(ymH[i]);
      for (int i = 0; i < Msrc; i++) ymD.Component(i) = *(ymH[i]);
      blas::cDotProduct(A.get(), xmD.Components(), ymoD.Components());
      error = 0.0;
      for (int i = 0; i < Nsrc; i++) {
        for (int j = 0; j < Msrc; j++) {
          B[i * Msrc + j] = blas::cDotProduct(xmD.Component(i), ymD.Component(j));
          error += std::abs(A.get()[i * Msrc + j] - B[i * Msrc + j]) / std::abs(B.get()[i * Msrc + j]);
        }
      }
      error /= Nsrc * Msrc;
      break;
  ...
  double benchmark(Kernel kernel, const int niter)
  {
...

    {
      ...
      case Kernel::axpy_block:
        for (int i = 0; i < niter; ++i) blas::axpy(Ar.get(), xmD.Components(), ymoD.Components());
        break;

      case Kernel::caxpy_block:
        for (int i = 0; i < niter; ++i) blas::caxpy(A.get(), xmD.get(), ymoD.get());
        break;

      case Kernel::axpyBzpcx_block:
        for (int i = 0; i < niter; ++i)
          blas::axpyBzpcx((double *)A.get(), xmD.Components(), zmoD.Components(), (double *)B.get(), *yD, (double *)C.get());
        break;

      case Kernel::reDotProductNorm_block:
        for (int i = 0; i < niter; ++i) blas::reDotProduct((double *)A2.get(), xmD.Components(), xmD.Components());
        break;

      case Kernel::reDotProduct_block:
        for (int i = 0; i < niter; ++i) blas::reDotProduct((double *)A.get(), xmD.Components(), ymoD.Components());
        break;

      case Kernel::cDotProductNorm_block:
        for (int i = 0; i < niter; ++i) blas::cDotProduct(A2.get(), xmD.Components(), xmD.Components());
        break;

      case Kernel::cDotProduct_block:
        for (int i = 0; i < niter; ++i) blas::cDotProduct(A.get(), xmD.Components(), ymoD.Components());
        break;
  ...
  void initFields(prec_pair_t prec_pair)
  {
    ...
    vH = std::make_shared<cpuColorSpinorField>(param);
    wH = std::make_shared<cpuColorSpinorField>(param);
    xH = std::make_shared<cpuColorSpinorField>(param);
    yH = std::make_shared<cpuColorSpinorField>(param);
    zH = std::make_shared<cpuColorSpinorField>(param);
    ...
    // xmH.reserve(Nsrc);
    for (int cid = 0; cid < Nsrc; cid++) xmH.emplace_back(std::make_unique<cpuColorSpinorField>(param));
    // ymH.reserve(Msrc);
    for (int cid = 0; cid < Msrc; cid++) ymH.emplace_back(std::make_unique<cpuColorSpinorField>(param));
    // zmH.reserve(Nsrc);
    for (int cid = 0; cid < Nsrc; cid++) zmH.emplace_back(std::make_unique<cpuColorSpinorField>(param));
    ...
    vD = std::make_unique<cudaColorSpinorField>(param);
    wD = std::make_unique<cudaColorSpinorField>(param);
    xD = std::make_unique<cudaColorSpinorField>(param);
    yD = std::make_unique<cudaColorSpinorField>(param);
    zD = std::make_unique<cudaColorSpinorField>(param);

    param.setPrecision(prec_other, prec_other, true);
    voD = std::make_unique<cudaColorSpinorField>(param);
    woD = std::make_unique<cudaColorSpinorField>(param);
    xoD = std::make_unique<cudaColorSpinorField>(param);
    yoD = std::make_unique<cudaColorSpinorField>(param);
    zoD = std::make_unique<cudaColorSpinorField>(param);
    ...
    xmD = ColorSpinorFieldVectorU(param, Nsrc);

    // param.composite_dim = Msrc;
    ymD = ColorSpinorFieldVectorU(param, Msrc);

    // param.composite_dim = Nsrc;
    zmD = ColorSpinorFieldVectorU(param, Nsrc);

    param.setPrecision(prec_other, prec_other, true);
    // param.composite_dim = Nsrc;
    xmoD = ColorSpinorFieldVectorU(param, Nsrc);

    // param.composite_dim = Msrc;
    ymoD = ColorSpinorFieldVectorU(param, Msrc);

    // param.composite_dim = Nsrc;
    zmoD = ColorSpinorFieldVectorU(param, Nsrc);
