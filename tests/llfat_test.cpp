...
static void llfat_test()
{
  ...
  createSiteLinkCPU(sitelink, qudaGaugeParam.cpu_prec, SITELINK_PHASE_MILC);
  ...
  for (int i = 0; i < 6; i++) { act_path_coeff[i] = 0.1 * i; }
  ...
  if (verify_results) {
    ...
    */
    ...
    exchange_cpu_sitelink(X, sitelink, ghost_sitelink, ghost_sitelink_diag, qudaGaugeParam.cpu_prec, &qudaGaugeParam,
                          optflag);
