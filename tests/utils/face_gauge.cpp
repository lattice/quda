...
void packGhostAllStaples(Float *cpuStaple, Float **cpuGhostBack, Float **cpuGhostFwd, int nFace, lat_dim_t &X)
{
  ...
  int faceVolumeCB[4] = {X[1] * X[2] * X[3] / 2, X[0] * X[2] * X[3] / 2, X[0] * X[1] * X[3] / 2, X[0] * X[1] * X[2] / 2};
  ...
  int f[4][4] = {{XYZ, XY, X[0], 1}, {XYZ, XY, 1, X[0]}, {XYZ, X[0], 1, XY}, {XY, X[0], 1, XYZ}};
          ...
          for (a = 0; a < A[dir]; a++) {
            for (b = 0; b < B[dir]; b++) {
              for (c = 0; c < C[dir]; c++) {
                ...
                if (oddness == 0) { // even
                  for (int i = 0; i < 18; i++) { even_dst[18 * even_dst_index + i] = even_src[18 * index + i]; }
                  even_dst_index++;
                } else { // odd
                  for (int i = 0; i < 18; i++) { odd_dst[18 * odd_dst_index + i] = odd_src[18 * index + i]; }
                  odd_dst_index++;
                }
              } // c
            }   // b
...
void pack_ghost_all_staples_cpu(void *staple, void **cpuGhostStapleBack, void **cpuGhostStapleFwd, int nFace,
                                QudaPrecision precision, lat_dim_t &X)
{
...
void pack_gauge_diag(void *buf, lat_dim_t &X, void **sitelink, int nu, int mu, int dir1, int dir2, QudaPrecision prec)
{
  ...
  for (int i = 0; i < X[dir2]; i++) {
    for (int j = 0; j < X[dir1]; j++) {
      int src_idx
        = ((X[nu] - 1) * mul_factor[nu] + 0 * mul_factor[mu] + i * mul_factor[dir2] + j * mul_factor[dir1]) >> 1;
...
void pack_ghost_all_links(void **cpuLink, void **cpuGhostBack, void **cpuGhostFwd, int dir, int nFace,
                          QudaPrecision precision, lat_dim_t &X)
{
  if (precision == QUDA_DOUBLE_PRECISION) {
  ...
  */
  ...
  for (int nu = XUP; nu <= TUP; nu++) {
    for (int mu = XUP; mu <= TUP; mu++) {
      if (nu == mu) { continue; }
      if (optflag && (!commDimPartitioned(mu) || !commDimPartitioned(nu))) { continue; }
                       ...
                       Float **sitelink_fwd_sendbuf, Float **sitelink_back_sendbuf, int optflag)
{
  ...
  for (int dir = 0; dir < 4; dir++) {
    ...
    pack_ghost_all_links((void **)sitelink, (void **)sitelink_back_sendbuf, (void **)sitelink_fwd_sendbuf, dir, nFace,
                         (QudaPrecision)(sizeof(Float)), X);
  }
...
#define MEMCOPY_GAUGE_FIELDS_BUF_TO_GRID_T(sitelink, ghost_buf, dst_face, src_face, dir, geom)                           \
  ...
  if (cpu_order == QUDA_QDP_GAUGE_ORDER) {                                                                               \
    ...
    memcpy(dst + (geom)*even_dst_idx * gaugebytes, src + (geom)*even_src_idx * gaugebytes,                               \
           (geom)*R[dir] * slice_3d[dir] * gaugebytes / 2);                                                              \
    memcpy(dst + (geom)*odd_dst_idx * gaugebytes, src + (geom)*odd_src_idx * gaugebytes,                                 \
           (geom)*R[dir] * slice_3d[dir] * gaugebytes / 2);                                                              \
  } else {                                                                                                               \
    errorQuda("Unsupported gauge order\n");                                                                              \
  }
                              ...
                              QudaPrecision gPrecision, int optflag, int geometry)
{
  ...
  int f_main[4][4] = {{E[2] * E[1] * E[0], E[1] * E[0], E[0], 1},
                      {E[2] * E[1] * E[0], E[1] * E[0], 1, E[0]},
                      {E[2] * E[1] * E[0], E[0], 1, E[1] * E[0]},
                      {E[1] * E[0], E[0], 1, E[2] * E[1] * E[0]}};

  int f_bound[4][4] = {{E[2] * E[1], E[1], 1, E[3] * E[2] * E[1]},
          ...
          for (b = startb[dir]; b < endb[dir]; b++)
            ...
            if (f_main[dir][2] != 1 || f_bound[dir][2] != 1) {
              for (c = startc[dir]; c < endc[dir]; c++) {
                ...
                int dst_idx
                  = (a * f_bound[dir][0] + b * f_bound[dir][1] + c * f_bound[dir][2] + (d - R[dir]) * f_bound[dir][3])
                  >> 1;
              ...
              for (int loop = 0; loop < 2; loop++) {
                ...
                if (c < endc[dir]) {
                  ...
                  MEMCOPY_GAUGE_FIELDS_GRID_TO_BUF(ghost_sitelink_back_sendbuf, dst_idx, sitelink, src_idx,
                                                   (endc[dir] - c + 1) / 2, dir, geometry);
          ...
          for (b = startb[dir]; b < endb[dir]; b++) {
            ...
            if (f_main[dir][2] != 1 || f_bound[dir][2] != 1) {
              for (c = startc[dir]; c < endc[dir]; c++) {
                ...
                int src_idx = (a * f_main[dir][0] + b * f_main[dir][1] + c * f_main[dir][2] + d * f_main[dir][3]) >> 1;
                int dst_idx
                  = (a * f_bound[dir][0] + b * f_bound[dir][1] + c * f_bound[dir][2] + (d - X[dir]) * f_bound[dir][3])
                  >> 1;
              ...
              for (int loop = 0; loop < 2; loop++) {
                ...
                if (c < endc[dir]) {
                  ...
                  MEMCOPY_GAUGE_FIELDS_GRID_TO_BUF(ghost_sitelink_fwd_sendbuf, dst_idx, sitelink, src_idx,
                                                   (endc[dir] - c + 1) / 2, dir, geometry);
                }
              } // for loop
            }   // if
            ...
            if (f_main[dir][2] != 1 || f_bound[dir][2] != 1) {
              for (c = startc[dir]; c < endc[dir]; c++) {
                ...
                if (commDimPartitioned(dir)) {
                  ...
                  src_idx
                    = (a * f_main[dir][0] + b * f_main[dir][1] + c * f_main[dir][2] + (d + X[dir]) * f_main[dir][3]) >> 1;
                }
              ...
              for (int loop = 0; loop < 2; loop++) {
                ...
                if (c < endc[dir]) {
                  ...
                  if (commDimPartitioned(dir)) {
                    src_idx
                      = (a * f_bound[dir][0] + b * f_bound[dir][1] + c * f_bound[dir][2] + d * f_bound[dir][3]) >> 1;
                  } else {
                    src_idx
                      = (a * f_main[dir][0] + b * f_main[dir][1] + c * f_main[dir][2] + (d + X[dir]) * f_main[dir][3])
                      >> 1;
                  }

                  MEMCOPY_GAUGE_FIELDS_BUF_TO_GRID(sitelink, dst_idx, ghost_sitelink_back, src_idx,
                                                   (endc[dir] - c + 1) / 2, dir, geometry);
          ...
          for (b = startb[dir]; b < endb[dir]; b++) {
            ...
            if (f_main[dir][2] != 1 || f_bound[dir][2] != 1) {
              for (c = startc[dir]; c < endc[dir]; c++) {
                ...
                int src_idx;
                if (commDimPartitioned(dir)) {
                  src_idx = (a * f_bound[dir][0] + b * f_bound[dir][1] + c * f_bound[dir][2]
                             + (d - X[dir] - R[dir]) * f_bound[dir][3])
                    >> 1;
                } else {
                  src_idx
                    = (a * f_main[dir][0] + b * f_main[dir][1] + c * f_main[dir][2] + (d - X[dir]) * f_main[dir][3]) >> 1;
              ...
              for (int loop = 0; loop < 2; loop++) {
                ...
                if (c < endc[dir]) {
                  ...
                  MEMCOPY_GAUGE_FIELDS_BUF_TO_GRID(sitelink, dst_idx, ghost_sitelink_fwd, src_idx,
                                                   (endc[dir] - c + 1) / 2, dir, geometry);
                } // if
              }   // for loop
            }     // if
...
template <typename Float>
void do_exchange_cpu_staple(Float *staple, Float **ghost_staple, Float **staple_fwd_sendbuf,
                            Float **staple_back_sendbuf, lat_dim_t &X)
{
  int nFace = 1;
  pack_ghost_all_staples_cpu(staple, (void **)staple_back_sendbuf, (void **)staple_fwd_sendbuf, nFace,
                             (QudaPrecision)(sizeof(Float)), X);
...
void exchange_cpu_staple(lat_dim_t &X, void *staple, void **ghost_staple, QudaPrecision gPrecision)
{
  ...
  if (gPrecision == QUDA_DOUBLE_PRECISION) {
    do_exchange_cpu_staple((double *)staple, (double **)ghost_staple, (double **)staple_fwd_sendbuf,
                           (double **)staple_back_sendbuf, X);
  } else { // single
    do_exchange_cpu_staple((float *)staple, (float **)ghost_staple, (float **)staple_fwd_sendbuf,
...
void exchange_llfat_cleanup(void)
{
  for (int i = 0; i < 4; i++) {
    ...
    if (fwd_nbr_staple[i]) {
      host_free(fwd_nbr_staple[i]);
      fwd_nbr_staple[i] = NULL;
    }
    if (back_nbr_staple[i]) {
      host_free(back_nbr_staple[i]);
      back_nbr_staple[i] = NULL;
    }
    if (fwd_nbr_staple_sendbuf[i]) {
      host_free(fwd_nbr_staple_sendbuf[i]);
      fwd_nbr_staple_sendbuf[i] = NULL;
    }
    if (back_nbr_staple_sendbuf[i]) {
      host_free(back_nbr_staple_sendbuf[i]);
      back_nbr_staple_sendbuf[i] = NULL;
    }
  }
}
