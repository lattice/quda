...
namespace quda
{
  ...
  void EigenSolver::prepareKrylovSpace(std::vector<ColorSpinorField *> &kSpace, std::vector<Complex> &evals)
  {
    ...
    for (int i = k_size; i < max_size; i++) kSpace.push_back(new ColorSpinorField(cs_param)); // FIXME - why do we need this to be zero initialized?
    for (int b = r_size; b < block_size; b++) { r.push_back(new ColorSpinorField(cs_param)); }
  ...
  void EigenSolver::printEigensolverSetup()
  {
    ...
    if (getVerbosity() >= QUDA_VERBOSE) {
      if(compressed_mode) printfQuda("compressed mode\n");
                                 ...
                                 std::vector<Complex> &evals, int size)
  {
    ...
    temp.push_back(new ColorSpinorField(cs_param));
                                 ...
                                 std::vector<Complex> &evals)
  {
    ...
    {
      // load the vectors
      VectorIO io(compressed_mode ? eig_param->coarse_vec_infile : eig_param->vec_infile, eig_param->io_parity_inflate == QUDA_BOOLEAN_TRUE);
      io.load(vecs_ptr);
    }
