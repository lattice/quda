...
namespace quda {
  ...
  /*cudaColorSpinorField::cudaColorSpinorField() : 
    ColorSpinorField(), alloc(false), init(false) {
...
//    int nFace = (nSpin == 1) ? 3 : 1; //3 faces for asqtad
  ...
  // In the face buffer, after communication, data is stored from backwards, then from 
  // forwards. This means that the unpacking routines follow the same format as the 
