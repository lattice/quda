  ...
  inline void issueRecv(ColorSpinorField &input, const Dslash &dslash, bool gdr, bool nccl = false)
  {
    for(int i=3; i>=0; i--){
      ...
      for(int dir=1; dir>=0; dir--) {
        PROFILE(if (dslash_comms) input.recvStart(2*i+dir, device::get_stream(2*i+dir), gdr, nccl), profile, QUDA_PROFILE_COMMS_START);
      }
    }
  }
  ...
  */
  ...
  template <typename Dslash> struct DslashFactory {
    ...
    static auto blacklist_map(const QudaDslashPolicy &policy)
    {
      ...
      case QudaDslashPolicy::QUDA_NCCL_DSLASH:
        ...
        if (dslash_policy_env) { // set the policies to tune for explicitly
          ...
          while (policy_list >> policy_) {
            ...
            if ((dslash_policy == QudaDslashPolicy::QUDA_GDR_DSLASH
                    || dslash_policy == QudaDslashPolicy::QUDA_NCCL_DSLASH
          ...
          for (auto &i : policies) {
            ...
            if (i == QudaDslashPolicy::QUDA_DSLASH ||
              ...
              (*dslashImp)(dslash, &(this->in), volume, ghostFace, profile);
                     ...
                     i == QudaDslashPolicy::QUDA_NCCL_DSLASH ||
