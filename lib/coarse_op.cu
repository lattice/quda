...
namespace quda {
  ...
  void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
		  const GaugeField &g, const CloverField &c, const CloverField &cI, double kappa, double mu, QudaDiracType dirac, QudaMatPCType matpc, bool twisted) {
    ...
    calculateY<false,Float,fineSpin,fineColor,coarseSpin,coarseColor,gOrder>
      (yAccessor, xAccessor, xInvAccessor, uvAccessor, avAccessor, vAccessor, gAccessor, cAccessor, cInvAccessor, Y, X, Xinv, Yhat, av, v, kappa, mu, dirac, matpc, twisted);
  }
  ...
  void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
    ...
    if (coarseColor == 2) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,2,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else if (coarseColor == 4) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,4,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else if (coarseColor == 8) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,8,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else if (coarseColor == 12) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,12,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else if (coarseColor == 16) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,16,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else if (coarseColor == 20) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,20,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else if (coarseColor == 24) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,24,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else if (coarseColor == 32) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,fineSpin,32,coarseSpin>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
  ...
  void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
		  const GaugeField &g, const CloverField &c, const CloverField &cI, double kappa, double mu, QudaDiracType dirac, QudaMatPCType matpc, bool twisted) {
    if (uv.Nspin() == 4) {
      calculateY<Float,csOrder,gOrder,clOrder,fineColor,4>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
  ...
  void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
		  const GaugeField &g, const CloverField &c, const CloverField &cI, double kappa, double mu, QudaDiracType dirac, QudaMatPCType matpc, bool twisted) {
    if (g.Ncolor() == 3) {
      calculateY<Float,csOrder,gOrder,clOrder,3>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
  ...
  void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
    ...
    if (c.Order() == QUDA_PACKED_CLOVER_ORDER) {
      calculateY<Float,csOrder,gOrder,QUDA_PACKED_CLOVER_ORDER>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
  ...
  void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
    ...
    if (g.FieldOrder() == QUDA_QDP_GAUGE_ORDER) {
      calculateY<Float,csOrder,QUDA_QDP_GAUGE_ORDER>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
      errorQuda("Unsupported field order %d\n", g.FieldOrder());
    }
  }
 ...
 void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
    ...
    if (T.Vectors().FieldOrder() == QUDA_SPACE_SPIN_COLOR_FIELD_ORDER) {
      calculateY<Float,QUDA_SPACE_SPIN_COLOR_FIELD_ORDER>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
  ...
  void calculateY(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, ColorSpinorField &uv, ColorSpinorField &av, const Transfer &T,
    ...
    if (Y.Precision() == QUDA_DOUBLE_PRECISION) {
      ...
      calculateY<double>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
#else
      errorQuda("Double precision multigrid has not been enabled");
#endif
    } else if (Y.Precision() == QUDA_SINGLE_PRECISION) {
      calculateY<float>(Y, X, Xinv, Yhat, uv, av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
  ...
  void CoarseOp(GaugeField &Y, GaugeField &X, GaugeField &Xinv, GaugeField &Yhat, const Transfer &T,
    ...
    ColorSpinorField *av = ((matpc != QUDA_MATPC_INVALID && clover) || (matpc != QUDA_MATPC_INVALID && twisted)) ? ColorSpinorField::Create(UVparam) :
    ...
    if (cloverInv && twisted) {
      ...
      calculateY(Y, X, Xinv, Yhat, *uv, *av, T, g, c, cI, kappa, mu, dirac, matpc, twisted);
    } else {
      cpuCloverField c(cf_param);
      if (clover) clover->saveCPUField(c);
      calculateY(Y, X, Xinv, Yhat, *uv, *av, T, g, c, c, kappa, mu, dirac, matpc, twisted);
    }
