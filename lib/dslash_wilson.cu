...
#include <dslash_policy.cuh>
			...
			const int *commOverride, TimeProfile &profile)
  {
    ...
    for(int i=0;i<4;i++){
      ...
      if(in->GhostOffset(i,0)%in->FieldOrder()) errorQuda("ghostOffset is not a multiple of FloatN\n");
      if(in->GhostOffset(i,1)%in->FieldOrder()) errorQuda("ghostOffset is not a multiple of FloatN\n");
