...
namespace quda {
  ...
  void StaggeredCoarseOp2(GaugeField &Y, GaugeField &X, const Transfer &T, const cudaGaugeField &gauge, const cudaGaugeField &longGauge,
                          const GaugeField& XinvKD, double mass, bool allow_truncation, QudaDiracType dirac, QudaMatPCType matpc, IntList<coarseColor, N...>)
  {
    if (Y.Ncolor() / 2 == coarseColor) {
      ...
      if constexpr (sizeof...(N) > 0) {
        StaggeredCoarseOp2<fineColor>(Y, X, T, gauge, longGauge, XinvKD, mass, allow_truncation, dirac, matpc, IntList<N...>());
      } else {
  ...
  void StaggeredCoarseOp(GaugeField &Y, GaugeField &X, const Transfer &T, const cudaGaugeField &gauge, const cudaGaugeField &longGauge,
                         const GaugeField& XinvKD, double mass, bool allow_truncation, QudaDiracType dirac, QudaMatPCType matpc, IntList<fineColor, N...>)
  {
    if (gauge.Ncolor() == fineColor) {
      IntList<@QUDA_MULTIGRID_NVEC_LIST@> coarseColors;
      StaggeredCoarseOp2<fineColor>(Y, X, T, gauge, longGauge, XinvKD, mass, allow_truncation, dirac, matpc, coarseColors);
    } else {
      if constexpr (sizeof...(N) > 0) {
        StaggeredCoarseOp<fineColor>(Y, X, T, gauge, longGauge, XinvKD, mass, allow_truncation, dirac, matpc, IntList<N...>());
