...
namespace quda {
  ...
  namespace blas {
    ...
    public:
      ...
      template <bool multi_1d, typename device_buffer_t, typename Arg>
      typename std::enable_if<multi_1d, void>::type set_param(device_buffer_t &&buf_d, Arg &arg, char select,
                                                              const T &h, const qudaStream_t &stream)
      {
      ...
      template <bool multi_1d, typename device_buffer_t, typename Arg>
      typename std::enable_if<!multi_1d, void>::type set_param(device_buffer_t &&buf_d, Arg &arg, char dummy,
                                                               const T &h, const qudaStream_t &stream)
      {
        using coeff_t = typename decltype(arg.f)::coeff_t;
      ...
      template <int NXZ> void compute(const qudaStream_t &stream)
      {
        ...
        if (location == QUDA_CUDA_FIELD_LOCATION) {
          ...
          if (a.data)
            set_param<decltype(f_)::multi_1d>(instance.get_constant_ptr("quda::blas::Amatrix_d"), arg, 'a', a, stream);
          if (b.data)
            set_param<decltype(f_)::multi_1d>(instance.get_constant_ptr("quda::blas::Bmatrix_d"), arg, 'b', b, stream);
          if (c.data)
            set_param<decltype(f_)::multi_1d>(instance.get_constant_ptr("quda::blas::Cmatrix_d"), arg, 'c', c, stream);
      ...
      template <int NXZ_max> typename std::enable_if<NXZ_max != 1, void>::type instantiate(const qudaStream_t &stream)
      {
        ...
        constexpr int pow2_max = !decltype(f)::multi_1d ?
          max_NXZ_power2<false, isFixed<store_t>::value>() :
          std::min(max_N_multi_1d(), max_NXZ_power2<false, isFixed<store_t>::value>());
        constexpr int linear_max
          = !decltype(f)::multi_1d ? MAX_MULTI_BLAS_N : std::min(max_N_multi_1d(), MAX_MULTI_BLAS_N);

        if (NXZ <= pow2_max && is_power2(NXZ))
          instantiatePow2<pow2_max>(stream);
        else if (NXZ <= linear_max)
          instantiateLinear<linear_max>(stream);
        else
          errorQuda("x.size %lu greater than maximum supported size (pow2 = %d, linear = %d)", x.size(), pow2_max,
                    linear_max);
      }
      ...
      long long flops() const { return NYW * NXZ * f.flops() * x[0]->Length(); }

      long long bytes() const
      {
        // X and Z reads are repeated (and hopefully cached) across NYW
        // each Y and W read/write is done once
        return NYW * NXZ * (f.read.X + f.write.X) * x[0]->Bytes() + NYW * (f.read.Y + f.write.Y) * y[0]->Bytes()
          + NYW * NXZ * (f.read.Z + f.write.Z) * z[0]->Bytes() + NYW * (f.read.W + f.write.W) * w[0]->Bytes();
      }
                      ...
                      const range &range_x, const range &range_y, int upper, int coeff_width)
    {
      ...
      if (y.size()
          > (size_t)max_YW_size(x.size(), x[0]->Precision(), y[0]->Precision(), false, false, coeff_width, false)) {
    ...
    void caxpyz_recurse(const Complex *a_, std::vector<ColorSpinorField *> &x, std::vector<ColorSpinorField *> &y,
                        std::vector<ColorSpinorField *> &z, const range &range_x, const range &range_y, int pass,
                        int upper)
    {
      // if greater than max single-kernel size, recurse
    ...
    {
      axpy_U(a, x.Components(), y.Components());
    }

    void axpy_L(const double *a, ColorSpinorField &x, ColorSpinorField &y)
    {
      axpy_L(a, x.Components(), y.Components());
    }
