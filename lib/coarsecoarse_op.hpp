...
namespace quda {
                        ...
                        bool need_bidirectional)
  {
    ...
    if (fineColor == 6) { // free field Wilson
      if (coarseColor == 6) {
        calculateYcoarse<use_mma, Float, vFloat, 6, fineSpin, 6, coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv,
                        ...
                        bool need_bidirectional)
  {
    if (T.Vectors().Nspin() == 2) {
      calculateYcoarse<use_mma, Float, vFloat, 2>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mass, mu, mu_factor,
                        ...
                        bool need_bidirectional)
  {
    ...
    if (Y.Precision() == QUDA_DOUBLE_PRECISION) {
      ...
      if (T.Vectors(X.Location()).Precision() == QUDA_DOUBLE_PRECISION) {
        calculateYcoarse<use_mma, double, double>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mass, mu, mu_factor,
                                                  dirac, matpc, need_bidirectional);
      } else {
	errorQuda("Unsupported precision %d\n", Y.Precision());
      ...
      if (T.Vectors(X.Location()).Precision() == QUDA_SINGLE_PRECISION) {
        calculateYcoarse<use_mma, float, float>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mass, mu, mu_factor, dirac,
	...
	errorQuda("Unsupported precision %d\n", T.Vectors(X.Location()).Precision());
      ...
      if (T.Vectors(X.Location()).Precision() == QUDA_HALF_PRECISION) {
        calculateYcoarse<use_mma, float, short>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mass, mu, mu_factor, dirac,
                                                matpc, need_bidirectional);
      } else {
	errorQuda("Unsupported precision %d\n", T.Vectors(X.Location()).Precision());
      }
  ...
  void calculateYcoarse(GaugeField &, GaugeField &, GaugeField &, GaugeField &, ColorSpinorField &,
                        ...
                        double, double, double, double, QudaDiracType, QudaMatPCType, bool)
  {
