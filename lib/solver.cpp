...
namespace quda {
  ...
  bool MultiRhsSolver::convergence(const double &r2, const double &hq2, const double &r2_tol,
  ...
  double MultiRhsSolver::stopping(const double &tol, const double &b2, QudaResidualType residual_type) {
  ...
  void MultiRhsSolver::PrintSummary(const char *name, int k, const double &r2, const double &b2) {
    if (getVerbosity() >= QUDA_SUMMARIZE) {
      if (param.compute_true_res) {
  if (param.residual_type & QUDA_HEAVY_QUARK_RESIDUAL) {
  ...
  void MultiRhsSolver::PrintStats(const char* name, int k, const double &r2,
