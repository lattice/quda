...
#define PROFILE_STOP(profile, idx)
    ...
    */
                          ...
                          int parity, MemoryLocation location, int packIndex, int shmem = 0)
    {
      ...
      if (pack) {
        ...
        PROFILE(if (location != Host) qudaEventRecord(packEnd[halo.bufferIndex], device::get_stream(packIndex)),
    ...
    */
    template <typename Dslash> inline void issueGather(const ColorSpinorField &halo, const Dslash &dslash)
    {
      ...
      for (int i = 3; i >= 0; i--) {
        ...
        for (int dir = 1; dir >= 0; dir--) { // forwards gather
          ...
          PROFILE(qudaStreamWaitEvent(device::get_stream(2 * i + dir), event, 0), profile,
                  QUDA_PROFILE_STREAM_WAIT_EVENT);

          // Initialize host transfer from source spinor
          PROFILE(if (dslash_copy) halo.gather(2 * i + dir, device::get_stream(2 * i + dir)), profile,
                  QUDA_PROFILE_GATHER);

          // Record the end of the gathering if not peer-to-peer
          if (!comm_peer2peer_enabled(dir, i)) {
            PROFILE(qudaEventRecord(gatherEnd[2 * i + dir], device::get_stream(2 * i + dir)), profile,
    ...
    /**
       ...
       @return stream index
    */
    template <typename T> inline int getStreamIndex(const T &dslashParam)
    {
      ...
      for (int i = 3; i >= 0; i--) {
        ...
        if (!comm_peer2peer_enabled(0, i))
          index = 2 * i + 0;
        else if (!comm_peer2peer_enabled(1, i))
          index = 2 * i + 1;
      }
    ...
    */
    template <typename Dslash> struct DslashBasic : DslashPolicyImp<Dslash> {
                      ...
                      TimeProfile &profile)
      {
        ...
        issuePack(halo, in, dslash, parity_src,
                  static_cast<MemoryLocation>(Device | (Remote * dslashParam.remote_write)), packIndex);

        issueGather(halo, dslash);

        PROFILE(if (dslash_interior_compute) dslash.apply(device::get_default_stream()), profile,
        ...
        while (pattern.completeSum < pattern.commDimTotal) {
          for (int i = 3; i >= 0; i--) {
            ...
            if (!pattern.dslashCompleted[2 * i] && pattern.dslashCompleted[pattern.previousDir[2 * i + 1]]
              ...
              PROFILE(if (dslash_exterior_compute) dslash.apply(device::get_default_stream()), profile,
                      QUDA_PROFILE_DSLASH_KERNEL);
        ...
        while (pattern.completeSum < pattern.commDimTotal) {
          for (int i = 3; i >= 0; i--) {
            ...
            for (int dir = 1; dir >= 0; dir--) {
              ...
              // Query if comms has finished
              if (!pattern.commsCompleted[2 * i + dir] && pattern.gatherCompleted[2 * i + dir]) {
                if (commsComplete(halo, dslash, i, dir, false, true, false)) {
                  pattern.commsCompleted[2 * i + dir] = 1;
                  pattern.completeSum++;
                }
              }
                      ...
                      TimeProfile &profile)
      {
        ...
        while (pattern.completeSum < pattern.commDimTotal) {
          ...
          for (int i = 3; i >= 0; i--) {
            ...
            for (int dir = 1; dir >= 0; dir--) {
              ...
              // Query if comms has finished
              if (!pattern.commsCompleted[2 * i + dir]) {
                if (commsComplete(halo, dslash, i, dir, false, false, false, packScatterIndex)) {
                  pattern.commsCompleted[2 * i + dir] = 1;
                  pattern.completeSum++;
                }
              }
        ...
        for (int i = 3; i >= 0; i--) {
          if (dslashParam.commDim[i] && (!comm_peer2peer_enabled(0, i) || !comm_peer2peer_enabled(1, i))) {
            ...
            PROFILE(qudaEventRecord(scatterEnd[0], device::get_stream(packScatterIndex)), profile,
                    QUDA_PROFILE_EVENT_RECORD);
            PROFILE(qudaStreamWaitEvent(device::get_default_stream(), scatterEnd[0], 0), profile,
                    QUDA_PROFILE_STREAM_WAIT_EVENT);
            break;
        ...
        while (pattern.completeSum < pattern.commDimTotal) {
          ...
          for (int i = 3; i >= 0; i--) {
            ...
            for (int dir = 1; dir >= 0; dir--) {
              ...
              // Query if comms have finished
              if (!pattern.commsCompleted[2 * i + dir]) {
                if (commsComplete(halo, dslash, i, dir, false, false, true)) {
                  pattern.commsCompleted[2 * i + dir] = 1;
                  pattern.completeSum++;
                }
              }
    ...
    */
    ...
    template <typename Dslash> struct DslashFactory {
      ...
      static auto blacklist_map(const QudaDslashPolicy &policy)
      {
        ...
        case QudaDslashPolicy::QUDA_GDR_RECV_DSLASH: return QudaDslashPolicy::QUDA_DSLASH;
        case QudaDslashPolicy::QUDA_FUSED_GDR_DSLASH:
        case QudaDslashPolicy::QUDA_FUSED_GDR_RECV_DSLASH: return QudaDslashPolicy::QUDA_FUSED_DSLASH;
        case QudaDslashPolicy::QUDA_ZERO_COPY_PACK_GDR_RECV_DSLASH: return QudaDslashPolicy::QUDA_ZERO_COPY_PACK_DSLASH;
      ...
      static std::unique_ptr<DslashPolicyImp<Dslash>> create(const QudaDslashPolicy &policy_)
      {
        ...
        case QudaDslashPolicy::QUDA_FUSED_ZERO_COPY_PACK_DSLASH:
          return std::make_unique<DslashFusedZeroCopyPack<Dslash>>();
        case QudaDslashPolicy::QUDA_ZERO_COPY_PACK_GDR_RECV_DSLASH:
          return std::make_unique<DslashZeroCopyPackGDRRecv<Dslash>>();
        case QudaDslashPolicy::QUDA_FUSED_ZERO_COPY_PACK_GDR_RECV_DSLASH:
          return std::make_unique<DslashFusedZeroCopyPackGDRRecv<Dslash>>();
        ...
        case QudaDslashPolicy::QUDA_DSLASH_FUSED_PACK_FUSED_HALO:
          return std::make_unique<DslashFusedPackFusedHalo<Dslash>>();
        case QudaDslashPolicy::QUDA_SHMEM_UBER_PACKINTRA_DSLASH:
          return std::make_unique<DslashShmemUberPackIntra<Dslash>>();
        case QudaDslashPolicy::QUDA_SHMEM_UBER_PACKFULL_DSLASH:
          return std::make_unique<DslashShmemUberPackFull<Dslash>>();
          ...
          for (auto &p2p : p2p_policies) {
            ...
            for (auto &i : policies) {
              ...
              if (i == QudaDslashPolicy::QUDA_DSLASH || i == QudaDslashPolicy::QUDA_FUSED_DSLASH
                  ...
                  || i == QudaDslashPolicy::QUDA_FUSED_ZERO_COPY_PACK_DSLASH || i == QudaDslashPolicy::QUDA_ZERO_COPY_DSLASH
                  || i == QudaDslashPolicy::QUDA_FUSED_ZERO_COPY_DSLASH || i == QudaDslashPolicy::QUDA_DSLASH_FUSED_PACK
    ...
    private:
      void apply(const qudaStream_t &) override
      {
        ...
        if (static_cast<QudaDslashPolicy>(tp.aux.x) == QudaDslashPolicy::QUDA_DSLASH_POLICY_DISABLED)
          errorQuda("Requested policy is disabled");

        bool p2p_enabled = comm_peer2peer_enabled_global();
        if (p2p_policies[tp.aux.y] == QudaP2PPolicy::QUDA_P2P_DEFAULT)
          comm_enable_peer2peer(false); // disable p2p if using default policy
        dslashParam.remote_write = (p2p_policies[tp.aux.y] == QudaP2PPolicy::QUDA_P2P_REMOTE_WRITE ?
                                      1 :
                                      0); // set whether we are using remote packing writes or copy engines
