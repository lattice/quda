    ...
    for (int d = 0; d < 4; d++) { // loop over dimension{
      ...
      if (arg.improved) {
        ...
        if (doHalo<kernel_type>(d) && ghost) {
          ...
          out += L * in;
          // printf("Halo (%i %i %i %i), idx %i x_cb %i ghost %i nbr_idx1 %i \t in %f %f %f %f %f %f\n",coord[0],coord[1],coord[2],coord[3], idx, x_cb,
          // ghost_idx, nbr_idx3,in.data[1].real(),in.data[1].imag(),in2.data[1].real(),in2.data[1].imag(),out.data[1].real(),out.data[1].imag());
        } else if (doBulk<kernel_type>() && !ghost) {
          const int fwd3_idx = linkIndexP3(coord, arg.dim, d);
          const Link L = arg.L(d, x_cb, parity);
          const Vector in = arg.in(fwd3_idx, their_spinor_parity);
          out += L * in;
          // printf("Halo (%i %i %i %i),  x_cb %i  \t in %f %f %f %f %f %f\n",coord[0],coord[1],coord[2],coord[3], x_cb,
          // in.data[0].real(),in.data[0].imag(),in.data[1].real(),in.data[1].imag(),out.data[1].real(),out.data[1].imag());
        }
      }
#endif
// #endif
#ifndef XONLY2
      {
        ...
        if (doHalo<kernel_type>(d) && ghost) {
          ...
          const Link UU
              = arg.improved ? arg.U(d, back_idx, 1 - parity) : arg.U(d, back_idx, 1 - parity, StaggeredPhase<Float>(coord, arg.dim, d, -1, arg.tboundary));
          // printf("%i %i (%i %i %i %i): %f %f %f %f %f %f\n",ghost_idx2, d, coord[0], coord[1], coord[2], coord[3], U(0,0).real(), U(1,1).real(),
          // U(2,2).real(),
  ...
  __device__ __host__ inline void staggered(Arg &arg, int idx, int parity)
  {
    ...
    // printf("NEW1 Out cb %i %i \t %f %f %f %f %f %f\n",x_cb, my_spinor_parity,
    // out.data[0].real(),out.data[0].imag(),out.data[1].real(),out.data[1].imag(),out.data[2].real(),out.data[2].imag());
  ...
  __global__ void staggeredGPU(Arg arg)
  {
    ...
    case 1:
      staggered<Float, nDim, nColor, nParity, dagger, xpay, kernel_type>(arg, x_cb, 1);
      break;
      // dslashStaggered<Float,nDim,nColor>(arg, x_cb, parity);
    }
  }

  template <typename Float, int nDim, int nColor, int nParity, bool dagger, bool xpay, KernelType kernel_type, typename Arg> struct StaggeredLaunch {
    template <typename Dslash> inline static void launch(Dslash &dslash, TuneParam &tp, Arg &arg, const cudaStream_t &stream)
    {
