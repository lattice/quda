...
namespace quda
{
  ...
  template NdegTwistedMassApply<Float, nColor, DDArg, ReconstructWilson::recon[reconI]>::NdegTwistedMassApply(
    cvector_ref<ColorSpinorField> &out, cvector_ref<const ColorSpinorField> &in, cvector_ref<const ColorSpinorField> &x,
    const GaugeField &U, double a, double b, double c, int parity, bool dagger, const int *comm_override,
