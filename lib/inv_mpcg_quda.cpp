...
namespace quda {
  ...
  {
    if (N <= 0) return;
    for (int i = 0; i < N; ++i) { d_out[i] = d_in[i] / gamma[i]; }

    for (int i = 0; i < N - 1; ++i) { d_out[i] += (d_in[i + 1] * (1 - rho[i + 1]) / (gamma[i + 1] * rho[i + 1])); }

    for (int i = 1; i < N; ++i) { d_out[i] -= d_in[i - 1] / (rho[i - 1] * gamma[i - 1]); }
  ...
  static void computeGramMatrix(double **G, std::vector<ColorSpinorField> &v, double *mu)
  {
    ...
    {
      std::vector<ColorSpinorField *> vp1;
      vp1.reserve((nsteps + 1) * nsteps);
      std::vector<ColorSpinorField *> vp2;
      vp2.reserve((nsteps + 1) * nsteps);
    ...
    double d[2*nsteps+1];
    std::vector<ColorSpinorField *> vp1;
    vp1.reserve(2 * nsteps + 1);
    std::vector<ColorSpinorField *> vp2;
    vp2.reserve(2 * nsteps + 1);
    for(int i=0; i<=nsteps; ++i){
  ...
  void MPCG::operator()(ColorSpinorField &, ColorSpinorField &)
  {
    ...
    double** G  = new double*[2*s+1];
    for (int i = 0; i < (2 * s + 1); ++i) { G[i] = new double[2 * s + 1]; }
    ...
    for (int i = 0; i < (2 * s + 1); ++i) { zero(G[i], (2 * s + 1)); }
    ...
    while(!convergence(r2,0.0,stop,0.0) && it < param.maxiter){
      ...
      while (!convergence(r2, 0.0, stop, 0.0) && j < s) {
        ...
        if (j == 0) {
          ...
          // compute g coeffs
          for(int i=0; i<(2*s+1); ++i){
            g[i] = rho[j - 1] * g_p1[i] - rho[j - 1] * gamma[j - 1] * d_p1[i] + (1 - rho[j - 1]) * g_p2[i];
          }
