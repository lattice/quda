  ...
  namespace blas3d {
	...
	if(!activeTuning()) {
	  for(int i=0; i<x.X()[3]; i++) {
	    result_global[i] = result_local[comm_coord(3) * x.X()[3] + i];
	    //printfQuda("reDot Result = (%e)\n", result_global[i]);
	  }
	}
      }
	...
	if(!activeTuning()) {
	  for(int i=0; i<x.X()[3]; i++) {
	    result_global[i].real(result_local[comm_coord(3) * 2*x.X()[3] + 2*i + 0]);
	    result_global[i].imag(result_local[comm_coord(3) * 2*x.X()[3] + 2*i + 1]);
	    //printfQuda("cDot Result = (%e,%e)\n", result_global[i].real(), result_global[i].imag());
	  }
	}
      }
