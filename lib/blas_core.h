...
template <typename FloatN, int M, 
  typename SpinorX, typename SpinorY, typename SpinorZ, typename SpinorW, typename Functor>
...
public:
  BlasCuda(SpinorX &X, SpinorY &Y, SpinorZ &Z, SpinorW &W, Functor &f, int length) :
  ...
  void apply(const cudaStream_t &stream) {
    ...
    blasKernel<FloatN,M>
      <<<tp.grid, tp.block, tp.shared_bytes, stream>>>(X, Y, Z, W, f, length);
  }
