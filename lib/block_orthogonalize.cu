...
namespace quda {
                          ...
                          const int *coarse_to_fine, const int *geo_bs, int spin_bs, int n_block_ortho)
  {
    ...
    if (V.Ncolor()/Nvec == 3) {
	  ...
	  // MG Lanczos additions
          ...
          BlockOrthogonalize<vFloat, bFloat, nSpin, spinBlockSize, nColor, 768>(V, B, fine_to_coarse, coarse_to_fine,
										geo_bs, n_block_ortho);
	} else if (Nvec == 896) {
          BlockOrthogonalize<vFloat, bFloat, nSpin, spinBlockSize, nColor, 896>(V, B, fine_to_coarse, coarse_to_fine,
										geo_bs, n_block_ortho);
                          ...
                          const int *coarse_to_fine, const int *geo_bs, const int spin_bs, const int n_block_ortho)
  {
    ...
    if (V.Precision() == QUDA_DOUBLE_PRECISION && B[0]->Precision() == QUDA_DOUBLE_PRECISION) {
      ...
      BlockOrthogonalize<double, double>(V, B, fine_to_coarse, coarse_to_fine, geo_bs, spin_bs, n_block_ortho);
#else
