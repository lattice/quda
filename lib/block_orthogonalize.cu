    ...
    if (V.Ncolor()/Nvec == 3) {
      ...
      if (V.Ncolor()/Nvec == 24) {
        ...
        if (Nvec == 64) {
          ...
          BlockOrthogonalize<vFloat,bFloat,nSpin,spinBlockSize,nColor,128>(V, B, fine_to_coarse, coarse_to_fine, geo_bs, n_block_ortho);
	} else if (Nvec == 256) {
          BlockOrthogonalize<vFloat,bFloat,nSpin,spinBlockSize,nColor,256>(V, B, fine_to_coarse, coarse_to_fine, geo_bs, n_block_ortho);
	} else if (Nvec == 512) {
          BlockOrthogonalize<vFloat,bFloat,nSpin,spinBlockSize,nColor,512>(V, B, fine_to_coarse, coarse_to_fine, geo_bs, n_block_ortho);
        } else {
          errorQuda("Unsupported nVec %d\n", Nvec);
        }
