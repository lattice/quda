...
(arg.C(0, parity, x_cb, s1+ch, s2+ch, c1, c2)*((Float) 1.0))
...
__device__ __host__ inline void applyInvClover(Arg &arg, int parity, int x_cb) {
	...
	/* Rolled loop, doesn work because of the macro accessing the inverse clover */
#if 0
	for(int s = 0; s < fineSpin/2; s++) {  //Fine Spin
	  for(int s_col = 0; s_col < fineSpin/2; s_col++) { //Loop over fine spin column
	    for(int ic_c = 0; ic_c < coarseColor; ic_c++) {  //Coarse Color
	      for(int ic = 0; ic < fineColor; ic++) { //Fine Color rows of gauge field
		for(int jc = 0; jc < fineColor; jc++) {  //Fine Color columns of gauge field
		  arg.AV(parity, x_cb, s+ch, ic, ic_c) +=
		  InvClv(s, ic, s_col, jc) * arg.UV(parity, x_cb, s_col+ch, jc, ic_c);
		}  //Fine color columns
	      }  //Fine color rows
	    } //Coarse color
	  }
