...
namespace quda {
                  ...
                  const int *fine_to_coarse, const int * const * spin_map, int parity)
  {
    if (in.Precision() == QUDA_DOUBLE_PRECISION) {
      if constexpr (is_enabled_multigrid_double()) {
        Prolongate<oFloat, double, fineColor, coarseColor>(out, in, v, fine_to_coarse, spin_map, parity);
      } else {
        errorQuda("Double precision multigrid has not been enabled");
      }
    } else if (in.Precision() == QUDA_SINGLE_PRECISION) {
      Prolongate<oFloat, float, fineColor, coarseColor>(out, in, v, fine_to_coarse, spin_map, parity);
    } else {
