...
namespace quda {
  ...
  /*
  // Summed contraction type kernels using Propagator:
  // Inherit from TunableLocalParityReduction so there is a reduction class built in
  //----------------------------------------------------------------------------
  template <typename Arg> class ContractionSummedPropCompute : TunableLocalParityReduction
  {
  protected:
    Arg &arg;
    const Propagator &x;
  ...
  private:
    ...
    void initTuneParam(TuneParam &param) const {
      TunableLocalParityReduction::initTuneParam(param);
      param.block.y = 2;
      param.grid.z = x.Vectors()[0]->X(Arg::reduction_dim); // Reduction dimension is mapped to different blocks in the Z dimension
    }
  ...
  public:
    ContractionSummedPropCompute(Arg &arg, const Propagator &x, const Propagator &y, const QudaContractType cType) :
      TunableLocalParityReduction(),
      arg(arg),
      x(x),
      y(y),
      cType(cType)
    {
      ...
      strcat(aux, x.Vectors()[0]->AuxString());
#ifdef JITIFY
      create_jitify_program("kernels/contraction.cuh");
#endif
    }
    virtual ~ContractionSummedPropCompute() {}
	...
	TuneParam tp = tuneLaunch(*this, getTuning(), getVerbosity());
        ...
        std::string function_name;
        switch (cType) {
	case QUDA_CONTRACT_TYPE_OPEN_FT_T: function_name = "quda::computeDegrandRossiContractionFT"; break;
	case QUDA_CONTRACT_TYPE_OPEN_FT_Z: function_name = "quda::computeDegrandRossiContractionFT"; break;
        default: errorQuda("Unexpected contraction type %d", cType);
        }
	...
	case QUDA_CONTRACT_TYPE_DR_FT_T:
        case QUDA_CONTRACT_TYPE_DR_FT_Z:
	  LAUNCH_KERNEL_LOCAL_PARITY(computeDegrandRossiContractionPropFT, (*this), tp, stream, arg, Arg);
	  break;
	case QUDA_CONTRACT_TYPE_OPEN_FT_T:
        case QUDA_CONTRACT_TYPE_OPEN_FT_Z:
	  errorQuda("Contraction type %d not implemented", cType);
    ...
    TuneKey tuneKey() const { return TuneKey(x.Vectors()[0]->VolString(), typeid(*this).name(), aux); }
    ...
    long long flops() const
    {      
      return ((x.Vectors()[0]->Nspin() * x.Vectors()[0]->Nspin() * 3 * 6ll) + (x.Vectors()[0]->Nspin() * x.Vectors()[0]->Nspin() * (4 + 12))) * x.Vectors()[0]->Volume();
    }
    
    long long bytes() const
    {
      return x.Vectors()[0]->Bytes() + y.Vectors()[0]->Bytes();
    }
  };
  
  template <typename real>
  void contract_summed_prop_quda(const Propagator &x, const Propagator &y,
				 std::vector<Complex> &result, const QudaContractType cType,
				 const int *const source_position, const int *const pxpypzpt)
  {
    if (cType == QUDA_CONTRACT_TYPE_DR_FT_Z){
      ContractionSummedPropArg<real, 2> arg(x, y, source_position, pxpypzpt); // reduce in the z direction
      ContractionSummedPropCompute<decltype(arg)> contraction_with_sum(arg, x, y, cType);
      contraction_with_sum.apply(0);
      arg.complete(result);
    } else if (cType == QUDA_CONTRACT_TYPE_DR_FT_T) {
      ContractionSummedPropArg<real, 3> arg(x, y, source_position, pxpypzpt); // reduce in the t direction
      ContractionSummedPropCompute<decltype(arg)> contraction_with_sum(arg, x, y, cType);
      contraction_with_sum.apply(0);
      arg.complete(result);
    } else {
      errorQuda("Unexpected contraction type %d given for summed contraction kernel.", cType);
    }
  }
  //#endif //GPU_CONTRACT
  
  
  void contractSummedPropQuda(const Propagator &x, const Propagator &y,
			      std::vector<Complex> &result, const QudaContractType cType,
			      const int *const source_position, const int *const pxpypzpt)
  {
#ifdef GPU_CONTRACT

    // Checks on the propagator
    if(x.Dim() != y.Dim())
      errorQuda("Propagators must have same dimensions: %lu %lu\n", x.Dim(), y.Dim());
    
    
    if(x.Location() != y.Location()) {
      errorQuda("Propagators must have same location: %s %s\n",
		x.Location() == QUDA_CPU_FIELD_LOCATION ? "host" : "device",
		y.Location() == QUDA_CPU_FIELD_LOCATION ? "host" : "device");
    }
    
    // Checks on the colorSpinorFields. Just use 0th vector as a representative
    checkPrecision((*x.Vectors()[0]), (*y.Vectors()[0]));
    if (x.Vectors()[0]->GammaBasis() != QUDA_DEGRAND_ROSSI_GAMMA_BASIS || y.Vectors()[0]->GammaBasis() != QUDA_DEGRAND_ROSSI_GAMMA_BASIS)
      errorQuda("Unexpected gamma basis x=%d y=%d", x.Vectors()[0]->GammaBasis(), y.Vectors()[0]->GammaBasis());
    if (x.Vectors()[0]->Ncolor() != 3 || y.Vectors()[0]->Ncolor() != 3) errorQuda("Unexpected number of colors x=%d y=%d", x.Vectors()[0]->Ncolor(), y.Vectors()[0]->Ncolor());
    if (x.Vectors()[0]->Nspin() != 4  || y.Vectors()[0]->Nspin() != 4)  errorQuda("Unexpected number of spins x=%d y=%d", x.Vectors()[0]->Nspin(), y.Vectors()[0]->Nspin());
    
    if (x.Precision() == QUDA_SINGLE_PRECISION) {
      contract_summed_prop_quda<float>(x, y, result, cType, source_position, pxpypzpt);
    } else if (x.Precision() == QUDA_DOUBLE_PRECISION) {
      contract_summed_prop_quda<double>(x, y, result, cType, source_position, pxpypzpt);
    } else {
      errorQuda("Precision %d not supported", x.Precision());
    }
