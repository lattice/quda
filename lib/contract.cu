...
public:
  ...
  private:
    ...
    //- identify in the body of the ContractionSpatialSumArg code where this happened. 
    unsigned int minThreads() const { return arg.threads; }
  ...
  public:
	...
	//- `this` class (see now what *this means?) and some other data
	TuneParam tp = tuneLaunch(*this, getTuning(), getVerbosity());
	...
	case QUDA_CONTRACT_TYPE_OPEN_SUM: function_name = "quda::computeColorContractionSum"; break;
	  ...
	  .launch(arg);
#else
	//- Here are are at the calling of the compute kernels, albeit wrapped in Macros!
	//- computeColorContractionSum will contract only the colour indices, leaving the 16
	//- complex numbers per open spin index per lattice site
	//- (one for each \mu, and \nu combination) and then sum each one of those 16 elements
	//- with its counterparts on the same timeslice.
			     ...
			     const size_t b1, complex<real> *result, const QudaContractType cType)
  {
    if(cType == QUDA_CONTRACT_TYPE_OPEN_SUM || cType == QUDA_CONTRACT_TYPE_DR_SUM) {
      ContractionSpatialSumArg<real> arg(x, y, s1, b1);
      ContractionSpatialSumCompute<decltype(arg)> contraction_with_sum(arg, x, y, cType);
  ...
  void contractSpatialQuda(const ColorSpinorField &x, const ColorSpinorField &y, const size_t s1, const size_t b1, void *result, const QudaContractType cType)
  {
#ifdef GPU_CONTRACT
    //- After some checks, 
    checkPrecision(x, y);
    ...
    if (x.Precision() == QUDA_SINGLE_PRECISION) {
      contract_spatial_quda<float>(x, y, s1, b1, (complex<float> *)result, cType);
    } else if (x.Precision() == QUDA_DOUBLE_PRECISION) {
      contract_spatial_quda<double>(x, y, s1, b1, (complex<double> *)result, cType);
    } else {
