 ...
 */
inline void SetPlanFFTMany(cufftHandle &plan, int4 size, int dim, QudaPrecision precision)
{
  auto type = precision == QUDA_DOUBLE_PRECISION ? CUFFT_Z2Z : CUFFT_C2C;
  switch (dim) {
  case 1:
  { int n[1] = { size.w };
    cufftPlanMany(&plan, 1, n, NULL, 1, 0, NULL, 1, 0, type, size.x * size.y * size.z); }
  break;
  case 3:
  { int n[3] = { size.x, size.y, size.z };
    cufftPlanMany(&plan, 3, n, NULL, 1, 0, NULL, 1, 0, type, size.w); }
  break;
  }
...
inline void SetPlanFFT2DMany(cufftHandle &plan, int4 size, int dim, QudaPrecision precision)
{
  ...
  { int n[2] = { size.w, size.z };
    cufftPlanMany(&plan, 2, n, NULL, 1, 0, NULL, 1, 0, type, size.x * size.y); }
  break;
  case 1:
  { int n[2] = { size.x, size.y };
    cufftPlanMany(&plan, 2, n, NULL, 1, 0, NULL, 1, 0, type, size.z * size.w); }
  break;
  }
