...
namespace quda {
  ...
  protected:
    ...
    //int blockStep() const { return deviceProp.qudaWarpSize/4; }
    //int blockMin() const { return deviceProp.qudaWarpSize/4; }
    ...
    bool advanceAux(TuneParam &param) const
    {
	  ...
	  param.block.x % deviceProp.qudaWarpSize == 0) {
    ...
    virtual void defaultTuneParam(TuneParam &param) const
    {
      ...
      // ensure that the default x block size is divisible by the qudaWarpSize
      param.block.x = deviceProp.qudaWarpSize;
      param.grid.x = (minThreads()+param.block.x-1)/param.block.x;
      param.block.z = dim_threads * 2;
      param.grid.z = 2*(Nc/Mc);
      param.shared_bytes = sharedBytesPerThread()*param.block.x*param.block.y*param.block.z > sharedBytesPerBlock(param) ?
