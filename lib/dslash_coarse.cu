...
namespace quda {
  ...
  template <typename Float, typename F, typename G, int nDim, int Ns, int Nc, int Mc, bool xpay>
  __device__ __host__ inline void coarseDslash(CoarseDslashArg<Float,F,G> &arg, int x_cb, int parity, int s, int color_block)
  {
    ...
    if (xpay) clover<Float,F,G,Ns,Nc,Mc>(out, arg, x_cb, parity, s, color_block);
  ...
  void coarseDslash(CoarseDslashArg<Float,F,G> arg)
  {
	...
	for (int s=0; s<2; s++) {
	  for (int color_block=0; color_block<Nc; color_block+=Mc) { // Mc=Nc means all colors in a thread
	    coarseDslash<Float,F,G,nDim,Ns,Nc,Mc,xpay>(arg, x_cb, parity, s, color_block);
	  }
	}
      }//VolumeCB
    } // parity
  ...
  protected:
    ...
    {
      return ((2*nDim+xpay*1)*(8*Ns*Nc*Ns*Nc)-2*Ns*Nc)*arg.nParity*arg.volumeCB;
    }
    long long bytes() const
    {
      return arg.out.Bytes() + 8*arg.inA.Bytes() + xpay*arg.inB.Bytes() + arg.nParity*(8*arg.Y.Bytes() + xpay*arg.X.Bytes());
    }
  ...
  public:
	...
	coarseDslash<Float,F,G,nDim,Ns,Nc,Mc,xpay>(arg);
      } else {
	TuneParam tp = tuneLaunch(*this, getTuning(), getVerbosity());
	coarseDslashKernel<Float,F,G,nDim,Ns,Nc,Mc,xpay> <<<tp.grid,tp.block,tp.shared_bytes,stream>>>(arg);
      }
    }
  ...
  void ApplyCoarse(ColorSpinorField &out, const ColorSpinorField &inA, const ColorSpinorField &inB,
		   const GaugeField &Y, const GaugeField &X, double kappa, int parity, bool xpay) {
    ...
    if (xpay) {
      CoarseDslash<Float,F,G,4,coarseSpin,coarseColor,colors_per_thread,true> dslash(arg, inA);
      dslash.apply(0);
    } else {
      CoarseDslash<Float,F,G,4,coarseSpin,coarseColor,colors_per_thread,false> dslash(arg, inA);
      dslash.apply(0);
    }
  }
  ...
  void ApplyCoarse(ColorSpinorField &out, const ColorSpinorField &inA, const ColorSpinorField &inB,
    ...
    if (inA.Location() == QUDA_CUDA_FIELD_LOCATION) {
      ApplyCoarse<Float,csOrder,gOrder,coarseColor,coarseSpin,QUDA_CUDA_FIELD_LOCATION>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else {
      ApplyCoarse<Float,csOrder,gOrder,coarseColor,coarseSpin,QUDA_CPU_FIELD_LOCATION>(out, inA, inB, Y, X, kappa, parity, xpay);
    }
  }
  ...
  void ApplyCoarse(ColorSpinorField &out, const ColorSpinorField &inA, const ColorSpinorField &inB,
    ...
    if (inA.Ncolor() == 2) {
      ApplyCoarse<Float,csOrder,gOrder,2,2>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (inA.Ncolor() == 4) {
      ApplyCoarse<Float,csOrder,gOrder,4,2>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (inA.Ncolor() == 8) {
      ApplyCoarse<Float,csOrder,gOrder,8,2>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (inA.Ncolor() == 12) {
      ApplyCoarse<Float,csOrder,gOrder,12,2>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (inA.Ncolor() == 16) {
      ApplyCoarse<Float,csOrder,gOrder,16,2>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (inA.Ncolor() == 20) {
      ApplyCoarse<Float,csOrder,gOrder,20,2>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (inA.Ncolor() == 24) {
      ApplyCoarse<Float,csOrder,gOrder,24,2>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else {
  ...
  void ApplyCoarse(ColorSpinorField &out, const ColorSpinorField &inA, const ColorSpinorField &inB,
    ...
    if (inA.FieldOrder() == QUDA_FLOAT2_FIELD_ORDER && Y.FieldOrder() == QUDA_FLOAT2_GAUGE_ORDER) {
      ApplyCoarse<Float,QUDA_FLOAT2_FIELD_ORDER, QUDA_FLOAT2_GAUGE_ORDER>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (inA.FieldOrder() == QUDA_SPACE_SPIN_COLOR_FIELD_ORDER && Y.FieldOrder() == QUDA_QDP_GAUGE_ORDER) {
      ApplyCoarse<Float,QUDA_SPACE_SPIN_COLOR_FIELD_ORDER,QUDA_QDP_GAUGE_ORDER>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else {
  ...
  void ApplyCoarse(ColorSpinorField &out, const ColorSpinorField &inA, const ColorSpinorField &inB,
    ...
    if (Y.Precision() == QUDA_DOUBLE_PRECISION) {
      ApplyCoarse<double>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else if (Y.Precision() == QUDA_SINGLE_PRECISION) {
      ApplyCoarse<float>(out, inA, inB, Y, X, kappa, parity, xpay);
    } else {
