...
//for twisted mass only:
  ...
  void cudaCloverField::copy(const CloverField &src, bool inverse) {
    ...
    if (typeid(src) == typeid(cudaCloverField)) {
      ...
      void *packCloverNorm = (precision == QUDA_HALF_PRECISION || precision == QUDA_QUARTER_PRECISION) ? static_cast<char*>(packClover) + bytes : 0;
	...
	if (precision == QUDA_HALF_PRECISION || precision == QUDA_QUARTER_PRECISION)
      ...
      void *packCloverNorm = (precision == QUDA_HALF_PRECISION || precision == QUDA_QUARTER_PRECISION) ? static_cast<char*>(packClover) + src.Bytes() : 0;
