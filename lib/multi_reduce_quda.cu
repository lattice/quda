...
#include <multi_reduce_core.h>
    ...
    */
    ...
    template <int NXZ, typename ReduceType, typename Float2, typename FloatN>
    struct Rdot : public MultiReduceFunctor<NXZ, ReduceType, Float2, FloatN> {
      typedef typename scalar<Float2>::type real;
      const int NYW;
      Rdot(const reduce::coeff_array<double> &a, const reduce::coeff_array<double> &b, const reduce::coeff_array<double> &c, int NYW) : NYW(NYW) { ; }
    ...
    */
    ...
    template<typename T>
    void convertResult(T* out, const double2* in, const unsigned int xlen, const unsigned int ylen) { 
      warningQuda("Block reduction did not convert the output, the result of the operation can be wrong!\n"); 
    }

    template<> void convertResult<double>(double* out, const double2* in, const unsigned int xlen, const unsigned int ylen) {

      for (unsigned int j = 0; j < xlen; j++)
         for (unsigned int i = 0; i < ylen; i++)
            out[i*xlen+j] = in[j*ylen + i].x;

    }

    template<> void convertResult<Complex>(Complex* out, const double2* in, const unsigned int xlen, const unsigned int ylen) {

      for (unsigned int j = 0; j < xlen; j++)
         for (unsigned int i = 0; i < ylen; i++)
            out[i*xlen+j] = Complex(in[j*ylen + i].x, in[j*ylen+i].y);
    }
    ...
    template <typename ReduceType, template <int MXZ, typename ReducerType, typename Float, typename FloatN> class ReducerDiagonal, typename writeDiagonal,
	      template <int MXZ, typename ReducerType, typename Float, typename FloatN> class ReducerOffDiagonal, typename writeOffDiagonal>
    void multiReduce_recurse(ReduceType* result, std::vector<ColorSpinorField*>& x, std::vector<ColorSpinorField*>& y,
      ...
      if (y.size() > tile_size) // if greater than max single-kernel size, split and recurse
      {
        ...
        ReduceType* result0 = &result[0];
        ReduceType* result1 = &result[x.size()*(y.size()/2)];
        ...
        multiReduce_recurse<ReduceType, ReducerDiagonal,writeDiagonal,ReducerOffDiagonal,writeOffDiagonal>(result0, x, y0, z, w0, i_idx, 2*j_idx+0, hermitian, tile_size);
        multiReduce_recurse<ReduceType, ReducerDiagonal,writeDiagonal,ReducerOffDiagonal,writeOffDiagonal>(result1, x, y1, z, w1, i_idx, 2*j_idx+1, hermitian, tile_size);
      }
      else
      {
        double2* dot2 = new double2[x.size()*y.size()];
	...
	if (x.size() <= tile_size && hermitian) {
	  if (j_idx < i_idx) { return; }
        ...
        reduce::coeff_array<ReduceType> a, b, c;
	...
	if (x.size() <= tile_size) {
	    ...
	    (dot2, a, b, c, x, y, z, w, i_idx, j_idx );
          break;
	...
	case 9:
	    ...
	    (dot2, a, b, c, x, y, z, w, i_idx, j_idx );
          ...
          ReduceType* tmpmajor = new ReduceType[x.size()*y.size()];
          ReduceType* result0 = &tmpmajor[0];
          ReduceType* result1 = &tmpmajor[(x.size()/2)*y.size()];
          ...
          multiReduce_recurse<ReduceType, ReducerDiagonal,writeDiagonal,ReducerOffDiagonal,writeOffDiagonal>(result0, x0, y, z0, w, 2*i_idx+0, j_idx, hermitian, tile_size);
          multiReduce_recurse<ReduceType, ReducerDiagonal,writeDiagonal,ReducerOffDiagonal,writeOffDiagonal>(result1, x1, y, z1, w, 2*i_idx+1, j_idx, hermitian, tile_size);
	...
	// we are at the leaf of the binary tree (e.g., we ran the kernel): perform the row-to-column-major transpose here.
        if (x.size() <= tile_size) convertResult<ReduceType>(result, dot2, x.size(), y.size());

        delete[] dot2;
      }
    }

    template <typename ReduceType, template <int MXZ, typename ReducerType, typename Float, typename FloatN> class ReducerDiagonal,
    ...
    public:
      TileSizeTune(ReduceType *result, vec &x, vec &y, vec &z, vec &w, bool hermitian, bool Anorm = false)
	...
	if (getTuning() && getTuneCache().find(tuneKey()) == getTuneCache().end()) {
	  ...
	  if ( x.size()==1 || y.size()==1 ) { // 1-d reduction
	    ...
	    for ( unsigned int tile_size=1; tile_size <= max_tile_size; tile_size++) {
	      multiReduce_recurse<ReduceType, ReducerDiagonal,writeDiagonal,ReducerOffDiagonal,writeOffDiagonal>
    ...
    void cDotProduct(Complex* result, std::vector<ColorSpinorField*>& x, std::vector<ColorSpinorField*>& y){
      ...
      TileSizeTune<Complex,Cdot,write<0,0,0,0>,Cdot,write<0,0,0,0> > tile(result_tmp, x, y, x, y, false);
      tile.apply(0);
    ...
    void hDotProduct(Complex* result, std::vector<ColorSpinorField*>& x, std::vector<ColorSpinorField*>& y){
      ...
      TileSizeTune<Complex, Cdot,write<0,0,0,0>,Cdot,write<0,0,0,0> > tile(result_tmp, x, y, x, y, true, false); // last false is b/c L2 norm
      tile.apply(0);
    ...
    void hDotProduct_Anorm(Complex* result, std::vector<ColorSpinorField*>& x, std::vector<ColorSpinorField*>& y){
      ...
      TileSizeTune<Complex, Cdot,write<0,0,0,0>,Cdot,write<0,0,0,0> > tile(result_tmp, x, y, x, y, true, true); // last true is b/c A norm
      tile.apply(0);
    ...
    void cDotProductCopy(Complex* result, std::vector<ColorSpinorField*>& x, std::vector<ColorSpinorField*>& y,
      ...
      TileSizeTune<Complex, CdotCopy,write<0,0,0,1>,Cdot,write<0,0,0,0> > tile(result_tmp, x, y, x, y, true);
      tile.apply(0);
    ...
    void reDotProduct(double* result, std::vector<ColorSpinorField*>& x, std::vector<ColorSpinorField*>& y){
      ...
      // To be consistent with the multi-blas functions, we should
      // switch this to row-major.
      TileSizeTune<double,Rdot,write<0,0,0,0>,Rdot,write<0,0,0,0> > tile(result_tmp, x, y, x, y, false);
      tile.apply(0);
