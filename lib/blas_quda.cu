...
public:
  BlasCuda(SpinorX &X, SpinorY &Y, SpinorZ &Z, SpinorW &W, SpinorV &V, Functor &f,
           ...
           ColorSpinorField &w, ColorSpinorField &v, int length) :
    nParity( (x.IsComposite() ? x.CompositeDim() : 1) * x.SiteSubset() ), // must be first
    arg(X, Y, Z, W, V, f, length/nParity), x(x), y(y), z(z), w(w), v(v),
  ...
  inline TuneKey tuneKey() const {
    return TuneKey(x.VolString(), typeid(arg.f).name(), aux);
  }
...
void nativeBlas(const double2 &a, const double2 &b, const double2 &c,
  ...
  BlasCuda<RegType,M, decltype(X), decltype(Y), decltype(Z), decltype(W), decltype(V),
           Functor<Float2, RegType> > blas(X, Y, Z, W, V, f, x, y, z, w, v, length);
...
void uni_blas(const double2 &a, const double2 &b, const double2 &c, ColorSpinorField &x,
  ...
  if (checkLocation(x, y, z, w, v) == QUDA_CUDA_FIELD_LOCATION) {
    ...
    if (x.Precision() == QUDA_DOUBLE_PRECISION) {
      ...
      if (x.Nspin() == 4 && x.FieldOrder() == QUDA_FLOAT4_FIELD_ORDER) {
        ...
        const int M = 1;
        nativeBlas<float4,float4,float4,M,Functor,writeX,writeY,writeZ,writeW,writeV>(a,b,c,x,y,z,w,v,x.Length()/(4*M));
#else
	errorQuda("blas has not been built for Nspin=%d fields", x.Nspin());
#endif
...
#if defined(GPU_WILSON_DIRAC) || defined(GPU_DOMAIN_WALL_DIRAC) || defined(GPU_STAGGERED_DIRAC)
	const int M = 1;
	nativeBlas<float2,float2,float2,M,Functor,writeX,writeY,writeZ,writeW,writeV>(a,b,c,x,y,z,w,v,x.Length()/(2*M));
#else
	errorQuda("blas has not been built for Nspin=%d fields", x.Nspin());
#endif
...
#if defined(GPU_WILSON_DIRAC) || defined(GPU_DOMAIN_WALL_DIRAC)
	const int M = 6;
	nativeBlas<float4,short4,short4,M,Functor,writeX,writeY,writeZ,writeW,writeV>(a,b,c,x,y,z,w,v,x.Volume());
#else
	errorQuda("blas has not been built for Nspin=%d fields", x.Nspin());
#endif
...
#if defined(GPU_WILSON_DIRAC) || defined(GPU_DOMAIN_WALL_DIRAC)
	const int M = 12;
	nativeBlas<float2,short2,short2,M,Functor,writeX,writeY,writeZ,writeW,writeV>(a,b,c,x,y,z,w,v,x.Volume());
#else
	errorQuda("blas has not been built for Nspin=%d fields", x.Nspin());
#endif
...
#if defined(GPU_WILSON_DIRAC) || defined(GPU_DOMAIN_WALL_DIRAC)
        const int M = 6;
        nativeBlas<float4,char4,char4,M,Functor,writeX,writeY,writeZ,writeW,writeV>(a,b,c,x,y,z,w,v,x.Volume());
#else
        errorQuda("blas has not been built for Nspin=%d fields", x.Nspin());
#endif
      } else if (x.Nspin() == 1) {//staggered
#ifdef GPU_STAGGERED_DIRAC
        const int M = 3;
        nativeBlas<float2,char2,char2,M,Functor,writeX,writeY,writeZ,writeW,writeV>(a,b,c,x,y,z,w,v,x.Volume());
#else
        errorQuda("blas has not been built for Nspin=%d fields", x.Nspin());
#endif
      } else {
...
template <template <typename Float, typename FloatN> class Functor,
          int writeX=0, int writeY=0, int writeZ=0, int writeW=0, int writeV=0>
void mixed_blas(const double2 &a, const double2 &b, const double2 &c, ColorSpinorField &x,
                ColorSpinorField &y, ColorSpinorField &z, ColorSpinorField &w, ColorSpinorField &v) {
  ...
  if (checkLocation(x, y, z, w, v) == QUDA_CUDA_FIELD_LOCATION) {
	...
	mixed_blas<axpbyz_,0,0,0,0,1>(make_double2(a,0.0), make_double2(b,0.0), make_double2(0.0,0.0),
                                      x, y, x, x, z);
      } else {
	uni_blas<axpbyz_,0,0,0,0,1>(make_double2(a, 0.0), make_double2(b, 0.0), make_double2(0.0, 0.0),
                                    x, y, x, x, z);
      }
    }
    ...
    void caxpy(const Complex &a, ColorSpinorField &x, ColorSpinorField &y) {
      if (x.Precision() != y.Precision()) {
        mixed_blas<caxpy_,0,1>(make_double2(real(a),imag(a)), make_double2(0.0, 0.0),
                               make_double2(0.0, 0.0), x, y, x, x, y);
      } else {
        uni_blas<caxpy_,0,1>(make_double2(real(a),imag(a)), make_double2(0.0, 0.0),
                             make_double2(0.0, 0.0), x, y, x, x, y);
      }
    }
    ...
    void caxpby(const Complex &a, ColorSpinorField &x, const Complex &b, ColorSpinorField &y) {
      uni_blas<caxpby_,0,1>(make_double2(REAL(a),IMAG(a)), make_double2(REAL(b), IMAG(b)),
                            make_double2(0.0, 0.0), x, y, x, x, y);
    }

    void cxpaypbz(ColorSpinorField &x, const Complex &a, ColorSpinorField &y,
		  const Complex &b, ColorSpinorField &z) {
      uni_blas<cxpaypbz_,0,0,1>(make_double2(REAL(a),IMAG(a)), make_double2(REAL(b), IMAG(b)),
    ...
    void axpyBzpcx(double a, ColorSpinorField& x, ColorSpinorField& y, double b,
      ...
      if (x.Precision() != y.Precision()) {
	// call hacked mixed precision kernel
	mixed_blas<axpyBzpcx_,1,1>(make_double2(a,0.0), make_double2(b,0.0),
                                   make_double2(c,0.0), x, y, z, x, y);
      } else {
	// swap arguments around
	uni_blas<axpyBzpcx_,1,1>(make_double2(a,0.0), make_double2(b,0.0),
                                 make_double2(c,0.0), x, y, z, x, y);
      }
    }
    ...
    void axpyZpbx(double a, ColorSpinorField& x, ColorSpinorField& y,
      ...
      if (x.Precision() != y.Precision()) {
	// call hacked mixed precision kernel
	mixed_blas<axpyZpbx_,1,1>(make_double2(a,0.0), make_double2(b,0.0), make_double2(0.0,0.0),
                                  x, y, z, x, y);
      } else {
	// swap arguments around
	uni_blas<axpyZpbx_,1,1>(make_double2(a,0.0), make_double2(b,0.0), make_double2(0.0,0.0),
                                x, y, z, x, y);
      }
    }
    ...
    void caxpyBzpx(const Complex &a, ColorSpinorField &x,
      ...
      if (x.Precision() != y.Precision()) {
        mixed_blas<caxpyBzpx_,1,1>(make_double2(REAL(a),IMAG(a)), make_double2(REAL(b), IMAG(b)),
                                   make_double2(0.0,0.0), x, y, z, x, y);
      } else {
        uni_blas<caxpyBzpx_,1,1>(make_double2(REAL(a),IMAG(a)), make_double2(REAL(b), IMAG(b)),
                                 make_double2(0.0,0.0), x, y, z, x, y);
      }
    }
    ...
    void caxpyBxpz(const Complex &a, ColorSpinorField &x,
		      ColorSpinorField &y, const Complex &b, ColorSpinorField &z) {
      if (x.Precision() != y.Precision()) {
        mixed_blas<caxpyBxpz_,0,1,1>(make_double2(REAL(a),IMAG(a)), make_double2(REAL(b), IMAG(b)),
                                     make_double2(0.0,0.0), x, y, z, x, y);
      } else {
        uni_blas<caxpyBxpz_,0,1,1>(make_double2(REAL(a),IMAG(a)), make_double2(REAL(b), IMAG(b)),
                                   make_double2(0.0,0.0), x, y, z, x, y);
      }
    }
    ...
    void caxpbypzYmbw(const Complex &a, ColorSpinorField &x, const Complex &b,
      ...
      uni_blas<caxpbypzYmbw_,0,1,1>(make_double2(REAL(a),IMAG(a)), make_double2(REAL(b), IMAG(b)),
                                    make_double2(0.0,0.0), x, y, z, w, y);
    }

    void cabxpyAx(double a, const Complex &b, ColorSpinorField &x, ColorSpinorField &y) {
      // swap arguments around
      uni_blas<cabxpyAx_,1,1>(make_double2(a,0.0), make_double2(REAL(b),IMAG(b)),
                              make_double2(0.0,0.0), x, y, x, x, y);
    }

    void caxpyXmaz(const Complex &a, ColorSpinorField &x,
		   ColorSpinorField &y, ColorSpinorField &z) {
      uni_blas<caxpyxmaz_,1,1>(make_double2(REAL(a), IMAG(a)), make_double2(0.0, 0.0),
                               make_double2(0.0, 0.0), x, y, z, x, y);
    }
	...
	errorQuda("This kernel cannot be run on CPU fields");
      ...
      uni_blas<caxpyxmazMR_,1,1>(make_double2(REAL(a), IMAG(a)), make_double2(0.0, 0.0),
                                 make_double2(0.0, 0.0), x, y, z, x, y);
    }
    ...
    void tripleCGUpdate(double a, double b, ColorSpinorField &x,
      ...
      if (x.Precision() != y.Precision()) {
      // call hacked mixed precision kernel
	mixed_blas<tripleCGUpdate_,0,1,1,1>(make_double2(a,0.0), make_double2(b,0.0),
                                            make_double2(0.0,0.0), x, y, z, w, y);
      } else {
	uni_blas<tripleCGUpdate_,0,1,1,1>(make_double2(a, 0.0), make_double2(b, 0.0),
                                          make_double2(0.0, 0.0), x, y, z, w, y);
      }
    }

    void doubleCG3Init(double a, ColorSpinorField &x, ColorSpinorField &y, ColorSpinorField &z) {
      uni_blas<doubleCG3Init_,1,1,0,0>(make_double2(a, 0.0), make_double2(0.0, 0.0),
                                       make_double2(0.0, 0.0), x, y, z, z, y);
    }

    void doubleCG3Update(double a, double b, ColorSpinorField &x, ColorSpinorField &y, ColorSpinorField &z) {
      uni_blas<doubleCG3Update_,1,1,0,0>(make_double2(a, 0.0), make_double2(b, 1.0-b),
                                         make_double2(0.0, 0.0), x, y, z, z, y);
    }
