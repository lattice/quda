...
namespace quda {
  namespace mobius_eofa {
    ...
    static __constant__ char m5_shift_d[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static __constant__ char m5inv_shift_lc_d[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static __constant__ char m5inv_shift_norm_d[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static __constant__ char m5inv_shift_dagger_lc_d[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static __constant__ char m5inv_shift_dagger_norm_d[size]; // constant buffer used for Mobius coefficients for GPU kernel
    
    static char m5_shift_h[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static char m5inv_shift_lc_h[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static char m5inv_shift_norm_h[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static char m5inv_shift_dagger_lc_h[size]; // constant buffer used for Mobius coefficients for GPU kernel
    static char m5inv_shift_dagger_norm_h[size]; // constant buffer used for Mobius coefficients for GPU kernel
        ...
        Dslash5Arg(ColorSpinorField &out, const ColorSpinorField &in, const ColorSpinorField &x,
          ...
          m_f(m_f), m_5(m_5), a(a_), dagger(dagger), xpay(a_ == 0.0 ? false : true), type(type)
          {
            ...
            switch(type) {
              case m5_eofa:
                int idx = 0;
                double N = ( (eofa_pm == 1) ? 1.0 : -1.0 ) * (2.0*eofa_shift*eofa_norm) * ( std::pow(alpha+1.0,Ls) + mq1*std::pow(alpha-1.0,Ls) );
                for(int s=0; s<Ls; ++s){
                  idx = (pm == 1) ? (s) : (Ls-1-s);
                  m5_shift_h[idx] = eofa_norm * N * std::pow(-1.0,s) * std::pow(alpha-1.0,s) / std::pow(alpha+1.0,Ls+s+1);
                } 
     ...
     */
    template <typename Float, int nColor, bool dagger, bool pm, bool xpay, Dslash5Type type, typename Arg>
      __device__ __host__ inline void dslash5(Arg &arg, int parity, int x_cb, int s) {
        ...
        { // forwards direction
          const Vector in = cache.load(threadIdx.x, (s+1)%arg.Ls, parity);
        ...
        { // backwards direction
          const Vector in = cache.load(threadIdx.x, (s+arg.Ls-1)%arg.Ls, parity);
          constexpr int proj_dir = dagger ? -1 : +1;
          if (s == 0) {
            out += (-arg.m_f * in.project(4, proj_dir)).reconstruct(4, proj_dir);
          } else {
        ...
        if (type == m5_eofa) {
          ...
          out += m5_shift_d[s] * arg.in((Ls-1)*arg.volume_4d_cb+x_cb).project(4,-1).reconstruct(4,-1);  
        }else if(!dagger && !pm){
          out += m5_shift_d[s] * arg.in((  0 )*arg.volume_4d_cb+x_cb).project(4,-1).reconstruct(4,-1);  
        }
    ...
    /**
      ...
      @param[in] s Ls dimension coordinate
     */
    template <typename Float, int nColor, bool dagger, bool xpay, Dslash5Type type,
             bool shared, bool var_inverse, typename Arg>
    ...
    void apply(const cudaStream_t &stream) {
      if (meta.Location() == QUDA_CPU_FIELD_LOCATION) {
        ...
        if (arg.type == DSLASH5_DWF) {
          ...
          if (arg.xpay) arg.dagger ?
            dslash5invCPU<Float,nColor, true, true,M5_INV_DWF,shared,var_inverse>(arg) :
              dslash5invCPU<Float,nColor,false, true,M5_INV_DWF,shared,var_inverse>(arg);
          else          arg.dagger ?
            dslash5invCPU<Float,nColor, true,false,M5_INV_DWF,shared,var_inverse>(arg) :
              dslash5invCPU<Float,nColor,false,false,M5_INV_DWF,shared,var_inverse>(arg);
        } else if (arg.type == M5_INV_MOBIUS) {
          if (arg.xpay) arg.dagger ?
            dslash5invCPU<Float,nColor, true, true,M5_INV_MOBIUS,shared,var_inverse>(arg) :
              dslash5invCPU<Float,nColor,false, true,M5_INV_MOBIUS,shared,var_inverse>(arg);
          else          arg.dagger ?
            dslash5invCPU<Float,nColor, true,false,M5_INV_MOBIUS,shared,var_inverse>(arg) :
              dslash5invCPU<Float,nColor,false,false,M5_INV_MOBIUS,shared,var_inverse>(arg);
        } else if (arg.type == M5_INV_ZMOBIUS) {
          if (arg.xpay) arg.dagger ?
            dslash5invCPU<Float,nColor, true, true,M5_INV_ZMOBIUS,shared,var_inverse>(arg) :
              dslash5invCPU<Float,nColor,false, true,M5_INV_ZMOBIUS,shared,var_inverse>(arg);
          else          arg.dagger ?
            dslash5invCPU<Float,nColor, true,false,M5_INV_ZMOBIUS,shared,var_inverse>(arg) :
