...
namespace quda {
  ...
  void extractGhost(const GaugeField &u, Float **Ghost, bool extract) {
	...
	if (typeid(Float)==typeid(short) && u.LinkType() == QUDA_ASQTAD_FAT_LINKS) {
	  extractGhost<short,length>(FloatNOrder<short,length,2,19>
				     (u, 0, (short**)Ghost), u, location, extract);
	} else {
	...
	extractGhost<Float,length>(G(u, 0, Ghost), u, location, extract);
      ...
      extractGhost<Float,length>(QDPJITOrder<Float,length>(u, 0, Ghost), u, location, extract);
#else
      ...
      extractGhost<Float,length>(CPSOrder<Float,length>(u, 0, Ghost), u, location, extract);
#else
      ...
      extractGhost<Float,length>(MILCOrder<Float,length>(u, 0, Ghost), u, location, extract);
#else
      ...
      extractGhost<Float,length>(BQCDOrder<Float,length>(u, 0, Ghost), u, location, extract);
#else
      ...
      extractGhost<Float,length>(TIFROrder<Float,length>(u, 0, Ghost), u, location, extract);
#else
  ...
  void extractGaugeGhost(const GaugeField &u, void **ghost, bool extract) {

    // if number of colors doesn't equal three then we must have
    // coarse-gauge field
    if (u.Ncolor() != 3) {
      extractGaugeGhostMG(u, ghost, extract);
    } else {
      if (u.Precision() == QUDA_DOUBLE_PRECISION) {
	extractGhost(u, (double**)ghost, extract);
      } else if (u.Precision() == QUDA_SINGLE_PRECISION) {
	extractGhost(u, (float**)ghost, extract);
      } else if (u.Precision() == QUDA_HALF_PRECISION) {
	extractGhost(u, (short**)ghost, extract);
      } else {
