...
namespace quda {
  ...
  template <typename Float, int nColor_, QudaReconstructType recon_> struct KernelArg : public ReduceArg<double2> {
    ...
    KernelArg(const GaugeField &data) : ReduceArg<double2>(), dataOr(data), threads(data.LocalVolumeCB())
    {
  ...
  template <int blockSize, int type, typename Arg> __global__ void compute(Arg arg)
  {
...
#pragma unroll
  ...
  public:
    CalcFunc(double2 &result, const GaugeField &u) : result(result), u(u) { apply(0); }
    ...
    {
      if (u.Ncolor() == 3 && type == 0)
        return 264LL * u.LocalVolume();
      else if (type == 1)
        return 2 * u.Geometry() * u.Ncolor() * u.LocalVolume();
      else
        return 0;
    }
  ...
  template <typename Float, int nColor, QudaReconstructType recon> struct computeDeterminant {
    computeDeterminant(GaugeField &data, double2 &det) { CalcFunc<Float, nColor, recon, 0>(det, data); }
  };

  template <typename Float, int nColor, QudaReconstructType recon> struct computeTrace {
    computeTrace(GaugeField &data, double2 &trace) { CalcFunc<Float, nColor, recon, 1>(trace, data); }
  };
