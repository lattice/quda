...
namespace quda
{
  ...
  template <int fineSpin, int coarseSpin, StaggeredTransferType transferType> struct StaggeredTransferOutSpin {
    static constexpr int outSpin = -1;
  };
  template <int fineSpin, int coarseSpin>
  struct StaggeredTransferOutSpin<fineSpin, coarseSpin, StaggeredTransferType::STAGGERED_TRANSFER_PROLONG> {
    static constexpr int outSpin = fineSpin;
  };
  template <int fineSpin, int coarseSpin>
  struct StaggeredTransferOutSpin<fineSpin, coarseSpin, StaggeredTransferType::STAGGERED_TRANSFER_RESTRICT> {
    static constexpr int outSpin = coarseSpin;
  };

  // Use a trait to define whether the "in" spin is the fine or coarse spin
  template <int fineSpin, int coarseSpin, StaggeredTransferType transferType> struct StaggeredTransferInSpin {
    static constexpr int inSpin = -1;
  };
  template <int fineSpin, int coarseSpin>
  struct StaggeredTransferInSpin<fineSpin, coarseSpin, StaggeredTransferType::STAGGERED_TRANSFER_PROLONG> {
    static constexpr int inSpin = coarseSpin;
  };
  template <int fineSpin, int coarseSpin>
  struct StaggeredTransferInSpin<fineSpin, coarseSpin, StaggeredTransferType::STAGGERED_TRANSFER_RESTRICT> {
    static constexpr int inSpin = fineSpin;
  };

  // Use a trait to define whether the "out" color is the fine or coarse color
  template <int fineColor, int coarseColor, StaggeredTransferType transferType> struct StaggeredTransferOutColor {
    static constexpr int outColor = -1;
  };
  template <int fineColor, int coarseColor>
  struct StaggeredTransferOutColor<fineColor, coarseColor, StaggeredTransferType::STAGGERED_TRANSFER_PROLONG> {
    static constexpr int outColor = fineColor;
  };
  template <int fineColor, int coarseColor>
  struct StaggeredTransferOutColor<fineColor, coarseColor, StaggeredTransferType::STAGGERED_TRANSFER_RESTRICT> {
    static constexpr int outColor = coarseColor;
  };

  // Use a trait to define whether the "in" color is the fine or coarse color
  template <int fineColor, int coarseColor, StaggeredTransferType transferType> struct StaggeredTransferInColor {
    static constexpr int inColor = -1;
  };
  template <int fineColor, int coarseColor>
  struct StaggeredTransferInColor<fineColor, coarseColor, StaggeredTransferType::STAGGERED_TRANSFER_PROLONG> {
    static constexpr int inColor = coarseColor;
  };
  template <int fineColor, int coarseColor>
  struct StaggeredTransferInColor<fineColor, coarseColor, StaggeredTransferType::STAGGERED_TRANSFER_RESTRICT> {
    static constexpr int inColor = fineColor;
  };
  ...
  template <>
  inline const ColorSpinorField &
  fineColorSpinorField<StaggeredTransferType::STAGGERED_TRANSFER_PROLONG>(const ColorSpinorField &quoteIn,
                                                                          const ColorSpinorField &quoteOut)
  {
    return quoteOut;
  }
  ...
  template <>
  inline const ColorSpinorField &
  fineColorSpinorField<StaggeredTransferType::STAGGERED_TRANSFER_RESTRICT>(const ColorSpinorField &quoteIn,
                                                                           const ColorSpinorField &quoteOut)
  {
    return quoteIn;
  }
  ...
  template <>
  inline const ColorSpinorField &
  coarseColorSpinorField<StaggeredTransferType::STAGGERED_TRANSFER_PROLONG>(const ColorSpinorField &quoteIn,
                                                                            const ColorSpinorField &quoteOut)
  {
    return quoteIn;
  }
  ...
  template <>
  inline const ColorSpinorField &
  coarseColorSpinorField<StaggeredTransferType::STAGGERED_TRANSFER_RESTRICT>(const ColorSpinorField &quoteIn,
                                                                             const ColorSpinorField &quoteOut)
  {
    return quoteOut;
  }
  ...
  */
  template <typename Float, int fineSpin, int fineColor, int coarseSpin, int coarseColor, QudaFieldOrder order,
            StaggeredTransferType theTransferType>
  struct StaggeredProlongRestrictArg {
    FieldOrderCB<Float, StaggeredTransferOutSpin<fineSpin, coarseSpin, theTransferType>::outSpin,
                 StaggeredTransferOutColor<fineColor, coarseColor, theTransferType>::outColor, 1, order>
      out;
    const FieldOrderCB<Float, StaggeredTransferInSpin<fineSpin, coarseSpin, theTransferType>::inSpin,
                       StaggeredTransferInColor<fineColor, coarseColor, theTransferType>::inColor, 1, order>
      in;
    ...
    StaggeredProlongRestrictArg(ColorSpinorField &out, const ColorSpinorField &in, const int *geo_map, const int parity) :
      out(out),
      in(in),
      geo_map(geo_map),
      spin_map(),
      ...
      fineX {fineColorSpinorField<transferType>(in, out).X()[0], fineColorSpinorField<transferType>(in, out).X()[1],
             fineColorSpinorField<transferType>(in, out).X()[2], fineColorSpinorField<transferType>(in, out).X()[3]},
    ...
    StaggeredProlongRestrictArg(
      const StaggeredProlongRestrictArg<Float, fineSpin, fineColor, coarseSpin, coarseColor, order, theTransferType> &arg) :
      out(arg.out),
      in(arg.in),
      geo_map(arg.geo_map),
      spin_map(),
      parity(arg.parity),
      nParity(arg.nParity),
      fineX {arg.fineX[0], arg.fineX[1], arg.fineX[2], arg.fineX[3]},
      fineVolumeCB(arg.fineVolumeCB),
      coarseVolumeCB(arg.coarseVolumeCB)
    {
      ;
    }
  };
  ...
  */
  ...
  staggeredProlongRestrict(OutAccessor &out, const InAccessor &in, int parity, int x_cb, int c, const int *geo_map,
                           const S &spin_map, const int fineVolumeCB, const int coarseVolumeCB, const int X[])
  {
    ...
    for (int parity = 0; parity < arg.nParity; parity++) {
      ...
      for (int x_cb = 0; x_cb < arg.fineVolumeCB; x_cb++) {
        for (int c = 0; c < fineColor; c++) {
          staggeredProlongRestrict<Arg::transferType>(arg.out, arg.in, parity, x_cb, c, arg.geo_map, arg.spin_map,
                                                      arg.fineVolumeCB, arg.coarseVolumeCB, arg.fineX);
        }
      }
    }
  }
  ...
  class StaggeredProlongRestrictLaunch : public TunableVectorYZ
  {
    ...
    {
      return fineColorSpinorField<transferType>(in, out).VolumeCB();
    } // fine parity is the block y dimension

  public:
    StaggeredProlongRestrictLaunch(ColorSpinorField &out, const ColorSpinorField &in, const int *fine_to_coarse,
                                   int parity) :
      TunableVectorYZ(fineColorSpinorField<transferType>(in, out).SiteSubset(), fineColor),
      out(out),
      in(in),
      fine_to_coarse(fine_to_coarse),
      parity(parity),
      location(checkLocation(out, in))
    {
    ...
    void apply(const qudaStream_t &stream)
    {
      if (location == QUDA_CPU_FIELD_LOCATION) {
        if (out.FieldOrder() == QUDA_SPACE_SPIN_COLOR_FIELD_ORDER) {
          StaggeredProlongRestrictArg<Float, fineSpin, fineColor, coarseSpin, coarseColor,
                                      QUDA_SPACE_SPIN_COLOR_FIELD_ORDER, transferType>
        ...
        if (out.FieldOrder() == QUDA_FLOAT2_FIELD_ORDER) {
          ...
          StaggeredProlongRestrictArg<Float, fineSpin, fineColor, coarseSpin, coarseColor, QUDA_FLOAT2_FIELD_ORDER, transferType>
            arg(out, in, fine_to_coarse, parity);
          qudaLaunchKernel(
            StaggeredProlongRestrictKernel<Float, fineSpin, fineColor, coarseSpin, coarseColor, decltype(arg)>, tp,
            stream, arg);
        } else {
          errorQuda("Unsupported field order %d", out.FieldOrder());
        }
    ...
    long long bytes() const
    {
      return in.Bytes() + out.Bytes()
        + fineColorSpinorField<transferType>(in, out).SiteSubset()
        * fineColorSpinorField<transferType>(in, out).VolumeCB() * sizeof(int);
    }
  };
  ...
  void StaggeredProlongRestrict(ColorSpinorField &out, const ColorSpinorField &in, const int *fine_to_coarse, int parity)
  {
    ...
    if (precision == QUDA_DOUBLE_PRECISION) {
      ...
      StaggeredProlongRestrict<double, fineSpin, fineColor, coarseSpin, coarseColor, transferType>(
        out, in, fine_to_coarse, parity);
#else
      errorQuda("Double precision multigrid has not been enabled");
#endif
    } else if (precision == QUDA_SINGLE_PRECISION) {
      StaggeredProlongRestrict<float, fineSpin, fineColor, coarseSpin, coarseColor, transferType>(
        out, in, fine_to_coarse, parity);
    } else {
  ...
  template <StaggeredTransferType transferType>
  void StaggeredProlongRestrict(ColorSpinorField &out, const ColorSpinorField &in, const int *fine_to_coarse,
                                const int *const *spin_map, int parity)
  {

    if (out.FieldOrder() != in.FieldOrder())
      errorQuda("Field orders do not match (out=%d, in=%d)", out.FieldOrder(), in.FieldOrder());
                           ...
                           const int *const *spin_map, int parity)
  {
    ...
    StaggeredProlongRestrict<StaggeredTransferType::STAGGERED_TRANSFER_PROLONG>(out, in, fine_to_coarse, spin_map,
                                                                                parity);
                         ...
                         const int *const *spin_map, int parity)
  {
    ...
    StaggeredProlongRestrict<StaggeredTransferType::STAGGERED_TRANSFER_RESTRICT>(out, in, fine_to_coarse, spin_map,
                                                                                 parity);
