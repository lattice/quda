...
namespace quda {
  ...
  void copyGaugeMG(const InOrder &inOrder, GaugeField &out, const GaugeField &in, QudaFieldLocation location,
                   sFloatOut *Out, sFloatOut **outGhost, int type)
  {
                   ...
                   sFloatOut **outGhost, sFloatIn **inGhost, int type)
  {
	...
	typedef typename gauge::FieldOrder<FloatIn,Nc,1,QUDA_FLOAT2_GAUGE_ORDER,false,sFloatIn> G;
        copyGaugeMG<sFloatOut, FloatIn, Nc>(G(const_cast<GaugeField &>(in), (void *)In, (void **)inGhost), out, in,
                                            location, Out, outGhost, type);
      } else {
  ...
  void copyGaugeMG(GaugeField &out, const GaugeField &in, QudaFieldLocation location, FloatOut *Out, FloatIn *In,
