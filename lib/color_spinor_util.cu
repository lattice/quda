  ...
  */
  ...
  void genericCudaPrintVector(const cudaColorSpinorField &field, unsigned int i)
  {
    ...
    if (isFixed<StoreType>::value) {
      cudaMemcpy(&scale, &norm_ptr[i], sizeof(float), qudaMemcpyDeviceToHost);
      scale *= fixedInvMaxValue<StoreType>::value;
    }

    for (int s = 0; s < Ns; s++) {
      for (int c = 0; c < Nc; c++) {
        cudaMemcpy(indiv_num, &field_ptr[A.index(i % 2, i / 2, s, c, 0)], 2 * sizeof(StoreType), qudaMemcpyDeviceToHost);
