...
namespace quda
{
  ...
  public:
	...
	computeEvecProject<Float><<<tp.grid, tp.block, tp.shared_bytes>>>(arg);
#endif
      } else {
  ...
  void evecProject(const ColorSpinorField &x, const ColorSpinorField &y, complex<Float> *result)
  {
  ...
  void evecProjectQuda(const ColorSpinorField &x, const ColorSpinorField &y, void *result)
  {
    ...
    if (x.Precision() == QUDA_SINGLE_PRECISION) {
      evecProject<float, 3>(x, y, (complex<float> *)result);
    } else if (x.Precision() == QUDA_DOUBLE_PRECISION) {
      evecProject<double, 3>(x, y, (complex<double> *)result);
    } else {
