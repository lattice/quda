...
namespace quda
{
  ...
  template <typename Float, int nColor, QudaReconstructType recon> class GaugeWFlowStep : TunableVectorYZ
  {
  ...
  public:
    GaugeWFlowStep(GaugeField &out, GaugeField &temp, const GaugeField &in, const double epsilon,
      ...
      TunableVectorYZ(2, wflow_dim), arg(out, temp, in, epsilon, wflow_type, step_type), meta(in)
    {
    ...
    void apply(const qudaStream_t &stream)
    {
      ...
      jitify_error = program->kernel("quda::computeWFlowStep")
                       .instantiate(arg.wflow_type, arg.step_type, Type<decltype(arg)>())
      ...
      case QUDA_WFLOW_TYPE_WILSON:
        ...
        case WFLOW_STEP_W1:
          qudaLaunchKernel(computeWFlowStep<QUDA_WFLOW_TYPE_WILSON, WFLOW_STEP_W1, decltype(arg)>, tp, stream, arg);
          break;
        case WFLOW_STEP_W2:
          qudaLaunchKernel(computeWFlowStep<QUDA_WFLOW_TYPE_WILSON, WFLOW_STEP_W2, decltype(arg)>, tp, stream, arg);
          break;
        case WFLOW_STEP_VT:
          qudaLaunchKernel(computeWFlowStep<QUDA_WFLOW_TYPE_WILSON, WFLOW_STEP_VT, decltype(arg)>, tp, stream, arg);
          break;
        }
        break;
      case QUDA_WFLOW_TYPE_SYMANZIK:
        switch (arg.step_type) {
        case WFLOW_STEP_W1:
          qudaLaunchKernel(computeWFlowStep<QUDA_WFLOW_TYPE_SYMANZIK, WFLOW_STEP_W1, decltype(arg)>, tp, stream, arg);
          break;
        case WFLOW_STEP_W2:
          qudaLaunchKernel(computeWFlowStep<QUDA_WFLOW_TYPE_SYMANZIK, WFLOW_STEP_W2, decltype(arg)>, tp, stream, arg);
          break;
        case WFLOW_STEP_VT:
          qudaLaunchKernel(computeWFlowStep<QUDA_WFLOW_TYPE_SYMANZIK, WFLOW_STEP_VT, decltype(arg)>, tp, stream, arg);
          break;
        }
        break;
    ...
    long long bytes() const
    {
      ...
      return ((1 + (wflow_dim - 1) * links) * arg.in.Bytes() + arg.out.Bytes() + temp_io * arg.temp.Bytes()) * 2ll
        * arg.threads * wflow_dim;
    }
  }; // GaugeWFlowStep
