...
namespace quda {
  ...
  template <typename store_t> class CloverInvert : TunableLocalParityReduction
  {
    CloverInvertArg<store_t> arg;
    const CloverField &meta; // used for meta data only

  public:
    CloverInvert(CloverField &clover, bool compute_tr_log) : arg(clover, compute_tr_log), meta(clover)
    {
