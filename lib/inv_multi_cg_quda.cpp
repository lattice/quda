...
#define EXITEARLY
    ...
    const double prec_tol = param.precision_sloppy == param.precision ? pow(10.,(-2*(int)b.Precision()+1)) : sloppy_tol;
 ...
 void MultiShiftCG::solve(std::vector<ColorSpinorField*>x, ColorSpinorField &b, std::vector<ColorSpinorField*> &p, double* r2_old_array )
  {
      if (checkLocation(*(x[0]), b) != QUDA_CUDA_FIELD_LOCATION)
      errorQuda("Not supported");
...
#define EXITEARLY
...
#ifdef EXITEARLYHIGH
    const double prec_tol = param.precision_sloppy == param.precision ? pow(10.,(-2*(int)b.Precision()+1)) : sloppy_tol*0.1;
    bool exitearly = false;
#else
    bool exitearly = false;
    const double prec_tol = pow(10.,(-2*(int)b.Precision()+1));
  ...
  printfQuda("Reliable update triggered by shift %i\n",reliable_shift);
      }
