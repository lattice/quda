...
//       errorQuda("\nMethod is not implemented for %d inverter type.\n", param.eig_global.invert_param->inv_type);
  ...
  void Deflation::increment(ColorSpinorField &Vm, int nev) {
    ...
    if(rv.CompositeDim() < (first_idx+nev) || param.tot_dim < (first_idx+nev)) {
	...
	vi_.push_back(accum);
        ...
        blas::axpy(alpha.get(), vj_, vi_); //i-<j,i>j
      ...
      if(alpha[0] > 1e-16) blas::ax(1.0 /sqrt(alpha[0]), *accum);
      else                 errorQuda("\nCannot orthogonalize %dth vector\n", i);

      rv[i] = *accum;

      param.matDeflation(*Av_sloppy, rv[i]);//precision must match!
      //load diagonal:
      *Av = *Av_sloppy;
      //param.matProj[i*param.ld+i] = cDotProduct(*accum, *Av);
      param.matProj[i*param.ld+i] = reDotProduct(*accum, *Av);

      if (i>0) {
        std::vector<ColorSpinorField*> vj_(rv(0,i));
        std::vector<ColorSpinorField*> av_;
	      av_.push_back(Av_sloppy);

	      //blas::cDotProduct(alpha.get(), vj_, av_);
        for(int j = 0; j < i; j++) alpha[j] = blas::reDotProduct(rv[j], *Av_sloppy);
  ...
  void Deflation::reduce(double tol, int max_nev) {
     ...
     memcpy(projm.get(), param.matProj, param.ld*param.cur_dim*sizeof(double));

     Map<MatrixXd, Unaligned, DynamicStride> projm_(projm.get(), param.cur_dim, param.cur_dim, DynamicStride(param.ld, 1));
     Map<VectorXd, Unaligned> evals_(evals.get(), param.cur_dim);

     SelfAdjointEigenSolver<MatrixXd> es(projm_);
     ...
     while ((relerr < tol) && (idx < max_nev))
     {
       ...
       for(int j = 0; j < param.cur_dim; j++) blas::axpy(projm.get()[j+idx*param.ld], rv[j], *r);
