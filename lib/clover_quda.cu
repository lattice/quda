      ...
      if(clover.Order() == QUDA_FLOAT2_CLOVER_ORDER){
        if(gauge.Order() == QUDA_FLOAT2_GAUGE_ORDER){
          ...
          if(gauge.Reconstruct() == QUDA_RECONSTRUCT_12){
            computeClover(clover::FloatNOrder<Float,72,2>(clover,0), gauge::FloatNOrder<Float,18,4,12>(gauge),  *Fmunu, cloverCoeff, location);
          }else{
            errorQuda("Reconstruction type %d not supported",gauge.Reconstruct());
          }
        }
        ...
        if(gauge.Order() == QUDA_FLOAT2_GAUGE_ORDER){
          if(gauge.Reconstruct() == QUDA_RECONSTRUCT_NO){
            computeClover(clover::FloatNOrder<Float,72,4>(clover,0), gauge::FloatNOrder<Float,18,2,18>(gauge),  *Fmunu, cloverCoeff, location);
          }else if(gauge.Reconstruct() == QUDA_RECONSTRUCT_12){
            computeClover(clover::FloatNOrder<Float,72,4>(clover,0), gauge::FloatNOrder<Float,18,2,12>(gauge),  *Fmunu, cloverCoeff, location);
          }else{
            errorQuda("Reconstruction type %d not supported",gauge.Reconstruct());
          }
          ...
          if(gauge.Reconstruct() == QUDA_RECONSTRUCT_12){
            computeClover(clover::FloatNOrder<Float,72,4>(clover,0), gauge::FloatNOrder<Float,18,4,12>(gauge), *Fmunu, cloverCoeff, location);
          }else{
            errorQuda("Reconstruction type %d not supported",gauge.Reconstruct());
          } // gauge order
        }
