...
namespace quda {
  ...
  */
  ...
  struct ProlongateArg {
    ...
    ProlongateArg(ColorSpinorField &out, const ColorSpinorField &in, const ColorSpinorField &V, const int *geo_map,
                  const int parity) :
    ...
    ProlongateArg(const ProlongateArg<Float, vFloat, fineSpin, fineColor, coarseSpin, coarseColor, order> &arg) :
      out(arg.out), in(arg.in), V(arg.V), geo_map(arg.geo_map), spin_map(), parity(arg.parity), nParity(arg.nParity)
    {
    }
  };
  ...
  */
  ...
  __device__ __host__ inline void prolongate(complex<Float> out[fineSpin * coarseColor], const Coarse &in, int parity,
                                             int x_cb, const int *geo_map, const S &spin_map, int fineVolumeCB)
  {
...
#pragma unroll
  ...
  */
  template <typename Float, int fineSpin, int fineColor, int coarseColor, int fine_colors_per_thread, class FineColor,
            class Rotator>
  __device__ __host__ inline void rotateFineColor(FineColor &out, const complex<Float> in[fineSpin * coarseColor],
                                                  const Rotator &V, int parity, int nParity, int x_cb,
...
#pragma unroll
  ...
  void Prolongate(Arg &arg) {
    for (int parity=0; parity<arg.nParity; parity++) {
      ...
      for (int x_cb=0; x_cb<arg.out.VolumeCB(); x_cb++) {
        ...
        prolongate<Float, fineSpin, coarseColor>(tmp, arg.in, parity, x_cb, arg.geo_map, arg.spin_map,
                                                 arg.out.VolumeCB());
  ...
  public:
    ProlongateLaunch(ColorSpinorField &out, const ColorSpinorField &in, const ColorSpinorField &V,
      ...
      TunableVectorYZ(out.SiteSubset(), fineColor / fine_colors_per_thread),
      out(out),
      in(in),
      V(V),
    ...
    void apply(const qudaStream_t &stream)
    {
      if (location == QUDA_CPU_FIELD_LOCATION) {
        if (out.FieldOrder() == QUDA_SPACE_SPIN_COLOR_FIELD_ORDER) {
          ProlongateArg<Float, vFloat, fineSpin, fineColor, coarseSpin, coarseColor, QUDA_SPACE_SPIN_COLOR_FIELD_ORDER> arg(
        ...
        if (out.FieldOrder() == QUDA_FLOAT2_FIELD_ORDER) {
          ...
          ProlongateArg<Float, vFloat, fineSpin, fineColor, coarseSpin, coarseColor, QUDA_FLOAT2_FIELD_ORDER> arg(
            out, in, V, fine_to_coarse, parity);
          qudaLaunchKernel(
            ProlongateKernel<Float, fineSpin, fineColor, coarseSpin, coarseColor, fine_colors_per_thread, decltype(arg)>,
            tp, stream, arg);
        } else {
          errorQuda("Unsupported field order %d", out.FieldOrder());
        }
                  ...
                  const int *fine_to_coarse, int parity)
  {
    ...
    if (v.Precision() == QUDA_HALF_PRECISION) {
#if QUDA_PRECISION & 2
      ProlongateLaunch<Float, short, fineSpin, fineColor, coarseSpin, coarseColor, fine_colors_per_thread> prolongator(
        out, in, v, fine_to_coarse, parity);
      prolongator.apply(0);
#else
      ...
      ProlongateLaunch<Float, Float, fineSpin, fineColor, coarseSpin, coarseColor, fine_colors_per_thread> prolongator(
        out, in, v, fine_to_coarse, parity);
      prolongator.apply(0);
    } else {
  ...
  void Prolongate(ColorSpinorField &out, const ColorSpinorField &in, const ColorSpinorField &v, int nVec,
                  ...
                  const int *fine_to_coarse, const int *const *spin_map, int parity)
  {
    ...
    if (out.FieldOrder() != in.FieldOrder() || out.FieldOrder() != v.FieldOrder())
      errorQuda("Field orders do not match (out=%d, in=%d, v=%d)", out.FieldOrder(), in.FieldOrder(), v.FieldOrder());
