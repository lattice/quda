...
namespace quda {
                        ...
                        bool need_bidirectional, bool use_mma)
  {
    ...
    if (fineColor == 2*N_COLORS) { // free field Wilson
      if (coarseColor == 2*N_COLORS) {
        calculateYcoarse<Float, vFloat, 2*N_COLORS, fineSpin, 2*N_COLORS, coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv,
  ...
  void calculateYcoarse(GaugeField &, GaugeField &, GaugeField &, GaugeField &, ColorSpinorField &,
                        ...
                        double, double, double, double, QudaDiracType, QudaMatPCType, bool, bool)
  {
