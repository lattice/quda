...
namespace quda {
                        ...
                        bool need_bidirectional, bool use_mma)
  {
	...
	(yAccessor, xAccessor, yAccessorAtomic, xAccessorAtomic,
	 ...
	 T.fineToCoarse(Y.Location()), T.coarseToFine(Y.Location()), use_mma);
    } else {
  ...
  void calculateYcoarse(GaugeField &Y, GaugeField &X, GaugeField &Yatomic, GaugeField &Xatomic,
			ColorSpinorField &uv, const Transfer &T, const GaugeField &g, const GaugeField &clover,
			const GaugeField &cloverInv, double kappa, double mu, double mu_factor, QudaDiracType dirac, QudaMatPCType matpc, bool need_bidirectional, bool use_mma)
  {
    ...
    if (fineColor == 6) { // free field Wilson
      if (coarseColor == 6) {
        // calculateYcoarse<Float,vFloat,6,fineSpin,6,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else {
    ...
    if (fineColor == 24) { // coarsened Wilson or free field staggered
      if (coarseColor == 24) { 
        calculateYcoarse<Float,vFloat,24,fineSpin,24,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else
#ifdef NSPIN4
      if (coarseColor == 32) {
        calculateYcoarse<Float,vFloat,24,fineSpin,32,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else
#endif // NSPIN4
#ifdef NSPIN1
      if (coarseColor == 64) {
        calculateYcoarse<Float,vFloat,24,fineSpin,64,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else if (coarseColor == 96) {
        calculateYcoarse<Float,vFloat,24,fineSpin,96,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else
#endif
      {
        errorQuda("Unsupported fineColor = %d coarseColor = %d\n", fineColor, coarseColor);
      ...
      if (coarseColor == 32) {
        calculateYcoarse<Float,vFloat,32,fineSpin,32,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else {
        errorQuda("Unsupported fineColor = %d coarseColor = %d\n", fineColor, coarseColor);
      }
#endif // NSPIN4
      ...
      if (coarseColor == 64) {
        calculateYcoarse<Float,vFloat,64,fineSpin,64,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else if (coarseColor == 96) {
        calculateYcoarse<Float,vFloat,64,fineSpin,96,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else {
        errorQuda("Unsupported fineColor = %d coarseColor = %d\n", fineColor, coarseColor);
      }
    } else if (fineColor == 96) {
      if (coarseColor == 96) {
        calculateYcoarse<Float,vFloat,96,fineSpin,96,coarseSpin>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else {
        errorQuda("Unsupported fineColor = %d coarseColor = %d\n", fineColor, coarseColor);
      }
#endif // NSPIN1
			...
			const GaugeField &cloverInv, double kappa, double mu, double mu_factor, QudaDiracType dirac, QudaMatPCType matpc, bool need_bidirectional, bool use_mma)
  {
    if (T.Vectors().Nspin() == 2) {
      calculateYcoarse<Float,vFloat,2>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
    } else {
  ...
  void calculateYcoarse(GaugeField &Y, GaugeField &X, GaugeField &Yatomic, GaugeField &Xatomic, ColorSpinorField &uv,
			...
			double kappa, double mu, double mu_factor, QudaDiracType dirac, QudaMatPCType matpc, bool need_bidirectional, bool use_mma)
  {
#ifdef GPU_MULTIGRID
	...
	calculateYcoarse<double,double>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else {
	...
	calculateYcoarse<float,float>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else {
	errorQuda("Unsupported precision %d\n", T.Vectors(X.Location()).Precision());
      }
    } else if (Y.Precision() == QUDA_HALF_PRECISION) {
      if (T.Vectors(X.Location()).Precision() == QUDA_HALF_PRECISION) {
	calculateYcoarse<float,short>(Y, X, Yatomic, Xatomic, uv, T, g, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
      } else {
	errorQuda("Unsupported precision %d\n", T.Vectors(X.Location()).Precision());
      }
    } else {
                      ...
                      bool need_bidirectional, bool use_mma)
  {
    ...
    calculateYcoarse(Y, X, *Yatomic, *Xatomic, *uv, T, gauge, clover, cloverInv, kappa, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma);
