...
namespace quda {
  ...
  public:
    GhostPack(void **ghost, const ColorSpinorField &a, QudaParity parity,
              int nFace, int dagger, MemoryLocation *destination, int shmem_) :
    ...
    {
      if (a.Ndim() == 5) launch<GhostPacker, true>(tp, stream, Arg<5>(a, work_items, ghost, parity, nFace, dagger, shmem));
      else               launch<GhostPacker, true>(tp, stream, Arg<4>(a, work_items, ghost, parity, nFace, dagger, shmem));
    }
  ...
  void genericPackGhost(void **ghost, const ColorSpinorField &a, QudaParity parity,
                        int nFace, int dagger, MemoryLocation *destination, int shmem)
  {
#ifndef NSPIN1
    ...
    if (a.Ncolor() == 3) {
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,3>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#ifdef GPU_MULTIGRID
    } else if (a.Ncolor() == 6) {
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,6>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 18) { // Needed for two level free field Wilson
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,18>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 24) { // Needed for K-D staggered Wilson
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,24>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#ifdef NSPIN4
    } else if (a.Ncolor() == 32) { // Needed for Wilson
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,32>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 36) { // Needed for three level free field Wilson
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,36>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#endif // NSPIN4
#ifdef NSPIN1
    } else if (a.Ncolor() == 64) { // Needed for staggered Nc = 64
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,64>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#endif // NSPIN1
    } else if (a.Ncolor() == 72) { // wilson 3 -> 24 nvec, or staggered 3 -> 24 nvec, which could end up getting used for Laplace...
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,72>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 96) { // wilson 3 -> 32 nvec, or staggered Nc = 96
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,96>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#ifdef NSPIN1
    } else if (a.Ncolor() == 192) { // staggered 3 -> 64 Nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,192>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 288) { // staggered 3 -> 96 Nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,288>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#endif // NSPIN1
    } else if (a.Ncolor() == 576) { // staggered KD free-field or wilson 24 -> 24 nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,576>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#ifdef NSPIN4
    } else if (a.Ncolor() == 768) { // wilson 24 -> 32 nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,768>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 1024) { // wilson 32 -> 32 nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,1024>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#endif // NSPIN4
#ifdef NSPIN1
    } else if (a.Ncolor() == 1536) { // staggered KD 24 -> 64 nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,1536>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 2304) { // staggered KD 24 -> 96 nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,2304>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 4096) { // staggered 64 -> 64
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,4096>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 6144) { // staggered 64 -> 96 nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,6144>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
    } else if (a.Ncolor() == 9216) { // staggered 96 -> 96 nvec
      GhostPack<Float,ghostFloat,order,Ns,precision_spin_color_mapper<Float,ghostFloat,Ns,9216>::nColor>(ghost, a, parity, nFace, dagger, destination, shmem);
#endif // NSPIN1
#endif // GPU_MULTIGRID
    } else {
...
#endif
  {
    if (a.Nspin() == 4) {
      ...
      genericPackGhost<Float,ghostFloat,spin_order_mapper<2,order>::order,2>(ghost, a, parity, nFace, dagger, destination, shmem);
#else
...
#ifdef NSPIN1
      if (order == QUDA_FLOAT4_FIELD_ORDER) errorQuda("Field order %d with nSpin = %d not supported", order, a.Nspin());
      genericPackGhost<Float,ghostFloat,spin_order_mapper<1,order>::order,1>(ghost, a, parity, nFace, dagger, destination, shmem);
#else
  ...
  inline void genericPackGhost(void **ghost, const ColorSpinorField &a, QudaParity parity,
    ...
    if (a.FieldOrder() == QUDA_FLOAT2_FIELD_ORDER) {
      ...
      genericPackGhost<typename non_native_precision_mapper<Float>::type,
                       ...
                       QUDA_SPACE_SPIN_COLOR_FIELD_ORDER>(ghost, a, parity, nFace, dagger, destination, shmem);
#else
  ...
  void genericPackGhost(void **ghost, const ColorSpinorField &a, QudaParity parity,
			int nFace, int dagger, MemoryLocation *destination_, int shmem) {
