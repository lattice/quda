...
namespace quda
{
  ...
  class CalculateStaggeredKDBlock : public TunableVectorYZ
  {
    ...
    long long bytes() const
    {
      ...
      return meta.Bytes() / 2 + (meta.Bytes() * X.Precision()) / meta.Precision()
        + 2 * coarseSpin * coarseColor * arg.coarseVolumeCB * X.Precision();
    }
  ...
  public:
    CalculateStaggeredKDBlock(Arg &arg, const GaugeField &meta, GaugeField &X) :
      TunableVectorYZ(fineColor * fineColor, 2), arg(arg), meta(meta), X(X)
    {
#ifdef JITIFY
      ...
      strcat(aux,
             (meta.Location() == QUDA_CUDA_FIELD_LOCATION && X.MemType() == QUDA_MEMORY_MAPPED) ? ",GPU-mapped," :
    ...
    bool advanceTuneParam(TuneParam &param) const
    {
      ...
      if (X.MemType() == QUDA_MEMORY_DEVICE)
  ...
  void calculateStaggeredKDBlock(xGauge &X, fineGauge &G, GaugeField &X_, const GaugeField &G_, double mass)
  {
    ...
    if (getVerbosity() >= QUDA_VERBOSE)
      printfQuda("Calculating the KD block on the %s\n", location == QUDA_CUDA_FIELD_LOCATION ? "GPU" : "CPU");
  ...
  void BuildStaggeredKahlerDiracInverse(GaugeField &Xinv, const cudaGaugeField &gauge, const double mass)
  {
    ...
    // on the CPU or GPU
    if (Xinv.FieldOrder() != QUDA_MILC_GAUGE_ORDER) errorQuda("Unsupported field order %d", Xinv.FieldOrder());
    ...
    if (location == QUDA_CUDA_FIELD_LOCATION) {
      ...
      cudaGaugeField *Xinv_ = (Xinv.Precision() == QUDA_SINGLE_PRECISION) ? static_cast<cudaGaugeField *>(&Xinv) :
                                                                            new cudaGaugeField(param);

      X_.copy(*X);

      blas::flops
        += invert((void *)Xinv_->Gauge_p(), (void *)X_.Gauge_p(), n, X_.Volume(), X_.Precision(), X->Location());
      ...
      if (Xinv.Precision() == QUDA_DOUBLE_PRECISION) errorQuda("Unsupported precision %d", Xinv.Precision());
