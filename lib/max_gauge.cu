...
namespace quda {
  ...
  template <typename reg_type, typename real, int Nc> double norm(const GaugeField &u, int d, norm_type_ type)
  {
    double norm_ = 0.0;
    switch (u.FieldOrder()) {
  ...
  template <typename T, bool fixed> double norm(const GaugeField &u, int d, norm_type_ type)
  {
  ...
  template <typename T> struct Norm {
    Norm(const GaugeField &u, double &nrm, int d, bool fixed, norm_type_ type)
    {
      ...
      if (fixed)
        nrm = norm<T, true>(u, d, type);
      else
        nrm = norm<T, false>(u, d, type);
    }
  };
