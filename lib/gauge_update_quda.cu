...
namespace quda {
  ...
  template <bool conj_mom, bool exact, typename Arg> __device__ __host__ void compute(Arg &arg, int x, int parity)
  {
	...
	if (!conj_mom) {
          for (int r = Arg::N; r > 0; r--) result = (arg.dt / r) * mom * result + link;
        } else {
          for (int r = Arg::N; r > 0; r--) result = (arg.dt / r) * conj(mom) * result + link;
        }
      } else {
	mom = arg.dt * mom;
        expsu3<Float>(mom);
  ...
  template <bool conj_mom, bool exact, typename Arg> __global__ void updateGaugeFieldKernel(Arg arg)
  {
  ...
  template <typename Arg, bool conj_mom, bool exact> class UpdateGaugeField : public TunableVectorY
  {
    Arg &arg;
  ...
  public:
    UpdateGaugeField(Arg &arg, const GaugeField &meta) : TunableVectorY(2), arg(arg), meta(meta) { }
    ...
    long long flops() const
    {
      ...
      return arg.nDim * 2 * arg.in.volumeCB * Arg::N
        * (Nc * Nc * 2 +                      // scalar-matrix multiply
    ...
    {
      return arg.nDim * 2 * arg.in.volumeCB * (arg.in.Bytes() + arg.out.Bytes() + arg.mom.Bytes());
    }
