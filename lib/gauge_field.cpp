...
namespace quda {
  ...
  void GaugeField::createGhostZone(const int *R, bool no_comms_fill, bool bidir) const
  {
    ...
    for (int i=0; i<nDim; i++) {
      ...
      ghostOffset[i][0] = (i == 0) ? 0 : ghostOffset[i-1][1] + ghostFace[i-1] * geometry_comms * nInternal * ghost_precision;
      ghostOffset[i][1] = (bidir ? ghostOffset[i][0] + ghostFace[i] * geometry_comms * nInternal * ghost_precision : ghostOffset[i][0]);
