...
namespace quda {
                           ...
                           IntList<coarseColor, N...>)
  {
    if (B.size() == coarseColor) {
      if constexpr (coarseColor >= fineColor) {
        BlockOrthogonalize<fineColor, coarseColor>(V, B, fine_to_coarse, coarse_to_fine, geo_bs, spin_bs, n_block_ortho, two_pass);
      } else {
        errorQuda("Invalid coarseColor = %d, cannot be less than fineColor = %d", coarseColor, fineColor);
      }
    } else {
      if constexpr (sizeof...(N) > 0) {
        BlockOrthogonalize2<fineColor>(V, B, fine_to_coarse, coarse_to_fine, geo_bs, spin_bs, n_block_ortho, two_pass, IntList<N...>());
      } else {
  ...
  void BlockOrthogonalize(ColorSpinorField &V, const std::vector<ColorSpinorField *> &B, const int *fine_to_coarse,
                          const int *coarse_to_fine, const int *geo_bs, int spin_bs, int n_block_ortho, bool two_pass, IntList<fineColor, N...>)
  {
    if (V.Ncolor() / B.size() == fineColor) {
      IntList<@QUDA_MULTIGRID_NVEC_LIST@> coarseColors;
      BlockOrthogonalize2<fineColor>(V, B, fine_to_coarse, coarse_to_fine, geo_bs, spin_bs, n_block_ortho, two_pass, coarseColors);
    } else {
      if constexpr (sizeof...(N) > 0) {
        BlockOrthogonalize(V, B, fine_to_coarse, coarse_to_fine, geo_bs, spin_bs, n_block_ortho, two_pass, IntList<N...>());
      } else {
