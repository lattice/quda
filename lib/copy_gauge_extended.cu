...
namespace quda {
   ...
   */
  template <typename OutOrder, typename InOrder> struct CopyGaugeExArg {
    OutOrder out;
    const InOrder in;
    ...
    CopyGaugeExArg(const OutOrder &out, const InOrder &in, const int *Xout, const int *Xin, const int *faceVolumeCB,
                   int nDim, int geometry) :
      out(out),
      in(in),
      nDim(nDim),
      geometry(geometry)
    ...
    void apply(const cudaStream_t &stream)
    {
      ...
      if (location == QUDA_CPU_FIELD_LOCATION) {
        if (arg.regularToextended)
          ...
          copyGaugeEx<FloatOut, FloatIn, length, OutOrder, InOrder, false>(arg);
      } else if (location == QUDA_CUDA_FIELD_LOCATION) {
        if (arg.regularToextended)
          copyGaugeExKernel<FloatOut, FloatIn, length, OutOrder, InOrder, true>
            <<<tp.grid, tp.block, tp.shared_bytes, stream>>>(arg);
        else
          copyGaugeExKernel<FloatOut, FloatIn, length, OutOrder, InOrder, false>
            <<<tp.grid, tp.block, tp.shared_bytes, stream>>>(arg);
      }
      }
      ...
      long long bytes() const
      {
        ...
        return 2 * sites
          * (arg.in.Bytes() + arg.in.hasPhase * sizeof(FloatIn) + arg.out.Bytes() + arg.out.hasPhase * sizeof(FloatOut));
      }
  };

  template <typename FloatOut, typename FloatIn, int length, typename OutOrder, typename InOrder>
  void copyGaugeEx(OutOrder outOrder, const InOrder inOrder, const int *E, const int *X, const int *faceVolumeCB,
                   const GaugeField &meta, QudaFieldLocation location)
  {

    CopyGaugeExArg<OutOrder, InOrder> arg(outOrder, inOrder, E, X, faceVolumeCB, meta.Ndim(), meta.Geometry());
  ...
  void copyGaugeEx(const InOrder &inOrder, const int *X, GaugeField &out, QudaFieldLocation location, FloatOut *Out)
  {
    ...
    if (out.isNative()) {
      ...
      copyGaugeEx<FloatOut, FloatIn, length>(QDPOrder<FloatOut, length>(out, Out), inOrder, out.X(), X, faceVolumeCB,
                                             out, location);
#else
      ...
      copyGaugeEx<FloatOut, FloatIn, length>(MILCOrder<FloatOut, length>(out, Out), inOrder, out.X(), X, faceVolumeCB,
                                             out, location);
#else
      ...
      copyGaugeEx<FloatOut, FloatIn, length>(TIFROrder<FloatOut, length>(out, Out), inOrder, out.X(), X, faceVolumeCB,
                                             out, location);
#else
...
#ifdef BUILD_QDP_INTERFACE
      copyGaugeEx<FloatOut, FloatIn, length>(QDPOrder<FloatIn, length>(in, In), in.X(), out, location, Out);
#else
      errorQuda("QDP interface has not been built\n");
#endif
...
#ifdef BUILD_MILC_INTERFACE
      copyGaugeEx<FloatOut, FloatIn, length>(MILCOrder<FloatIn, length>(in, In), in.X(), out, location, Out);
#else
      errorQuda("MILC interface has not been built\n");
#endif
...
#ifdef BUILD_TIFR_INTERFACE
      copyGaugeEx<FloatOut, FloatIn, length>(TIFROrder<FloatIn, length>(in, In), in.X(), out, location, Out);
#else
      errorQuda("TIFR interface has not been built\n");
#endif
  ...
  void copyGaugeEx(GaugeField &out, const GaugeField &in, QudaFieldLocation location, FloatOut *Out, FloatIn *In)
  {
    ...
    void copyExtendedGauge(GaugeField &out, const GaugeField &in, QudaFieldLocation location, void *Out, void *In)
    {
