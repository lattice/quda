...
namespace quda {
  ...
  void computeQCharge(const Gauge data, const GaugeField& Fmunu, Float &qChg){
  ...
  Float computeQCharge(const GaugeField &Fmunu){
  ...
  double computeQCharge(const GaugeField& Fmunu){
    ...
    if (!Fmunu.isNative())
      errorQuda("Order %d with %d reconstruct not supported", Fmunu.Order(), Fmunu.Reconstruct());
