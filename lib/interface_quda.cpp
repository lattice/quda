...
inline cudaError_t checkCuda(cudaError_t result)
{
  if (result != cudaSuccess) {
    printfQuda("CUDA Runtime Error: %s\n", cudaGetErrorString(result));
    //std::assert(result == cudaSuccess);
  }
  return result;
}
		     ...
		     QudaInvertParam inv_param, unsigned int nEv, const int X[4])
{
 ...
 void laphBaryonKernel(int n1, int n2, int n3, int nMom,
  ...
  profileCuBLAS.TPSTART(QUDA_PROFILE_COMPUTE);
  blas_lapack::native::BatchGEMM(d_coeffs2, d_evec, d_q2, cublas_param_init, QUDA_CUDA_FIELD_LOCATION);
  cublas_param_init.m = n3;
  blas_lapack::native::BatchGEMM(d_coeffs3, d_evec, d_q3, cublas_param_init, QUDA_CUDA_FIELD_LOCATION);
  profileCuBLAS.TPSTOP(QUDA_PROFILE_COMPUTE);
  ...
  for (int dil1=0; dil1<n1; dil1++) {
	...
	colorContractQuda(*quda_diq[0], *quda_q3[dil3], 
			  (std::complex<double>*)d_tmp + nSites*nInBlock, 0);
	...
	if (nInBlock == blockSizeMomProj || ((dil1+1 == n1) && (dil2+1 == n2) && (dil3+1 == n3))) {
	  ...
	  profileCuBLAS.TPSTART(QUDA_PROFILE_COMPUTE);	  
	  blas_lapack::native::BatchGEMM(d_mom, d_tmp, d_ret, cublas_param_mom_sum, QUDA_CUDA_FIELD_LOCATION);
	  profileCuBLAS.TPSTOP(QUDA_PROFILE_COMPUTE);	  
	  nInBlock = 0;
	}
      }
 ...
 void laphBaryonKernelComputeModeTripletMid(int nMom, int nEv, 
					    ...
					    const int X[4]) { 
   profileBaryonKernelModeTripletsMib.TPSTART(QUDA_PROFILE_TOTAL);
