...
// JF
    ...
    void copyCuda(cudaColorSpinorField &dst, const cudaColorSpinorField &src) {
      ...
      if (dst.Precision() == src.Precision()) {
        int copy_bytes = src.Length()*src.Precision();
        cudaMemcpy(dst.V(), src.V(), copy_bytes, cudaMemcpyDeviceToDevice);
        if (dst.Precision() == QUDA_HALF_PRECISION) {
          cudaMemcpy(dst.Norm(), src.Norm(), src.NormBytes(), cudaMemcpyDeviceToDevice);
          blas_bytes += 2*(unsigned long long)src.RealLength()*sizeof(float);
        }
