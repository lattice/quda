...
namespace quda
{
  ...
  void computeCloverForce(GaugeField &mom, const GaugeField &gaugeEx, const GaugeField &gauge,
                          ...
                          double sigma_coeff, bool detratio, bool trlog, bool rhmc, QudaInvertParam &inv_param)
  {
...
// error fuer trlog + detratio und rhmc vs detratio
    ...
    if (detratio) blas::xpy(x0(parity), p(parity));
    if (not_dagger || rhmc) dirac->Dagger(QUDA_DAG_YES);
    if (rhmc) gamma5(p(parity), p(parity));

    dirac->Dslash(p(other_parity), p(parity), other_parity); // and now the even part of Y
    if (not_dagger || rhmc) dirac->Dagger(QUDA_DAG_NO);
    ...
    if (trlog) computeCloverSigmaTrace(oprod, clover, sigma_coeff, other_parity);
