...
namespace quda {
  ...
  template <typename real, int Nc> double norm(const CloverField &u, bool inverse, norm_type_ type)
  {
    double norm_ = 0.0;
    switch (u.Order()) {
  ...
  template <typename real> double _norm(const CloverField &u, bool inverse, norm_type_ type)
  {
    double norm_ = 0.0;
    switch(u.Ncolor()) {
  ...
  double CloverField::norm1(bool inverse) const { return _norm(*this, inverse, NORM1); }

  double CloverField::norm2(bool inverse) const { return _norm(*this, inverse, NORM2); }

  double CloverField::abs_max(bool inverse) const { return _norm(*this, inverse, ABS_MAX); }

  double CloverField::abs_min(bool inverse) const { return _norm(*this, inverse, ABS_MIN); }
