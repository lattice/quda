...
namespace quda {
  ...
  void CoarseCoarseOp2(GaugeField &Y, GaugeField &X, const Transfer &T, const GaugeField &gauge,
                       ...
                       QudaDiracType dirac, QudaMatPCType matpc, bool need_bidirectional, bool use_mma, IntList<coarseColor, N...>)
  {
    if (Y.Ncolor() / 2 == coarseColor) {
      if constexpr (coarseColor >= fineColor) {
        if (use_mma) {
          CoarseCoarseOp<fineColor, coarseColor, true>(Y, X, T, gauge, clover, cloverInv, kappa, mass, mu, mu_factor, dirac, matpc, need_bidirectional);
        } else {
          CoarseCoarseOp<fineColor, coarseColor, false>(Y, X, T, gauge, clover, cloverInv, kappa, mass, mu, mu_factor, dirac, matpc, need_bidirectional);
        }
      } else {
      ...
      if constexpr (sizeof...(N) > 0) {
        CoarseCoarseOp2<fineColor>(Y, X, T, gauge, clover, cloverInv, kappa, mass, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma, IntList<N...>());
      } else {
  ...
  void CoarseCoarseOp(GaugeField &Y, GaugeField &X, const Transfer &T, const GaugeField &gauge,
                      const GaugeField &clover, const GaugeField &cloverInv, double kappa, double mass, double mu, double mu_factor,
                      QudaDiracType dirac, QudaMatPCType matpc, bool need_bidirectional, bool use_mma, IntList<fineColor, N...>)
  {
    if (gauge.Ncolor() / T.Vectors().Nspin() == fineColor) {
      IntList<@QUDA_MULTIGRID_NVEC_LIST@> coarseColors;
      CoarseCoarseOp2<fineColor>(Y, X, T, gauge, clover, cloverInv, kappa, mass, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma, coarseColors);
    } else {
      if constexpr (sizeof...(N) > 0) {
        CoarseCoarseOp(Y, X, T, gauge, clover, cloverInv, kappa, mass, mu, mu_factor, dirac, matpc, need_bidirectional, use_mma, IntList<N...>());
      } else {
