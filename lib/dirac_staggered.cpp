...
namespace quda {
  ...
  void DiracStaggeredPC::MLocal(ColorSpinorField &out, const ColorSpinorField &in) const
  {
    ...
    ApplyLocalStaggered(*tmp1, in, *gauge, *gauge, 0.0, in, other_parity, false, QUDA_STAGGERED_LOCAL_STEP1);

    // apply -D_eo [-D_oe] + 4 m^2
    ApplyLocalStaggered(out, *tmp1, *gauge, *gauge, 4. * mass * mass, in, parity, false, QUDA_STAGGERED_LOCAL_STEP2);
