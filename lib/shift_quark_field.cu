...
namespace quda {
  ...
  template<IndexType idxType, typename Int>
    ...
    template <typename FloatN, int N, typename Arg> __global__ void shiftColorSpinorFieldKernel(Arg arg)
    {
      int shift[4] = {0, 0, 0, 0};
    ...
    template <typename FloatN, int N, typename Arg> __global__ void shiftColorSpinorFieldExternalKernel(Arg arg)
    {
      unsigned int idx = blockIdx.x * (blockDim.x) + threadIdx.x;
      unsigned int gridSize = gridDim.x * blockDim.x;
