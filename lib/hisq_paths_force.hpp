...
namespace quda
{
  ...
  namespace fermion_force
  {
    ...
    struct dim_dir_pair {
      ...
      int signed_dir() { return is_invalid() ? -1 : ((dir == 1) ? dim : (7 - dim)); }
    ...
    public:
      OneLinkForce(Arg &arg, const GaugeField &link, const GaugeField &force) :
        TunableKernel3D(link, 2, 4), arg(arg), force(force), link(link)
      {
    ...
    template <typename Arg> class AllThreeAllLepageLinkForce : public TunableKernel2D
    {
      ...
      {
        return sizeof(Matrix<complex<typename Arg::real>, Arg::nColor>);
      }

      unsigned int maxSharedBytesPerBlock() const override { return maxDynamicSharedBytesPerBlock(); }

    public:
      AllThreeAllLepageLinkForce(Arg &arg, const GaugeField &link, dim_dir_pair sig, dim_dir_pair mu,
                                 dim_dir_pair mu_next, const PathCoefficients<typename Arg::real> &act_path_coeff,
                                 const GaugeField &force, const GaugeField &p3, const GaugeField &pMu_next) :
        ...
        has_lepage(act_path_coeff.lepage != 0.)
      {
        ...
        if (mu.is_valid()) {
          ...
          u32toa(aux2, mu.dim);
          strcat(aux, aux2);
          if (has_lepage) { strcat(aux, ",lepage"); }
        }
        if (mu_next.is_valid()) {
          strcat(aux, ",mu_next=");
      ...
      template <int sig, int mu, int mu_next> void instantiate(TuneParam &tp, const qudaStream_t &stream)
      {
        if (has_lepage)
          launch<AllThreeAllLepageLink>(
            tp, stream, FatLinkParam<Arg, sig, mu, mu_next, DIR_IGNORED, DIR_IGNORED, COMPUTE_LEPAGE_YES>(arg));
        else
          launch<AllThreeAllLepageLink>(
            tp, stream, FatLinkParam<Arg, sig, mu, mu_next, DIR_IGNORED, DIR_IGNORED, COMPUTE_LEPAGE_NO>(arg));
      }

      template <int sig, int mu> void instantiate(dim_dir_pair mu_next, TuneParam &tp, const qudaStream_t &stream)
      {
        if (mu_next.is_invalid())
          instantiate<sig, mu, DIR_IGNORED>(tp, stream);
        else if (mu_next.is_forward())
          instantiate<sig, mu, DIR_POSITIVE>(tp, stream);
        else
          instantiate<sig, mu, DIR_NEGATIVE>(tp, stream);
      }
      ...
      {
        if (mu.is_invalid())
          instantiate<sig, DIR_IGNORED>(mu_next, tp, stream);
        else if (mu.is_forward())
          instantiate<sig, DIR_POSITIVE>(mu_next, tp, stream);
        else
          instantiate<sig, DIR_NEGATIVE>(mu_next, tp, stream);
      }

      void instantiate(dim_dir_pair sig, dim_dir_pair mu, dim_dir_pair mu_next, TuneParam &tp, const qudaStream_t &stream)
      {
        if (sig.is_forward())
          instantiate<DIR_POSITIVE>(mu, mu_next, tp, stream);
        else
          instantiate<DIR_NEGATIVE>(mu, mu_next, tp, stream);
      }
      ...
      long long bytes() const override
      {
        ...
        // logic correction
        if (mu_next.is_invalid() && !has_lepage) { link_bytes_per_site -= arg.link.Bytes(); }
    ...
    template <typename Arg> class AllFiveAllSevenLinkForce : public TunableKernel2D
    {
      ...
      unsigned int sharedBytesPerThread() const override
      {
        return (sig.is_forward() ? 3 : 2) * sizeof(Matrix<complex<typename Arg::real>, Arg::nColor>);
      }
    ...
    public:
        ...
        nu_next(nu_next)
      {
        ...
        if (nu.is_valid()) {
          ...
          for (arg.rho = 0; arg.rho < 4; arg.rho++) {
            if (arg.rho != sig.dim && arg.rho != mu.dim && arg.rho != nu.dim) break;
          }
        } else {
          arg.rho = -1;
        }
      ...
      {
        if (mu.is_forward())
          instantiate<sig, DIR_POSITIVE>(nu, nu_next, tp, stream);
        else
          instantiate<sig, DIR_NEGATIVE>(nu, nu_next, tp, stream);
      }

      void instantiate(dim_dir_pair sig, dim_dir_pair mu, dim_dir_pair nu, dim_dir_pair nu_next, TuneParam &tp,
                       const qudaStream_t &stream)
      {
        if (sig.is_forward())
          instantiate<DIR_POSITIVE>(mu, nu, nu_next, tp, stream);
        else
          instantiate<DIR_NEGATIVE>(mu, nu, nu_next, tp, stream);
      }
      ...
      long long bytes() const override
      {
        ...
        if (nu.is_valid()) {
          ...
          cm_bytes_per_site += 2 * arg.qNuMu.Bytes() + 2 * arg.pNuMu.Bytes() + arg.p5.Bytes() + 2 * arg.shortP.Bytes()
            + 4 * arg.force.Bytes();
          if (sig.is_forward()) cm_bytes_per_site += arg.qNuMu.Bytes() + arg.pNuMu.Bytes();
        }

        // MiddleFive contribution
        if (nu_next.is_valid()) {
          link_bytes_per_site += 3 * arg.link.Bytes();
          cm_bytes_per_site += arg.pMu.Bytes() + arg.p5.Bytes() + arg.pNuMu_next.Bytes() + arg.qNuMu_next.Bytes();
        }
    ...
    struct HisqStaplesForce {
      ...
      void hisqFiveSeven(GaugeField &newOprod, GaugeField &P3, GaugeField_ref &P5, GaugeField_ref &Pnumu,
                         GaugeField_ref &Qnumu, GaugeField_ref &Pnumu_next, GaugeField_ref &Qnumu_next,
                         const GaugeField &Pmu, const GaugeField &link, const PathCoefficients<real> &act_path_coeff,
                         dim_dir_pair sig_pair, dim_dir_pair mu_pair)
      {
        ...
        AllFiveAllSevenLinkArg<Float, nColor, recon, phase> middleFiveLinkArg(
          newOprod, P3, Pmu, P5, Pnumu_next, Qnumu_next, Pnumu, Qnumu, link, act_path_coeff);
        AllFiveAllSevenLinkForce<decltype(middleFiveLinkArg)> middleFiveArg(middleFiveLinkArg, link, sig_pair, mu_pair,
                                                                            dim_dir_pair::invalid_pair(), nu_vals[0],
                                                                            newOprod, P3, P5, Pnumu, Qnumu);
        ...
        for (int i = 0; i < 3; i++) {
          ...
          AllFiveAllSevenLinkArg<Float, nColor, recon, phase> allFiveAllSevenLinkArg(
            newOprod, P3, Pmu, P5, Pnumu, Qnumu, Pnumu_next, Qnumu_next, link, act_path_coeff);
          AllFiveAllSevenLinkForce<decltype(allFiveAllSevenLinkArg)> allFiveAllSevenLink(
            allFiveAllSevenLinkArg, link, sig_pair, mu_pair, nu_vals[i], nu_vals[i + 1], newOprod, P3, P5, Pnumu_next,
            Qnumu_next);
        ...
        AllFiveAllSevenLinkArg<Float, nColor, recon, phase> allSevenSideFiveLinkArg(
          newOprod, P3, Pmu, P5, Pnumu, Qnumu, Pnumu_next, Qnumu_next, link, act_path_coeff);
        AllFiveAllSevenLinkForce<decltype(allSevenSideFiveLinkArg)> allSevenSideFiveLink(
          allSevenSideFiveLinkArg, link, sig_pair, mu_pair, nu_vals[3], dim_dir_pair::invalid_pair(), newOprod, P3, P5,
          Pnumu, Qnumu);
      }

      HisqStaplesForce(const GaugeField &link, GaugeField &P3, GaugeField_ref &Pmu, GaugeField_ref &P5,
                       GaugeField_ref &Pnumu, GaugeField_ref &Qnumu, GaugeField_ref &Pmu_next,
                       GaugeField_ref &Pnumu_next, GaugeField_ref &Qnumu_next, GaugeField &newOprod,
                       const GaugeField &oprod, const double *path_coeff_array)
      {
        ...
        for (int sig = 0; sig < 8; sig++) {
          ...
          // In: oprod, link
          // Ignored: Pmu_next
          AllThreeAllLepageLinkArg<Float, nColor, recon, phase> middleThreeLinkArg(newOprod, P3, oprod, Pmu, Pmu_next,
                                                                                   link, act_path_coeff);
          AllThreeAllLepageLinkForce<decltype(middleThreeLinkArg)> middleThreeLink(
            middleThreeLinkArg, link, sig_pair, dim_dir_pair::invalid_pair(), mu_vals[0], act_path_coeff, newOprod, P3,
            Pmu_next);
          ...
          hisqFiveSeven(newOprod, P3, P5, Pnumu, Qnumu, Pnumu_next, Qnumu_next, Pmu_next, link, act_path_coeff,
                        sig_pair, mu_vals[0]);
          ...
          for (int i = 0; i < 5; i++) {
            ...
            // Out: (second) Pmu
            AllThreeAllLepageLinkArg<Float, nColor, recon, phase> allThreeAllLepageLinkArg(
              newOprod, P3, oprod, Pmu, Pmu_next, link, act_path_coeff);
            AllThreeAllLepageLinkForce<decltype(allThreeAllLepageLinkArg)> allLepageAllThreeLink(
              allThreeAllLepageLinkArg, link, sig_pair, mu_vals[i], mu_vals[i + 1], act_path_coeff, newOprod, P3,
              Pmu_next);

            // All 5 and 7 link contributions, as above
            hisqFiveSeven(newOprod, P3, P5, Pnumu, Qnumu, Pnumu_next, Qnumu_next, Pmu_next, link, act_path_coeff,
                          sig_pair, mu_vals[i + 1]);
          }
          ...
          AllThreeAllLepageLinkArg<Float, nColor, recon, phase> allLepageSideThreeLinkArg(
            newOprod, P3, oprod, Pmu, Pmu_next, link, act_path_coeff);
          AllThreeAllLepageLinkForce<decltype(allLepageSideThreeLinkArg)> allLepageSideThreeLink(
            allLepageSideThreeLinkArg, link, sig_pair, mu_vals[5], dim_dir_pair::invalid_pair(), act_path_coeff,
            newOprod, P3, Pmu_next);
        } // sig
      }
    };

    template <typename Float, QudaReconstructType recon, QudaStaggeredPhase phase>
    void hisqStaplesForceImpl(GaugeField &newOprod, const GaugeField &oprod, const GaugeField &link,
                              const double path_coeff_array[6])
    {
      ...
      GaugeField_ref Pmu_ref(Pmu);
      GaugeField_ref P5_ref(P5);
      GaugeField_ref Pnumu_ref(Pnumu);
      GaugeField_ref Qnumu_ref(Qnumu);
      GaugeField_ref Pmu_next_ref(Pmu_next);
      GaugeField_ref Pnumu_next_ref(Pnumu_next);
      GaugeField_ref Qnumu_next_ref(Qnumu_next);

      HisqStaplesForce<Float, 3, recon, phase>(link, P3, Pmu_ref, P5_ref, Pnumu_ref, Qnumu_ref, Pmu_next_ref,
                                               Pnumu_next_ref, Qnumu_next_ref, newOprod, oprod, path_coeff_array);

      getProfile().TPSTOP(QUDA_PROFILE_COMPUTE);
    }

    template <typename Arg> class HisqLongForce : public TunableKernel2D
    {
    ...
    public:
      HisqLongForce(Arg &arg, GaugeField &force, const GaugeField &meta) :
        TunableKernel2D(meta, 2), arg(arg), force(force), meta(meta)
      {
        strcat(aux, comm_dim_partitioned_string());
    ...
    template <typename Float, QudaReconstructType recon, QudaStaggeredPhase phase>
    void hisqLongLinkForceImpl(GaugeField &newOprod, const GaugeField &oldOprod, const GaugeField &link, double coeff)
    {
      getProfile().TPSTART(QUDA_PROFILE_COMPUTE);
      checkNative(link, oldOprod, newOprod);
      checkLocation(newOprod, oldOprod, link);
      checkPrecision(newOprod, link, oldOprod);
      HisqLongLinkForce<Float, 3, recon, phase>(link, newOprod, oldOprod, coeff);
      getProfile().TPSTOP(QUDA_PROFILE_COMPUTE);
    }

    template <typename Arg> class HisqCompleteLinkForce : public TunableKernel2D
    {
    ...
    template <typename Float, QudaReconstructType recon, QudaStaggeredPhase phase>
    void hisqCompleteForceImpl(GaugeField &force, const GaugeField &link)
    {
      getProfile().TPSTART(QUDA_PROFILE_COMPUTE);
      checkNative(link, force);
      checkLocation(force, link);
      checkPrecision(link, force);
      HisqCompleteForce<Float, 3, recon, phase>(link, force);
      getProfile().TPSTOP(QUDA_PROFILE_COMPUTE);
    }
