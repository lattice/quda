  ...
  namespace blas_lapack
  {
	...
	if (cublas_param.data_type == QUDA_CUBLAS_DATATYPE_Z) {
	  ...
	  if(batch > 1) {
	    ...
	    Z **A_ptr_array_h = static_cast<Z**>(pool_pinned_malloc(batch*sizeof(Z*)));
	    Z **B_ptr_array_h = static_cast<Z**>(pool_pinned_malloc(batch*sizeof(Z*)));
	    Z **C_ptr_array_h = static_cast<Z**>(pool_pinned_malloc(batch*sizeof(Z*)));

	    for (uint64_t i = 0; i < batch; i++) {
              A_ptr_array[i] = static_cast<Z*>(A_d) + cublas_param.a_offset + i * A_batch_size;
              B_ptr_array[i] = static_cast<Z*>(B_d) + cublas_param.b_offset + i * B_batch_size;
              C_ptr_array[i] = static_cast<Z*>(C_d) + cublas_param.c_offset + i * C_batch_size;
            }

	    qudaMemcpy(A_ptr_array, A_ptr_array_h, batch * sizeof(Z *), cudaMemcpyHostToDevice);
	    qudaMemcpy(B_ptr_array, B_ptr_array_h, batch * sizeof(Z *), cudaMemcpyHostToDevice);
	    qudaMemcpy(C_ptr_array, C_ptr_array_h, batch * sizeof(Z *), cudaMemcpyHostToDevice);
	  ...
	  if(batch > 1) {
	    ...
	    D **A_ptr_array_h = static_cast<D**>(pool_pinned_malloc(batch*sizeof(D*)));
	    D **B_ptr_array_h = static_cast<D**>(pool_pinned_malloc(batch*sizeof(D*)));
	    D **C_ptr_array_h = static_cast<D**>(pool_pinned_malloc(batch*sizeof(D*)));

	    for (uint64_t i = 0; i < batch; i++) {
              A_ptr_array[i] = static_cast<D*>(A_d) + cublas_param.a_offset + i * A_batch_size;
              B_ptr_array[i] = static_cast<D*>(B_d) + cublas_param.b_offset + i * B_batch_size;
              C_ptr_array[i] = static_cast<D*>(C_d) + cublas_param.c_offset + i * C_batch_size;
            }
	    
	    qudaMemcpy(A_ptr_array, A_ptr_array_h, batch * sizeof(D *), cudaMemcpyHostToDevice);
	    qudaMemcpy(B_ptr_array, B_ptr_array_h, batch * sizeof(D *), cudaMemcpyHostToDevice);
	    qudaMemcpy(C_ptr_array, C_ptr_array_h, batch * sizeof(D *), cudaMemcpyHostToDevice);
	  ...
	  if(batch > 1) {
	    ...
	    S **A_ptr_array_h = static_cast<S**>(pool_device_malloc(batch*sizeof(S*)));
	    S **B_ptr_array_h = static_cast<S**>(pool_device_malloc(batch*sizeof(S*)));
	    S **C_ptr_array_h = static_cast<S**>(pool_device_malloc(batch*sizeof(S*)));

	    for (uint64_t i = 0; i < batch; i++) {
              A_ptr_array[i] = static_cast<S*>(A_d) + cublas_param.a_offset + i * A_batch_size;
              B_ptr_array[i] = static_cast<S*>(B_d) + cublas_param.b_offset + i * B_batch_size;
              C_ptr_array[i] = static_cast<S*>(C_d) + cublas_param.c_offset + i * C_batch_size;
            }
	    
	    qudaMemcpy(A_ptr_array, A_ptr_array_h, batch * sizeof(S *), cudaMemcpyHostToDevice);
	    qudaMemcpy(B_ptr_array, B_ptr_array_h, batch * sizeof(S *), cudaMemcpyHostToDevice);
	    qudaMemcpy(C_ptr_array, C_ptr_array_h, batch * sizeof(S *), cudaMemcpyHostToDevice);
	...
	return flops;
      ...
      long long stridedBatchGEMM(void *A_data, void* B_data, void* C_data, QudaCublasParam cublas_param, QudaFieldLocation location)
      {
        ...
        if (cublas_param.data_order == QUDA_CUBLAS_DATAORDER_ROW) {
          ...
          std::swap(cublas_param.trans_a, cublas_param.trans_b);
          std::swap(cublas_param.a_offset, cublas_param.b_offset);
          std::swap(cublas_param.strideA, cublas_param.strideB);
          std::swap(A_data, B_data);
        }
	...
	// is used.
        unsigned int strideA = cublas_param.strideA < 0 ? A_batch_size : cublas_param.strideA;
        unsigned int strideB = cublas_param.strideB < 0 ? B_batch_size : cublas_param.strideB;
        unsigned int strideC = cublas_param.strideC < 0 ? C_batch_size : cublas_param.strideC;
        ...
        if (cublas_param.data_type == QUDA_CUBLAS_DATATYPE_Z) {
          ...
          if(batch > 1) {
            ...
            error = cublasZgemmStridedBatched(handle, trans_a, trans_b, cublas_param.m,
                                              cublas_param.n, cublas_param.k, &alpha,
                                              (Z*)A_d + cublas_param.a_offset, cublas_param.lda, strideA,
                                              (Z*)B_d + cublas_param.b_offset, cublas_param.ldb, strideB, &beta,
                                              (Z*)C_d + cublas_param.c_offset, cublas_param.ldc, strideC, batch);
          ...
          if(batch > 1) {
            ...
            error = cublasCgemmStridedBatched(handle, trans_a, trans_b, cublas_param.m,
                                              cublas_param.n, cublas_param.k, &alpha,
                                              (C*)A_d + cublas_param.a_offset, cublas_param.lda, strideA,
                                              (C*)B_d + cublas_param.b_offset, cublas_param.ldb, strideB, &beta,
                                              (C*)C_d + cublas_param.c_offset, cublas_param.ldc, strideC, batch);
          ...
          if(batch > 1) {
            ...
            error = cublasDgemmStridedBatched(handle, trans_a, trans_b, cublas_param.m,
                                              cublas_param.n, cublas_param.k, &alpha,
                                              (D*)A_d + cublas_param.a_offset, cublas_param.lda, strideA,
                                              (D*)B_d + cublas_param.b_offset, cublas_param.ldb, strideB, &beta,
                                              (D*)C_d + cublas_param.c_offset, cublas_param.ldc, strideC, batch);
          ...
          if(batch > 1) {
            ...
            error = cublasSgemmStridedBatched(handle, trans_a, trans_b, cublas_param.m,
                                              cublas_param.n, cublas_param.k, &alpha,
                                              (S*)A_d + cublas_param.a_offset, cublas_param.lda, strideA,
                                              (S*)B_d + cublas_param.b_offset, cublas_param.ldb, strideB, &beta,
                                              (S*)C_d + cublas_param.c_offset, cublas_param.ldc, strideC, batch);
