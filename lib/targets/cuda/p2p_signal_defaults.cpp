...
//     REMOTE_IPC, and default() returns STREAM_GATED.
//   - Non-MNNVL build:           both supported, and default() returns
//     REMOTE_IPC (events) -- the established develop behaviour, least surprise.
...
namespace quda::comm
{
  ...
  {
#ifdef QUDA_MNNVL
    // REMOTE_IPC (events) is unsupported across the MNNVL fabric, so STREAM_GATED
    // is the only choice.
    return QudaP2PSignal::STREAM_GATED;
#else
    // Default to REMOTE_IPC (events) to match develop behaviour; STREAM_GATED is
    // opt-in via QUDA_P2P_TRANSPORT=stream_gated (validated by the resolver against
    // p2p_signal_supported()).
    return QudaP2PSignal::REMOTE_IPC;
#endif
  }
} // namespace quda::comm
