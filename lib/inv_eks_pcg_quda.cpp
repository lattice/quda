...
*/
class EKS_PCGUpdate : public Worker {
  ...
  public:
#ifdef BLOCKSOLVER_MULTIFUNCTIONS
    EKS_PCGUpdate(std::shared_ptr<ColorSpinorField>& x_sloppyp, std::shared_ptr<ColorSpinorField> &  p_oldp, Complex* alpha) :
#else
    EKS_PCGUpdate(std::shared_ptr<ColorSpinorField>& x_sloppyp, std::shared_ptr<ColorSpinorField> &  p_oldp, MatrixXcd& alpha) :
#endif
...
// the current status in EKS_PCG is to just look for reliable updates in 'r'.
int EKS_PCG::block_reliable(double &rNorm, double &maxrx, double &maxrr, const double &r2, const double &delta) {
  // reliable updates
  // rNorm = sqrt(r2);
...
// temporary workaround
...
#ifdef BLOCKSOLVER_MULTIFUNCTIONS
  EKS_PCGUpdate EKS_PCG_update(x_sloppyp, p_oldp, alpha_raw);
#else
  EKS_PCGUpdate EKS_PCG_update(x_sloppyp, p_oldp, alpha);
#endif

#else // !BLOCKSOLVER_PRECISE_Q

#ifdef BLOCKSOLVER_MULTIFUNCTIONS
  EKS_PCGUpdate EKS_PCG_update(x_sloppyp, tmpp, alpha_raw);
#else
  EKS_PCGUpdate EKS_PCG_update(x_sloppyp, tmpp, alpha);
#endif
  ...
  while ( !allconverged && k < param.maxiter ) {
    ...
    {
      dslash::aux_worker = &EKS_PCG_update;
    }
    else
    {
...
//MWALTBETA
  ...
  blas::zero(*tmpp);
    ...
    if (block_reliable(rNorm, maxrx, maxrr, r2, delta))
    {
      ...
      // Reliable updates step 9: Set S = C * C_old^{-1} (equation 6.1 in the EKS_PCGrQ paper)
      // S = C * C_old.inverse();
#ifdef BLOCKSOLVER_VERBOSE
