...
#define ASSN_GAUGE_MATRIX_18_FLOAT2_TEX(G, gauge, dir, idx, stride)	\
 for (int i=0; i<9; i++) {						\
   G[i] = TEX1DFETCH(float2, (gauge), idx + ((dir/2)*9+i)*stride);	\
 }
...
#define ASSN_GAUGE_MATRIX_12_FLOAT4_TEX(G, gauge, dir, idx, stride)	\
  for (int i=0; i<3; i++) {						\
    float4 tmp = TEX1DFETCH(float4, (gauge), idx + ((dir/2)*3+i)*stride); \
    G[2*i+0] = make_float2(tmp.x, tmp.y);				\
    G[2*i+1] = make_float2(tmp.z, tmp.w);				\
  }									\
  for (int i=6; i<9; i++) G[i] = make_float2(0,0);
...
#define ASSN_GAUGE_MATRIX_8_FLOAT4_TEX(G, gauge, dir, idx, stride)	\
  for (int i=0; i<2; i++) {						\
    float4 tmp = TEX1DFETCH(float4, (gauge), idx + ((dir/2)*2+i)*stride); \
    G[2*i+0] = make_float2(tmp.x,tmp.y);				\
    G[2*i+1] = make_float2(tmp.z,tmp.w);				\
  }									\
  for (int i=4; i<9; i++) G[i] = make_float2(0,0);			\
  G[7].x = G[0].x;							\
  G[7].y = G[0].y;
...
#define ASSN_GAUGE_MATRIX_8_SHORT4_TEX(G, gauge, dir, idx, stride)	\
  ...
  for (int i=4; i<9; i++) G[i] = make_float2(0,0);			\
  G[7].x = G[0].x = pi_f*G[0].x;					\
  G[7].y = G[0].y = pi_f*G[0].y;

#define ASSN_GAUGE_MATRIX_18_DOUBLE2(G, gauge_, dir, idx, stride)    \
  double2 *gauge = (double2*)gauge_;				     \
  for (int i=0; i<9; i++) {					     \
    G[i] = gauge[idx + ((dir/2)*9+i)*stride];			     \
  }
...
#define ASSN_GAUGE_MATRIX_18_SHORT2(G, gauge_, dir, idx, stride)	\
  short2 *gauge = (short2*)gauge_;					\
  for (int i=0; i<9; i++) {						\
    G[i] = short22float2(((short2*)gauge)[idx + ((dir/2)*9+i)*stride]);	\
  }
  
#define ASSN_GAUGE_MATRIX_12_DOUBLE2(G, gauge_, dir, idx, stride)   \
  double2 *gauge = (double2*)gauge_;				    \
  for (int i=0; i<6; i++) {					    \
    G[i] = gauge[idx + ((dir/2)*6+i)*stride];			    \
  }

#define ASSN_GAUGE_MATRIX_12_FLOAT4(G, gauge_, dir, idx, stride)    \
  float4 *gauge = (float4*)gauge_;				    \
  for (int i=0; i<3; i++) {					    \
    float4 tmp = gauge[idx + ((dir/2)*3+i)*stride];		    \
    G[2*i+0] = make_float2(tmp.x, tmp.y);			    \
    G[2*i+1] = make_float2(tmp.z, tmp.w);			    \
  }								    \
  for (int i=6; i<9; i++) G[i] = make_float2(0,0);
...
#define ASSN_GAUGE_MATRIX_12_SHORT4(G, gauge_, dir, idx, stride)	\
  short4 *gauge = (short4*)gauge_;					\
  for (int i=0; i<3; i++) {						\
    float4 tmp = short42float4(gauge[idx + ((dir/2)*3+i)*stride]);	\
    G[2*i+0] = make_float2(tmp.x, tmp.y);				\
    G[2*i+1] = make_float2(tmp.z, tmp.w);				\
  }									\
  for (int i=6; i<9; i++) G[i] = make_float2(0,0);
...
#define ASSN_GAUGE_MATRIX_8_DOUBLE2(G, gauge_, dir, idx, stride)	\
  double2 *gauge = (double2*)gauge_;					\
  for (int i=0; i<4; i++) {						\
    G[i] = gauge[idx + ((dir/2)*4+i)*stride];				\
  }									\
  for (int i=0; i<4; i++) {						\
    G[i] = make_double2(0,0);						\
  }									\
  G[7] = G[0];								\
...
#define ASSN_GAUGE_MATRIX_8_FLOAT4(G, gauge_, dir, idx, stride)		\
  float4 *gauge = (float4*)gauge_;					\
  for (int i=0; i<2; i++) {						\
    float4 tmp = gauge[idx + ((dir/2)*2+i)*stride];			\
    G[2*i+0] = make_float2(tmp.x,tmp.y);				\
    G[2*i+1] = make_float2(tmp.z,tmp.w);				\
  }									\
  for (int i=4; i<9; i++) G[i] = make_float2(0,0);			\
...
#define ASSN_GAUGE_MATRIX_8_SHORT4(G, gauge_, dir, idx, stride)		\
  short4 *gauge = (short4*)gauge_;					\
  for (int i=0; i<2; i++) {						\
    float4 tmp = short2float4(gauge[idx + ((dir/2)*2+i)*stride]);	\
    G[2*i+0] = make_float2(tmp.x,tmp.y);				\
    G[2*i+1] = make_float2(tmp.z,tmp.w);				\
  }									\
  for (int i=4; i<9; i++) G[i] = make_float2(0,0);			\
...
#define RESCALE(G, max) for (int i=0; i<9; i++) { G[i].x *= max; G[i].y *= max; }
...
#define ASSN_GAUGE_MATRIX_18_DOUBLE2_TEX(G, gauge, dir, idx, stride) \
   for (int i=0; i<9; i++) {					     \
     G[i] = fetch_double2((gauge), idx + ((dir/2)*9+i)*stride);	     \
   }

#define ASSN_GAUGE_MATRIX_12_DOUBLE2_TEX(G, gauge, dir, idx, stride)	\
   for (int i=0; i<6; i++) {						\
     G[i] = fetch_double2((gauge), idx + ((dir/2)*6+i)*stride);		\
   }									\
   for (int i=6; i<9; i++) {						\
     G[i] = make_double2(0,0);						\
   }
