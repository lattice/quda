...
namespace quda
{
  ...
  using path_coeff_t = floatfloat;
#else
  using path_coeff_t = double;
#endif

  template <int dim_> struct paths {
    static constexpr int dim = dim_;
    const int num_paths;
    const int max_length;
    int *input_path[dim];
    const int *length;
    const path_coeff_t *path_coeff;
    int *buffer;
    int count;

    paths(std::vector<int **> &input_path, std::vector<int> &length_h, std::vector<real_t> &path_coeff_h, int num_paths,
          int max_length) :
      num_paths(num_paths), max_length(max_length), count(0)
    {
      ...
      int pad = ((sizeof(path_coeff_t) - bytes % sizeof(path_coeff_t)) % sizeof(path_coeff_t)) / sizeof(int);
      bytes += pad * sizeof(int) + num_paths * sizeof(path_coeff_t);
      ...
      path_coeff_t *path_coeff_
        = reinterpret_cast<path_coeff_t *>(path_h + dim * num_paths * max_length + num_paths + pad);
      for (auto i = 0; i < num_paths; i++)
        path_coeff_[i] = static_cast<path_coeff_t>(static_cast<double>(path_coeff_h[i]));
      ...
      path_coeff = reinterpret_cast<path_coeff_t *>(buffer + dim * num_paths * max_length + num_paths + pad);
    }
  ...
  */
  ...
  __device__ __host__ inline typename Arg::Link computeGaugePath(const Arg &arg, int x[4], int parity, const int *path,
                                                                 int length, I &dx)
  {
