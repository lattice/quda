...
namespace quda
{
  ...
  template <typename vFloatSpinor_, typename vFloatGauge_, int coarseDof_, int fineColor_, bool dagger_,
            typename fineColorSpinor, typename xInvGauge>
  struct ApplyStaggeredKDBlockArg {
    ...
    static constexpr int numTiles
      = (coarseDof + xinvColTileSize - 1) / xinvColTileSize; /** Number of tiles, should always be three. */
    ...
    ApplyStaggeredKDBlockArg(fineColorSpinor &out, const fineColorSpinor &in, const xInvGauge &xInv, const int *x_size_,
                             const int *xc_size_) :
      out(out), in(in), xInv(xInv), fineVolumeCB(in.volumeCB), coarseVolumeCB(xInv.VolumeCB())
    {
  ...
  template <typename Arg> __global__ void ApplyStaggeredKDBlockGPU(Arg arg)
  {
    ...
    constexpr int buffer_size = Arg::fineColor * 16
      * Arg::paddedSpinorSizeKD; // 256 * ((Arg::fineColor * 16 * Arg::paddedSpinorSizeKD * sizeof(complex)) / 256 + 1);
    ...
    complex *xinv_buffer = cs_buffer + 2 * (blockDim.x / 256) * buffer_size + unit_of_work * fullTileSize
      + mid_idx * Arg::xinvRowTileSize * Arg::xinvPaddedColTileSize;
    ...
    int y_bit = tmp_mid_idx & 1;
    tmp_mid_idx >>= 1;
    int z_bit = tmp_mid_idx & 1;
    tmp_mid_idx >>= 1;
    int t_bit = tmp_mid_idx & 1;
    tmp_mid_idx >>= 1;
    ...
    int x_spinor_cb = (((((2 * coarseCoords[3] + t_bit) * arg.x_size[2]) + 2 * coarseCoords[2] + z_bit) * arg.x_size[1]
                        + 2 * coarseCoords[1] + y_bit)
                         * arg.x_size[0]
                       + 2 * coarseCoords[0] + x_bit)
      >> 1;
...
#pragma unroll
          for (int col = 0; col < Arg::xinvColTileSize; col++) {
            xinv_buffer[fast_idx * Arg::xinvPaddedColTileSize + col]
              = conj(arg.xInv(0, parity_coarse_xinv, x_coarse_xinv_cb, 0, 0, Arg::xinvColTileSize * tile_col + col,
                              Arg::xinvRowTileSize * tile_row + fast_idx));
          }
        } else {
#pragma unroll
          for (int row = 0; row < Arg::xinvColTileSize; row++) {
            xinv_buffer[row * Arg::xinvPaddedColTileSize + fast_idx]
              = arg.xInv(0, parity_coarse_xinv, x_coarse_xinv_cb, 0, 0, Arg::xinvRowTileSize * tile_row + row,
                         Arg::xinvColTileSize * tile_col + fast_idx);
          }
        }
...
#pragma unroll
        for (int row = 0; row < Arg::xinvRowTileSize; row++) {
          ...
          const complex cs_component
            = in_buffer[Arg::paddedSpinorSizeKD * Arg::fineColor * mid_idx + Arg::xinvColTileSize * tile_col + fast_idx];
          const complex prod = cmul(xinv_elem, cs_component);

          const complex the_sum = WarpReduce16(temp_storage_16).Sum(prod);

          if (fast_idx == 0) {
            out_buffer[Arg::paddedSpinorSizeKD * Arg::fineColor * mid_idx + Arg::xinvRowTileSize * tile_row + row]
              += the_sum;
          }
        }
      }
