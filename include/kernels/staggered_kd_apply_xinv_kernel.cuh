...
namespace quda {
  ...
  template<typename Arg> struct StaggeredKDBlock {
    ...
    __device__ inline void operator()(int tid)
    {
      ...
      const int unit_of_work = device::thread_idx(tid, arg).x / 256;
      ...
      complex* xinv_buffer = arg.cs_buffer.data() + 2 * (device::block_dim(arg).x / 256) * buffer_size + unit_of_work * fullTileSize + mid_idx * Arg::xinvRowTileSize * Arg::xinvPaddedColTileSize;
