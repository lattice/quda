  ...
  */
  ...
  __device__ __host__ __forceinline__ void ndegTwistedMass(Arg &arg, int idx, int flavor, int parity)
  {
    ...
    if (!dagger || asymmetric) // defined in dslash_wilson.cuh
      ...
      applyWilsonTM<Float,nDim,nColor,nParity,dagger,2,kernel_type>(out, arg, coord, x_cb, flavor, parity, idx, thread_dim, active);
    ...
    if (xpay && kernel_type == INTERIOR_KERNEL) {
      ...
      if (!dagger || asymmetric) { // apply inverse twist which is undone below
        ...
        if (flavor==0) out += arg.a_inv * (x0 + arg.b_inv*x0.igamma(4) + arg.c_inv*x1);
        else           out += arg.a_inv * (x1 - arg.b_inv*x1.igamma(4) + arg.c_inv*x0);
      } else {
    ...
    if ( isComplete<kernel_type>(arg, coord) && active ) {
      if (!dagger || asymmetric) { // apply A^{-1} to D*in
        ...
        if (flavor==0) out = arg.a * (out + arg.b*out.igamma(4) + arg.c*cache.load(threadIdx.x,1,threadIdx.z));
        else           out = arg.a * (out - arg.b*out.igamma(4) + arg.c*cache.load(threadIdx.x,0,threadIdx.z));
      }
    }
