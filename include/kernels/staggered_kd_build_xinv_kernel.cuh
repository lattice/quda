...
namespace quda
{
  ...
  template <typename Float_, int coarseSpin_, int fineColor_, int coarseColor_, typename xGauge, typename fineGauge>
  struct CalculateStaggeredKDBlockArg {
    ...
    CalculateStaggeredKDBlockArg(xGauge &X, const fineGauge &U, const double mass, const int *x_size_,
                                 const int *xc_size_) :
      X(X), U(U), spin_map(), mass(static_cast<Float>(mass)), fineVolumeCB(U.VolumeCB()), coarseVolumeCB(X.VolumeCB())
    {
...
#pragma unroll
        for (int c = 0; c < Arg::coarseColor; c++) {
          arg.X(0, parity, x_cb, s, s, c, c)
            = complex<Float>(static_cast<Float>(2.0) * arg.mass, 0.0); // staggered conventions. No need to +=
        }                                                              // Color
      }                                                                // Spin
    }
  }

  template <typename Arg> void ComputeStaggeredKDBlockCPU(Arg arg)
  {
...
#pragma omp parallel for
  ...
  template <typename Arg> __global__ void ComputeStaggeredKDBlockGPU(Arg arg)
  {
