...
namespace quda
{
  ...
  template <typename Float, int coarseSpin, int fineColor, int coarseColor, typename coarseGauge, typename fineGauge>
  struct CalculateStaggeredYArg {
    ...
    CalculateStaggeredYArg(coarseGauge &Y, coarseGauge &X, const fineGauge &U, double mass, const int *x_size_,
                           const int *xc_size_, int *geo_bs_, int spin_bs_) :
      Y(Y),
      X(X),
      U(U),
      spin_bs(spin_bs_),
...
#pragma unroll
        for (int c = 0; c < coarseColor; c++) {
          arg.X(0, parity, x_cb, s, s, c, c)
            = static_cast<Float>(2.0) * complex<Float>(arg.mass, 0.0); // staggered conventions. No need to +=
        }                                                              // Color
      }                                                                // Spin
    }
  }
