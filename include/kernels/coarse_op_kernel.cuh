...
namespace quda {
  ...
  template <bool from_coarse_, typename Float_, int fineSpin_, int coarseSpin_, int fineColor_, int coarseColor_,
  ...
  struct CalculateYArg {
    ...
    CalculateYArg(coarseGauge &Y, coarseGauge &X, coarseGaugeAtomic &Y_atomic, coarseGaugeAtomic &X_atomic,
                  ...
                  const fineClover &Cinv, double kappa, double mass, double mu, double mu_factor, const int *x_size_,
                  const int *xc_size_, int *geo_bs_, int spin_bs_, const int *fine_to_coarse, const int *coarse_to_fine,
                  bool bidirectional) :
      Y(Y),
      X(X),
      Y_atomic(Y_atomic),
  ...
  */
  ...
  template <int dim, QudaDirection dir, typename Arg> void ComputeUVCPU(Arg &arg)
  {
    using TileType = typename Arg::uvTileType;
...
#pragma omp parallel for
        ...
        for (int ic = 0; ic < TileType::m; ic += TileType::M)   // fine color
          for (int jc = 0; jc < TileType::n; jc += TileType::N) // coarse color
            if (dir == QUDA_FORWARDS
                || dir
                  == QUDA_IN_PLACE) // only for preconditioned clover is V != AV, will need extra logic for staggered KD
  ...
  template <int dim, QudaDirection dir, typename Arg> __global__ void ComputeUVGPU(Arg arg)
  {
    ...
    if (dir == QUDA_FORWARDS
        || dir == QUDA_IN_PLACE) // only for preconditioned clover is V != AV, will need extra logic for staggered KD
  ...
  */
  template <typename Arg> __device__ __host__ inline void computeAV(Arg &arg, int parity, int x_cb, int ch, int ic_c)
  {
...
#pragma omp parallel for
  ...
  template <typename Arg> __global__ void ComputeAVGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
    if (x_cb >= arg.fineVolumeCB) return;
  ...
  /**
     ...
     Where: s = fine spin, c' = coarse color, c = fine color
  */
  template <typename Arg> __device__ __host__ inline void computeTMAV(Arg &arg, int parity, int x_cb, int v)
  {
    using Float = typename Arg::Float;
    ...
    for (int s = 0; s < Arg::fineSpin / 2; s++) {
      for (int c = 0; c < Arg::fineColor; c++) { arg.AV(parity, x_cb, s, c, v) = arg.V(parity, x_cb, s, c, v) * fp; }
    }

    for (int s = Arg::fineSpin / 2; s < Arg::fineSpin; s++) {
      for (int c = 0; c < Arg::fineColor; c++) { arg.AV(parity, x_cb, s, c, v) = arg.V(parity, x_cb, s, c, v) * fm; }
    }

  } // computeTMAV

  template <typename Arg> void ComputeTMAVCPU(Arg &arg)
  {
    for (int parity=0; parity<2; parity++) {
#pragma omp parallel for
      for (int x_cb=0; x_cb<arg.fineVolumeCB; x_cb++) {
  ...
  template <typename Arg> __global__ void ComputeTMAVGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
    if (x_cb >= arg.fineVolumeCB) return;
...
#pragma unroll
        for (int j = 0; j <= i; j++) {
          A(i, j) = arg.C(0, parity, x_cb, 2 * ch + i / Arg::fineColor, 2 * ch + j / Arg::fineColor, i % Arg::fineColor,
                          j % Arg::fineColor);
        }
      }
...
#pragma omp parallel for reduction(max:max)
  ...
  template <bool twist, typename Arg> __global__ void ComputeCloverInvMaxGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
    if (x_cb >= arg.fineVolumeCB) return;
  ...
  /**
     ...
     Where: s = fine spin, c' = coarse color, c = fine color
  */
  template <typename Arg> __device__ __host__ inline void computeTMCAV(Arg &arg, int parity, int x_cb, int ch, int ic_c)
  {
    using Float = typename Arg::Float;
    constexpr int N = Arg::fineSpin * Arg::fineColor / 2;
    HMatrix<Float, N> A;
...
#pragma omp parallel for
  ...
  template <typename Arg> __global__ void ComputeTMCAVGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
    if (x_cb >= arg.fineVolumeCB) return;
  ...
  template <typename Arg> __device__ __host__ inline int virtualThreadIdx(const Arg &arg)
  {
  ...
  template <typename Arg> __device__ __host__ inline int virtualBlockDim(const Arg &arg)
  {
    int block_dim_x = blockDim.x / arg.aggregates_per_block;
    return block_dim_x;
  }

  template <typename Arg> __device__ __host__ inline int coarseIndex(const Arg &arg)
  {
    constexpr int warp_size = 32;
    int warp_lane = threadIdx.x % warp_size;
    int x_coarse = (arg.coarse_color_wave ? blockIdx.y : blockIdx.x) * arg.aggregates_per_block
      + warp_lane % arg.aggregates_per_block;
    return x_coarse;
  }
...
#pragma unroll
  ...
  inline __device__ __host__ void storeCoarseSharedAtomic(VUV &vuv, bool isDiagonal, int coarse_x_cb, int coarse_parity,
                                                          int i0, int j0, int parity, Arg &arg)
  {
    using Float = typename Arg::Float;
    using TileType = typename Arg::vuvTileType;
#ifdef __CUDA_ARCH__
    const int dim_index = arg.dim_index % arg.Y_atomic.geometry;
    __shared__ complex<storeType> X[Arg::max_color_height_per_block][Arg::max_color_width_per_block][4][Arg::coarseSpin]
                                   [Arg::coarseSpin];
    __shared__ complex<storeType> Y[Arg::max_color_height_per_block][Arg::max_color_width_per_block][4][Arg::coarseSpin]
                                   [Arg::coarseSpin];
...
#pragma unroll
            for (int s_col = 0; s_col < Arg::coarseSpin; s_col++) { // Chiral column block
              atomic_helper<Float, storeType>(&X[i_block0 + i][j_block0 + j][x_][s_row][s_col], arg.X_atomic,
                                              vuv[s_row * Arg::coarseSpin + s_col](i, j));
            }
          }
        } else {
#pragma unroll
...
#pragma unroll
            for (int s_col = 0; s_col < Arg::coarseSpin; s_col++) { // Chiral column block
              atomic_helper<Float, storeType>(&Y[i_block0 + i][j_block0 + j][x_][s_row][s_col], arg.Y_atomic,
                                              vuv[s_row * Arg::coarseSpin + s_col](i, j));
            }
          }
        }
...
#pragma unroll
  ...
  template <bool parity_flip, QudaDirection dir, typename VUV, typename Arg>
  inline __device__ __host__ void storeCoarseGlobalAtomic(VUV &vuv, bool isDiagonal, int coarse_x_cb, int coarse_parity,
                                                          int i0, int j0, Arg &arg)
  {
    using Float = typename Arg::Float;
    const int dim_index = arg.dim_index % arg.Y_atomic.geometry;
    using TileType = typename Arg::vuvTileType;
...
#pragma unroll
              for (int j = 0; j < TileType::N; j++)
                arg.X_atomic.atomicAdd(0, coarse_parity, coarse_x_cb, s_col, s_row, j0 + j, i0 + i,
                                       conj(vuv[s_row * Arg::coarseSpin + s_col](i, j)));
          }
        }
      } else {
#pragma unroll
...
#pragma unroll
  ...
  template <bool shared_atomic, bool parity_flip, int dim, QudaDirection dir, typename Arg, typename Gamma>
                                      ...
                                      int parity_coarse_, int coarse_x_cb_)
  {
    ...
    const bool isDiagonal
      = (isFromCoarseClover || ((coord[dim] + 1) % arg.x_size[dim]) / arg.geo_bs[dim] == coord_coarse[dim]) ? true :
                                                                                                              false;
...
#pragma unroll
                                    ...
                                    int &c_row, int &c_col)
  {
    if (arg.coarse_color_wave) {
      ...
      int c_row_block_idx_z = parity_flip ? (parity_c_row_block_idx_z % arg.coarse_color_grid_z) :
                                            (parity_c_row_block_idx_z / 2); // coarse color row index
  ...
  template <int dim, QudaDirection dir, typename Arg> void ComputeVUVCPU(Arg &arg)
  {
    using Float = typename Arg::Float;
...
#pragma omp parallel for
  ...
  template <bool shared_atomic, bool parity_flip, int dim, QudaDirection dir, typename Arg>
  __global__ void ComputeVUVGPU(Arg arg)
  {
    ...
    computeVUV<shared_atomic, parity_flip, dim, dir>(arg, gamma, parity, x_cb, c_row * arg.vuvTile.M,
                                                     c_col * arg.vuvTile.N, parity_coarse, x_coarse_cb);
  }
   ...
   */
  template <typename Arg> __device__ __host__ void computeYreverse(Arg &arg, int parity, int x_cb, int ic_c, int jc_c)
  {
    auto &Y = arg.Y;
...
#pragma unroll
  ...
  template <typename Arg> void ComputeYReverseCPU(Arg &arg)
  {
    for (int parity=0; parity<2; parity++) {
#pragma omp parallel for
  ...
  template <typename Arg> __global__ void ComputeYReverseGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
...
#pragma unroll
          ...
          X[s_c * Arg::coarseSpin + s_c]
            = cmac(conj(arg.V(parity, x_cb, s, ic, ic_c)), CV, X[s_c * Arg::coarseSpin + s_c]);
        } // Fine color row
      }   // Fine spin column
    }     // Fine spin
...
#pragma unroll
  ...
  template <typename Arg> void ComputeCoarseCloverCPU(Arg &arg)
  {
    for (int parity=0; parity<2; parity++) {
#pragma omp parallel for
      for (int x_cb=0; x_cb<arg.fineVolumeCB; x_cb++) {
        for (int jc_c = 0; jc_c < Arg::coarseColor; jc_c++) {
          for (int ic_c = 0; ic_c < Arg::coarseColor; ic_c++) { computeCoarseClover(arg, parity, x_cb, ic_c, jc_c); }
        }
      } // c/b volume
    } // parity
  }

  template <typename Arg> __global__ void ComputeCoarseCloverGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
    if (x_cb >= arg.fineVolumeCB) return;
  ...
  template <typename Arg> void AddCoarseDiagonalCPU(Arg &arg)
  {
    using Float = typename Arg::Float;
...
#pragma omp parallel for
  ...
  //Adds the identity matrix to the coarse local term.
  template <typename Arg> __global__ void AddCoarseDiagonalGPU(Arg arg)
  {
    using Float = typename Arg::Float;
  ...
  template <typename Arg> void AddCoarseStaggeredMassCPU(Arg &arg)
  {
    using Float = typename Arg::Float;
    for (int parity = 0; parity < 2; parity++) {
#pragma omp parallel for
  ...
  template <typename Arg> __global__ void AddCoarseStaggeredMassGPU(Arg arg)
  {
    using Float = typename Arg::Float;
  ...
  template <typename Arg> void AddCoarseTmDiagonalCPU(Arg &arg)
  {
    using Float = typename Arg::Float;
...
#pragma omp parallel for
  ...
  //Adds the twisted-mass term to the coarse local term.
  template <typename Arg> __global__ void AddCoarseTmDiagonalGPU(Arg arg)
  {
    using Float = typename Arg::Float;
   ...
   */
  template <typename Arg> __device__ __host__ void convert(Arg &arg, int parity, int x_cb, int c_row, int c_col)
  {
    using Float = typename Arg::Float;
...
#pragma unroll
  ...
  template <typename Arg> void ConvertCPU(Arg &arg)
  {
    for (int parity=0; parity<2; parity++) {
#pragma omp parallel for
      for (int x_cb=0; x_cb<arg.coarseVolumeCB; x_cb++) {
  ...
  template <typename Arg> __global__ void ConvertGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
    if (x_cb >= arg.coarseVolumeCB) return;
   ...
   */
  template <typename Arg> __device__ __host__ void rescaleY(Arg &arg, int parity, int x_cb, int c_row, int c_col)
  {
    using Float = typename Arg::Float;
...
#pragma unroll
  ...
  template <typename Arg> void RescaleYCPU(Arg &arg)
  {
    for (int parity=0; parity<2; parity++) {
#pragma omp parallel for
      for (int x_cb=0; x_cb<arg.coarseVolumeCB; x_cb++) {
  ...
  template <typename Arg> __global__ void RescaleYGPU(Arg arg)
  {
    int x_cb = blockDim.x*blockIdx.x + threadIdx.x;
    if (x_cb >= arg.coarseVolumeCB) return;
