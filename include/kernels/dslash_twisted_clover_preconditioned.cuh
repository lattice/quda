...
namespace quda {
  ...
  struct TwistedCloverArg : WilsonArg<Float,nColor,reconstruct_> {
    ...
    TwistedCloverArg(ColorSpinorField &out, const ColorSpinorField &in, const GaugeField &U,
                     const CloverField &A, double a, double b, bool xpay, const ColorSpinorField &x,
      ...
      A(A, false), A2inv(A, dynamic_clover ? false : true), // if dynamic clover we don't want the inverse field
      a(a), b(dagger ? -0.5*b : 0.5*b) // factor of 0.5 comes from basis transform
    { }
  };
