...
namespace quda
{
  ...
  struct QChargeArg : public ReduceArg<double3> {
    using Float = Float_;
    ...
    QChargeArg(const GaugeField &Fmunu, Float *qDensity = nullptr) :
      ReduceArg<double3>(), f(Fmunu), threads(Fmunu.VolumeCB()), qDensity(qDensity)
    {
    }
  };
...
#pragma unroll
      for (int i = 0; i < 6; i++) {
        ...
        if (i < 3)
          E.x -= getTrace(tmp * tmp).real(); // spatial
        else
          E.y -= getTrace(tmp * tmp).real(); // temporal
      }
      ...
      // unroll computation
#pragma unroll
      for (int i = 0; i < 3; i++) { Qi[i] = getTrace(F[i] * F[5 - i]).real(); }
