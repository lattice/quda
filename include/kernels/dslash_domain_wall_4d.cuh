...
namespace quda
{
  ...
  template <bool dagger, bool xpay, KernelType kernel_type, typename Arg> struct domainWall4D : dslash_default {
    ...
    const Arg &arg;
    template <typename Ftor> constexpr domainWall4D(const Ftor &ftor) : dslash_default {ftor.block_idx}, arg(ftor.arg)
    {
    }
    static constexpr const char *filename() { return KERNEL_FILE; } // this file name - used for run-time compilation

    template <KernelType mykernel_type = kernel_type>
    __device__ __host__ __forceinline__ void operator()(int idx, int src_s, int parity) const
    {
