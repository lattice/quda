...
namespace quda
{
  ...
  template <int Ls, int nParity, bool dagger, bool xpay, KernelType kernel_type, typename Arg>
    __global__ void alternative_dslash_kernel(Arg arg) {
      ...
      // static_assert(Arg::nDim == 5, "For 5d fermion only.");
      ...
      while (thread_idx < arg.Ls * K) {
        ...
        int d = dir % 4;
        bool fwd = dir < 4;
        int fwd_bwd = 1 - (d / 4) * 2;        // +1 for fwd, -1 for bwd
        ...
        bool ghost = fwd ? (coord[d] + arg.nFace >= arg.dim[d]) : (coord[d] - arg.nFace < 0);
        ...
        if (!ghost) {
          for (int spin = 0; spin < 4; spin++) {
            smem_obj_a(coord.s * 8 + spin * 2 + 0, color * 8 + dir) = projected_in(spin, 0).real(); // real
            smem_obj_a(coord.s * 8 + spin * 2 + 1, color * 8 + dir) = projected_in(spin, 1).imag(); // imag
          }
        } else {
          for (int spin = 0; spin < 4; spin++) {
            smem_obj_a(coord.s * 8 + spin * 2 + 0, color * 8 + dir) = 0; // real
            smem_obj_a(coord.s * 8 + spin * 2 + 1, color * 8 + dir) = 0; // imag
          }
        }
      ...
      for (int m = threadIdx.x; m < bM; m += blockDim.x) {
        for (int n = threadIdx.y; n < bN; n += blockDim.y) {
          smem_obj_c(m, n) = 0;
          for (int k = 0; k < bK; k++)
            smem_obj_c(m, n) += smem_obj_a(m, k) * smem_obj_b(k, n);
        }
      }
#else
#pragma unroll
...
#pragma unroll
      ...
      __shared__ unsigned reduce_buffer[Ls];
      ...
      while (thread_idx < Ls * 12) {
        ...
        atomicMax(&reduce_buffer[Ls], fabsf(p_real) > fabsf(p_imag) ? fabsf(p_real) : fabsf(p_imag));
        thread_idx += blockDim.x * blockDim.y;
      }
      ...
      while (thread_idx < Ls * 12) {
        ...
        fixed_point_t fixed_real = static_cast<fixed_point_t>(fp_real * fixedMaxValue<fixed_point_t>::value / reduce_buffer[s]);
        fixed_point_t fixed_imag = static_cast<fixed_point_t>(fp_imag * fixedMaxValue<fixed_point_t>::value / reduce_buffer[s]);

        arg.out(x_cb_4d + arg.dc.volume_4d_cb * s, parity, color, spin) = complex<fixed_point_t>(fixed_real, fixed_imag);
