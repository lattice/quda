...
namespace quda {
  ...
  struct CloverForceArg : kernel_param<> {
    ...
    CloverForceArg(GaugeField &force, const GaugeField &U, const ColorSpinorField &inA, const ColorSpinorField &inB,
      ...
      VolumeCB(inA.VolumeCB()),
      GhostFaceCB(inB.GhostFaceCB()[dim])
    {
  ...
  template <typename Arg> struct Exterior {
    ...
    __device__ __host__ inline void operator()(int x_cb)
    {
      ...
      int x[4];
      for (int flavor=0; flavor<=arg.doublet; ++flavor){
        const int flavor_offset_bulk_idx = flavor * (arg.VolumeCB/2);
        const int flavor_offset_ghost_idx = flavor * (arg.GhostFaceCB/2);
