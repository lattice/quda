...
namespace quda {
  ...
  struct SolverParam {
    ...
    */
     ...
     */
    void updateInvertParam(QudaInvertParam &param, int offset=-1, int rhs=-1) {
      ...
      if (offset >= 0) {
        if(offset == rhs){
          param.true_res_offset[rhs] = true_res;
          param.iter_res_offset[rhs] = iter;
          param.true_res_hq_offset[rhs] = true_res_hq;
        }
        else{
