...
__device__ inline void construct_matrix_a_generic(half *sm_a,
                                                  compute_type *generic) {
  ...
  while (x < Ls_) {
    ...
    float value =
        generic[x * Ls_ +
                threadIdx.y]; // Assuming the input matrix is row major
  ...
  while (x < Ls_) {
    ...
    if (mp && mm) {
      ...
      if (dagger) {
        ...
        factorR = inv * powf(k, __int2float_rn(exp)) *
                  (x > threadIdx.y ? -arg.m_f : 1.f);
      } else {
      ...
      if (dagger) {
        exp = x < threadIdx.y ? Ls_ - threadIdx.y + x : x - threadIdx.y;
        factorL = inv * powf(k, __int2float_rn(exp)) *
                  (x < threadIdx.y ? -arg.m_f : 1.f);
      } else {
        exp = x > threadIdx.y ? Ls_ - x + threadIdx.y : threadIdx.y - x;
        factorL = inv * powf(k, __int2float_rn(exp)) *
    ...
    A[(offset_k + 0) * (M_sm / 2) + (offset_m + 0)] =
        __floats2half2_rn(RpL, 0.0f);
    A[(offset_k + 0) * (M_sm / 2) + (offset_m + 1)] =
        __floats2half2_rn(RmL, 0.0f);

    A[(offset_k + 1) * (M_sm / 2) + (offset_m + 0)] =
        __floats2half2_rn(0.0f, RpL);
    A[(offset_k + 1) * (M_sm / 2) + (offset_m + 1)] =
        __floats2half2_rn(0.0f, RmL);

    A[(offset_k + 2) * (M_sm / 2) + (offset_m + 0)] =
        __floats2half2_rn(RmL, 0.0f);
    A[(offset_k + 2) * (M_sm / 2) + (offset_m + 1)] =
        __floats2half2_rn(RpL, 0.0f);

    A[(offset_k + 3) * (M_sm / 2) + (offset_m + 0)] =
        __floats2half2_rn(0.0f, RmL);
    A[(offset_k + 3) * (M_sm / 2) + (offset_m + 1)] =
        __floats2half2_rn(0.0f, RpL);
...
__device__ inline void construct_matrix_a_d5(Arg &arg, half *sm_a) {
  ...
  while (x < Ls_) {
    ...
    // exp = 0 means we are on the diagonal.
    float RpL = exp == 0 ? arg.alpha * (factorR + factorL) + b
                         : arg.alpha * (factorR + factorL);
...
__device__ inline void load_matrix_b_tex(Input &input, half2 *sm_b, int sid,
                                         const float scale) {
  ...
  sm_b[(threadIdx.y * 4 + 0) * N_sm_d2 + 3 * threadIdx.x + 0] =
      __floats2half2_rn(in_tex.x * f, in_tex.y * f);
  sm_b[(threadIdx.y * 4 + 0) * N_sm_d2 + 3 * threadIdx.x + 1] =
      __floats2half2_rn(in_tex.z * f, in_tex.w * f);

  in_tex = tex1Dfetch<float4>(input.tex, 1 * input.volumeCB + sid);
  sm_b[(threadIdx.y * 4 + 0) * N_sm_d2 + 3 * threadIdx.x + 2] =
      __floats2half2_rn(in_tex.x * f, in_tex.y * f);
  sm_b[(threadIdx.y * 4 + 1) * N_sm_d2 + 3 * threadIdx.x + 0] =
      __floats2half2_rn(in_tex.z * f, in_tex.w * f);

  in_tex = tex1Dfetch<float4>(input.tex, 2 * input.volumeCB + sid);
  sm_b[(threadIdx.y * 4 + 1) * N_sm_d2 + 3 * threadIdx.x + 1] =
      __floats2half2_rn(in_tex.x * f, in_tex.y * f);
  sm_b[(threadIdx.y * 4 + 1) * N_sm_d2 + 3 * threadIdx.x + 2] =
      __floats2half2_rn(in_tex.z * f, in_tex.w * f);

  in_tex = tex1Dfetch<float4>(input.tex, 3 * input.volumeCB + sid);
  sm_b[(threadIdx.y * 4 + 2) * N_sm_d2 + 3 * threadIdx.x + 0] =
      __floats2half2_rn(in_tex.x * f, in_tex.y * f);
  sm_b[(threadIdx.y * 4 + 2) * N_sm_d2 + 3 * threadIdx.x + 1] =
      __floats2half2_rn(in_tex.z * f, in_tex.w * f);

  in_tex = tex1Dfetch<float4>(input.tex, 4 * input.volumeCB + sid);
  sm_b[(threadIdx.y * 4 + 2) * N_sm_d2 + 3 * threadIdx.x + 2] =
      __floats2half2_rn(in_tex.x * f, in_tex.y * f);
  sm_b[(threadIdx.y * 4 + 3) * N_sm_d2 + 3 * threadIdx.x + 0] =
      __floats2half2_rn(in_tex.z * f, in_tex.w * f);

  in_tex = tex1Dfetch<float4>(input.tex, 5 * input.volumeCB + sid);
  sm_b[(threadIdx.y * 4 + 3) * N_sm_d2 + 3 * threadIdx.x + 1] =
      __floats2half2_rn(in_tex.x * f, in_tex.y * f);
  sm_b[(threadIdx.y * 4 + 3) * N_sm_d2 + 3 * threadIdx.x + 2] =
      __floats2half2_rn(in_tex.z * f, in_tex.w * f);
}

template <class integer_vec>
__device__ inline integer_vec __2half22integer4_rn(const half2 &a,
                                                   const half2 &b) {
  integer_vec c;
...
__device__ inline void __half_max_abs_half2__(half &max, const half2 &input) {
  ...
  static constexpr uint32_t maximum_mask =
      0x7fff7fffu; // 0111 1111 1111 1111 0111 1111 1111 1111

  uint32_t input_masked =
      *reinterpret_cast<const uint32_t *>(&input) & maximum_mask;
...
__inline__ __device__ void __float_max_abs_floats__(float &max,
                                                    const float &input) {
  static constexpr uint32_t maximum_mask =
...
__device__ inline void block_wise_reduce_vector(const Vector &v,
                                                float *smem_scale) {
...
#pragma unroll
  ...
  __syncthreads();
  if (warp_id == 0) {
    warp_max = (lane_id < ((blockDim.x * blockDim.y) >> 5))
                   ? smem_scale[lane_id]
                   : 0.0f;
    warp_wise_reduce_float(warp_max);
    if (lane_id == 0) {
...
__device__ inline void load_matrix_b_vector(Vector &v, half2 *sm_b,
                                            float *smem_scale,
                                            float m_scale = 1.0f) {
  if (accumulate) {
...
#pragma unroll
      for (int color = 0; color < 3; color++) {
        int idx = (threadIdx.y * 4 + spin) * (N_sm_d2 * 2) + 6 * threadIdx.x +
                  2 * color;
        v(spin, color) +=
            complex<float>(__half2float(B[idx + 0]), __half2float(B[idx + 1])) *
            previous_scale;
      }
    }
  }
...
__device__ inline void store_matrix_c(Output &output, half2 *sm_b, int sid,
                                      const float scale) {
  half max_ = 0.0f;
  constexpr int N_sm_d2 = N_sm / 2;
#pragma unroll
...
#pragma unroll
  ...
  const half2 max_i_div_max2_ =
      __half2half2(__hdiv(fixedMaxValue<storage_type>::value, max_));

  typedef typename VectorType<storage_type, 4>::type storage_vec;
  storage_vec *out = reinterpret_cast<storage_vec *>(output.field);
  half2 a, b;

  a = __hmul2(sm_b[(threadIdx.y * 4 + 0) * N_sm_d2 + 3 * threadIdx.x + 0],
              max_i_div_max2_);
  b = __hmul2(sm_b[(threadIdx.y * 4 + 0) * N_sm_d2 + 3 * threadIdx.x + 1],
              max_i_div_max2_);
  out[sid + 0 * output.volumeCB] = __2half22integer4_rn<storage_vec>(a, b);

  a = __hmul2(sm_b[(threadIdx.y * 4 + 0) * N_sm_d2 + 3 * threadIdx.x + 2],
              max_i_div_max2_);
  b = __hmul2(sm_b[(threadIdx.y * 4 + 1) * N_sm_d2 + 3 * threadIdx.x + 0],
              max_i_div_max2_);
  out[sid + 1 * output.volumeCB] = __2half22integer4_rn<storage_vec>(a, b);

  a = __hmul2(sm_b[(threadIdx.y * 4 + 1) * N_sm_d2 + 3 * threadIdx.x + 1],
              max_i_div_max2_);
  b = __hmul2(sm_b[(threadIdx.y * 4 + 1) * N_sm_d2 + 3 * threadIdx.x + 2],
              max_i_div_max2_);
  out[sid + 2 * output.volumeCB] = __2half22integer4_rn<storage_vec>(a, b);

  a = __hmul2(sm_b[(threadIdx.y * 4 + 2) * N_sm_d2 + 3 * threadIdx.x + 0],
              max_i_div_max2_);
  b = __hmul2(sm_b[(threadIdx.y * 4 + 2) * N_sm_d2 + 3 * threadIdx.x + 1],
              max_i_div_max2_);
  out[sid + 3 * output.volumeCB] = __2half22integer4_rn<storage_vec>(a, b);

  a = __hmul2(sm_b[(threadIdx.y * 4 + 2) * N_sm_d2 + 3 * threadIdx.x + 2],
              max_i_div_max2_);
  b = __hmul2(sm_b[(threadIdx.y * 4 + 3) * N_sm_d2 + 3 * threadIdx.x + 0],
              max_i_div_max2_);
  out[sid + 4 * output.volumeCB] = __2half22integer4_rn<storage_vec>(a, b);

  a = __hmul2(sm_b[(threadIdx.y * 4 + 3) * N_sm_d2 + 3 * threadIdx.x + 1],
              max_i_div_max2_);
  b = __hmul2(sm_b[(threadIdx.y * 4 + 3) * N_sm_d2 + 3 * threadIdx.x + 2],
              max_i_div_max2_);
...
template <int block_dim_x, int Ls_, int M, int N, int M_sm, int N_sm,
          bool reload, class T>
__device__ inline void wmma_gemm(T *a_frag, half *sm_a, half *sm_b,
                                 half *sm_c) {
  ...
  static_assert((tm_dim * tn_dim) % total_warp == 0,
                "(tm_dim*tn_dim)%%total_warp==0\n");
  static_assert(tn_dim % (tm_dim * tn_dim / total_warp) == 0,
                "tn_dim%%(tm_dim*tn_dim/total_warp)==0\n");
...
#pragma unroll
    for (int k = 0; k < tm_dim; k++) {
      ...
      // Load Matrix
      if (reload) {
        nvcuda::wmma::load_matrix_sync(a_frag[0], sm_a + a_row + a_col * M_sm,
                                       M_sm);
      }
    ...
    nvcuda::wmma::store_matrix_sync(sm_c + c_col + c_row * N_sm, c_frag, N_sm,
