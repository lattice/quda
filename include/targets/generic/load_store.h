...
namespace quda
{
  ...
  */
  ...
  template <bool is_device> struct vector_load_async_impl {
    template <typename T, class Pipe> __device__ __host__ inline void operator()(T *out, const T *ptr, int idx, Pipe &pipe)
    {
  ...
  __device__ __host__ inline void vector_load_async(void *out, const void *ptr, int idx, pipe_t &pipe)
  {
    target::dispatch<vector_load_async_impl>(reinterpret_cast<VectorType *>(out), reinterpret_cast<const VectorType *>(ptr), idx, pipe);
  }
