  ...
  namespace target
  {
    namespace rng
    {
      ...
      inline void matvecmod(Trans a, State &v, uint64_t m)
      {
        ...
        v[0]
          = (uint32_t)((((uint64_t)a[0][0] * v0) % m + ((uint64_t)a[0][1] * v1) % m + ((uint64_t)a[0][2] * v2) % m) % m);
        v[1]
          = (uint32_t)((((uint64_t)a[1][0] * v0) % m + ((uint64_t)a[1][1] * v1) % m + ((uint64_t)a[1][2] * v2) % m) % m);
        v[2]
          = (uint32_t)((((uint64_t)a[2][0] * v0) % m + ((uint64_t)a[2][1] * v1) % m + ((uint64_t)a[2][2] * v2) % m) % m);
      }
      ...
      constexpr Trans a1 {State {0u, 1u, 0u}, State {0u, 0u, 1u}, State {(uint32_t)(m1 - a13n), (uint32_t)a12, 0u}};
      constexpr Trans a2 {State {0u, 1u, 0u}, State {0u, 0u, 1u}, State {(uint32_t)(m2 - a23n), 0u, (uint32_t)a21}};
