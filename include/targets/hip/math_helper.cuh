...
namespace quda {
 ...
 /**
  ...
  template <bool is_device> struct sincos_impl {
    inline void operator()(const double& a, double *s, double *c) { *s=::sin(a); *c=::cos(a); }
  };
  ...
  template<>
  inline __host__ __device__ void sincos(const double& a, double * s, double *c) { target::dispatch<sincos_impl>(a, s, c); }  
  ...
  template<> struct rsqrtf_impl<true> { 
   ...
   template<> struct rsqrt_impl<true> {
    __device__ inline double operator()(const double& a) {
	    return ::rsqrt(a);
    }
   };
  ...
  template<>
  inline __host__ __device__ double rsqrt(const double& a) { return target::dispatch<rsqrt_impl>(a); }
  ...
  template <> struct sinf_impl<true> { __device__ inline float operator()(const float& a) { return ::sinf(a); } };

  template <bool is_device> struct cosf_impl { inline float operator()(const float& a) { return ::cosf(a); } };
  template <> struct cosf_impl<true> { __device__ inline float operator()(const float& a) { return ::cosf(a); } };
 

  template <bool is_device> struct sin_impl { inline double operator()(const double& a) { return ::sin(a); } };
  template <> struct sin_impl<true> { __device__ inline double operator()(const double& a) { return ::sin(a); } };

  template <bool is_device> struct cos_impl { inline double operator()(const double& a) { return ::cos(a); } };
  template <> struct cos_impl<true> { __device__ inline double operator()(const double& a) { return ::cos(a); } };
...
template <>
  ...
  template <>
    ...
    __device__ __host__ static void SinCos(const double &a, double *s, double *c) { target::dispatch<sincos_impl>(a,s,c); }
  };

template <>
    struct Trig<true,double> {
    __device__ __host__ static double Atan2( const double &a, const double &b) { return ::atan2(a,b)/M_PI; }
    __device__ __host__ static double Sin(const double &a) { return target::dispatch<sin_impl>(a*static_cast<double>(M_PI)); }
    __device__ __host__ static double Cos(const double &a) { return target::dispatch<cos_impl>(a*static_cast<double>(M_PI)); }
