...
namespace quda
{
  ...
  template <> struct vector_load_impl<true> {
    ...
    __device__ inline void operator()(float4 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
      load_cached_float4<prefetch_size>(value, reinterpret_cast<const float4 *>(ptr) + idx);
    }

    template <size_t prefetch_size>
    __device__ inline void operator()(float2 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
      load_cached_float2<prefetch_size>(value, reinterpret_cast<const float2 *>(ptr) + idx);
    }

    template <size_t prefetch_size>
    __device__ inline void operator()(float &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
    ...
    template <typename T, size_t prefetch_size>
    __device__ inline void operator()(double4 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
      load_cached_double4<prefetch_size>(value, reinterpret_cast<const double4 *>(ptr) + idx);
    }

    template <typename T, size_t prefetch_size>
    __device__ inline void operator()(float8 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
    ...
    template <size_t prefetch_size>
    __device__ inline void operator()(double2 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
      load_cached_double2<prefetch_size>(value, reinterpret_cast<const double2 *>(ptr) + idx);
    }

    template <size_t prefetch_size>
    __device__ inline void operator()(short2 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
      load_cached_short2<prefetch_size>(value, reinterpret_cast<const short2 *>(ptr) + idx);      
    }

    template <size_t prefetch_size>
    __device__ inline void operator()(short4 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &)
    {
      load_cached_short4<prefetch_size>(value, reinterpret_cast<const short4 *>(ptr) + idx);      
    }

    template <size_t prefetch_size>
    __device__ inline void operator()(short8 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &prefetch)
    {
      float4 tmp;
    ...
    template <size_t prefetch_size>
    __device__ inline void operator()(char8 &value, const void *ptr, index_t idx, const prefetch_t<prefetch_size> &prefetch)
    {
      float2 tmp;
      operator()(tmp, ptr, idx, prefetch);
  ...
  template <> struct vector_store_impl<true> {
...
#if __COMPUTE_CAPABILITY__ >= 1000
    __device__ inline void operator()(void *ptr, index_t idx, const double4 &value)
    {
      store_streaming_double4(reinterpret_cast<double4 *>(ptr) + idx, value.x, value.y, value.z, value.w);
    }

    __device__ inline void operator()(void *ptr, index_t idx, const float8 &value)
    {
      store_streaming_float8(reinterpret_cast<float8 *>(ptr) + idx, value);
    }
#endif

    __device__ inline void operator()(void *ptr, index_t idx, const double2 &value)
    {
      store_streaming_double2(reinterpret_cast<double2 *>(ptr) + idx, value.x, value.y);
    }

    __device__ inline void operator()(void *ptr, index_t idx, const float4 &value)
    {
      store_streaming_float4(reinterpret_cast<float4 *>(ptr) + idx, value.x, value.y, value.z, value.w);
    }

    __device__ inline void operator()(void *ptr, index_t idx, const float2 &value)
    {
      store_streaming_float2(reinterpret_cast<float2 *>(ptr) + idx, value.x, value.y);
    }

    __device__ inline void operator()(void *ptr, index_t idx, const short4 &value)
    {
    ...
    __device__ inline void operator()(void *ptr, index_t idx, const short2 &value)
    {
