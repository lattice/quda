...
namespace quda {
  ...
  protected:
    ...
    */
	...
	//qudaLaunchKernel(BlockKernel2D<Block::block[idx], Transformer, Arg>, tp, stream, arg);
	launchBlockKernel2D<Block::block[idx], Transformer, Arg>(tp, stream, arg);
      }
...
#ifdef JITIFY
      launch_error = launch_jitify_block<Transformer>("BlockKernel2D", tp, stream, arg);
#else
