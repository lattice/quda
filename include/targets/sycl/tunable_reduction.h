    ...
    */
      ...
      launch(Arg &arg, const TuneParam &tp, const qudaStream_t &stream)
    {
      if (tp.block.x == block_size_x)
        ...
        return launchReduction2D<block_size_x, block_size_y, Transformer, Arg>(tp, stream, arg);
      else
    ...
    */
      ...
      launch(Arg &arg, const TuneParam &tp, const qudaStream_t &stream)
    {
      if (tp.block.x == block_size_x)
        ...
        return launchMultiReduction<block_size_x, block_size_y, Transformer, Arg>(tp, stream, arg);
      else
        return launch<block_size_x - device::warp_size(), Transformer>(arg, tp, stream);
    }
