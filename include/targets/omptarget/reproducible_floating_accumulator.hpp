...
//using std::max;
//#else
// OpenMP: use standard array storage.
#include <array>
...
template <class ftype> struct RFA_bins
{
  ...
  static_assert(std::is_same_v<ftype, double> || std::is_same_v<ftype, float>);
  static_assert(MAXINDEX > 0);
...
// OpenMP: share one device table across translation units.
#pragma omp declare target
extern RFA_bins<reduction_t> bin_device_buffer;
#pragma omp end declare target

/** @brief Initialize the OpenMP bin table for the current device. */
void init_rfa_bins();

template <bool is_device> struct rfa_bins_impl {
  const reduction_t *operator()(int x) { return &reducer::get_rfa_bins()[x]; }
};
template <> struct rfa_bins_impl<true> {
  const reduction_t *operator()(int x) { return &bin_device_buffer[x]; }
};
...
private:
  ...
  static constexpr int MANT_DIG = std::numeric_limits<ftype>::digits;
  ///Binned floating-point maximum index
  ...
  {
    // OpenMP: select the device table without calling the host initializer.
    return target::dispatch<rfa_bins_impl>(x);
    //return &bins[x];
  }
  ...
  static constexpr auto maxindex() { return MAXINDEX; }
...
#pragma unroll
 ...
 public:
  ...
  ReproducibleFloatingAccumulator(ReproducibleFloatingAccumulator &&) = default;
  ...
  ReproducibleFloatingAccumulator& operator=(ReproducibleFloatingAccumulator &&) = default;
  ...
  __host__ __device__ inline explicit operator double() const { return double(conv()); }
  __host__ __device__ inline explicit operator float() const { return float(conv()); }
...
#ifdef QUDA_TARGET_OMPTARGET // OpenMP vector types.
  ...
  namespace reducer
  {
    ...
    {
      // OpenMP: initialize once per device.
      reproducible::init_rfa_bins();
    }
  } // namespace reducer
  ...
  __device__ __host__ inline array<rfa_t<T>, n> operator+(const array<rfa_t<T>, n> &a, const array<rfa_t<T>, n> &b)
  {
    array<rfa_t<T>, n> c;
#pragma unroll
