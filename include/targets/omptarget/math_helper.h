...
namespace quda
{
  ...
  template <typename T> inline T sinpi(const T a) { return ::sin(a * static_cast<T>(M_PI)); }
  template <typename T> inline T cospi(const T a) { return ::cos(a * static_cast<T>(M_PI)); }
