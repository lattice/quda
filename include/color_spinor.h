...
#pragma unroll
    ...
    template <typename T> __device__ __host__ inline ColorSpinor<T, Nc, 4> convert_and_scale(const T &a)
    {
      ColorSpinor<T, Nc, 4> output;
#pragma unroll
      for (int i = 0; i < size; i++) { output.data[i] = static_cast<complex<T>>(data[i]) * a; }
      return output;
    }
