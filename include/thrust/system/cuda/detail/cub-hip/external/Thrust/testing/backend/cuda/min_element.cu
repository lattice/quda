...
void TestMinElementDevice(ExecutionPolicy exec)
{
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(min_element_kernel), dim3(1), dim3(1), 0, 0, exec, d_data.begin(), d_data.end(), d_result.begin());
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(min_element_kernel), dim3(1), dim3(1), 0, 0, exec, d_data.begin(), d_data.end(), thrust::greater<int>(), d_result.begin());
