  ...
  */
  ...
  namespace colorspinor
  {
    ...
    template <typename Float, int nSpin, int nColor, int nVec, QudaFieldOrder order> struct AccessorCB {
      ...
      constexpr index_t index(int, int, int, int, int, index_t) const { return 0; }
    };
    ...
    template <typename Float, int nSpin, int nColor, int nVec, QudaFieldOrder order> struct GhostAccessorCB {
      ...
      constexpr index_t index(int, int, int, int, int, int, int) const { return 0; }
    };
    ...
    struct AccessorCB<Float, nSpin, nColor, nVec, QUDA_SPACE_SPIN_COLOR_FIELD_ORDER> {
       ...
       */
      constexpr index_t index(int parity, int x_cb, int s, int c, int v, index_t) const
      {
      ...
      __device__ __host__ inline void load(complex<Float> out[nSpinBlock * nColor * nVec], complex<Float> *in,
                                           int parity, int x_cb, int chi, index_t) const
      {
        constexpr int N = nSpin * nColor * nVec;
        constexpr int M = nSpinBlock * nColor * nVec;
#pragma unroll
        for (int i = 0; i < M; i++) {
          auto tmp = vector_load<Float, 2>(in + static_cast<index_t>(parity) * offset_cb,
                                           static_cast<index_t>(x_cb) * N + chi * M + i);
    ...
    struct GhostAccessorCB<Float, nSpin, nColor, nVec, QUDA_SPACE_SPIN_COLOR_FIELD_ORDER> {
      ...
      index_t ghostOffset[4] = {};
      GhostAccessorCB(const ColorSpinorField &a, int nFace = 1)
      {
        for (int d = 0; d < 4; d++) {
          faceVolumeCB[d] = static_cast<index_t>(a.SurfaceCB(d)) * nFace;
      ...
      constexpr index_t index(int dim, int parity, int x_cb, int s, int c, int v) const
      {
        return static_cast<index_t>(parity) * ghostOffset[dim]
          + ((static_cast<index_t>(x_cb) * nSpin + s) * nColor + c) * nVec + v;
      }
    };

    template <typename Index, int nSpin, int nColor, int nVec, int N> // note this will not work for N=1
    constexpr Index indexFloatN(int x_cb, int s, int c, int v, Index stride)
    {
...
#pragma unroll
    ...
    struct AccessorCB<Float, nSpin, nColor, nVec, QUDA_NATIVE_FIELD_ORDER> {
      ...
      constexpr index_t index(int parity, int x_cb, int s, int c, int v, index_t stride) const
      {
        return static_cast<index_t>(parity) * offset_cb
      ...
      template <int nSpinBlock>
      __device__ __host__ inline void load(complex<Float> out[nSpinBlock * nColor * nVec], complex<Float> *in,
                                           int parity, int x_cb, int chi, index_t stride) const
      {
...
#pragma unroll
    ...
    template <> struct AccessorCB<short, 1, 3, 1, QUDA_NATIVE_FIELD_ORDER> {
      ...
      template <int nSpinBlock>
      __device__ __host__ inline void load(complex<short> out[3], complex<short> *in, int parity, int x_cb, int,
                                           index_t) const
      {
        auto tmp = vector_load<float, 4>(in + static_cast<index_t>(parity) * offset_cb, x_cb);
    ...
    struct GhostAccessorCB<Float, nSpin, nColor, nVec, QUDA_NATIVE_FIELD_ORDER> {
      ...
      {
        return static_cast<index_t>(parity) * ghostOffset[dim]
          + indexFloatN<index_t, nSpin, nColor, nVec, N>(x_cb, s, c, v, faceVolumeCB[dim]);
      }
    };
    ...
    */
    ...
    public:
      ...
      const index_t norm_idx;                                         /**< Index into norm field */
      ...
      */
      __device__ __host__ inline fieldorder_wrapper(complex<storeFloat> *v, index_t idx, Float scale, Float scale_inv,
                                                    norm_t *norm = nullptr, index_t norm_idx = 0,
                                                    bool norm_write = false) :
      ...
      /**
         ...
         @param[in] a Complex number we want to store in this accessor
      */
      template <typename theirFloat> __device__ __host__ inline void operator=(const complex<theirFloat> &a) const
      {
        ...
        if (block_float && norm_write) norm[norm_idx] = static_cast<norm_t>(scale_inv);
        if constexpr (fixed) {
          v[idx] = complex<storeFloat>(f2i_round<storeFloat>(scale * a.real()), f2i_round<storeFloat>(scale * a.imag()));
        } else {
          v[idx] = complex<storeFloat>(store_cast<storeFloat>(a.real()), store_cast<storeFloat>(a.imag()));
        }
      }
      ...
      template <typename theirFloat> __device__ __host__ inline void operator=(const theirFloat &a) const
      {
        if constexpr (fixed) {
          *this = complex<Float>(static_cast<Float>(a), static_cast<Float>(0.0));
          ...
          v[idx] = complex<storeFloat>(store_cast<storeFloat>(a), storeFloat(0));
        }
      }
...
#pragma unroll
                for (int vec = 0; vec < nVec; vec++) {
                  int k = (s * nColor + c) * nVec + vec;
                  out[k] = complex<Float>(static_cast<Float>(tmp[k].real()), static_cast<Float>(tmp[k].imag()));
                }
              }
            }
          }
          ...
          if constexpr (fixed && block_float && nColor == 3 && nSpin == 1 && nVec == 1) {
            ...
            if constexpr (fixed) {
              if constexpr (block_float) {
                norm_ = v.norm[static_cast<index_t>(parity) * v.norm_offset + x_cb];
              } else {
                norm_ = v.scale_inv;
              }
            }
...
#pragma unroll
      ...
      {
        Float scale = Float(1);
        Float scale_inv = Float(1);
        norm_t *norm = nullptr;
        index_t norm_offset = 0;
        if constexpr (fixed) {
          if constexpr (fixed && block_float && nColor == 3 && nSpin == 1 && nVec == 1) {
            norm = v.norm;
            norm_offset = static_cast<index_t>(parity) * v.norm_offset + static_cast<index_t>(4) * x_cb + 3;
          } else if constexpr (block_float) {
            norm = v.norm;
            norm_offset = static_cast<index_t>(parity) * v.norm_offset + x_cb;
          } else {
            scale = v.scale;
      ...
      constexpr index_t VolumeCB() const { return volumeCB; }
    ...
    struct GhostNOrder<Float, Ns, Nc, spin_project, false> {
      ...
      {
        for (int i = 0; i < 4; i++) { faceVolumeCB[i] = static_cast<index_t>(a.SurfaceCB(i)) * nFace; }
      ...
      {
        real v[length_ghost];
        const index_t parity_off = static_cast<index_t>(parity) * faceVolumeCB[dim];
        norm_type nrm = isFixed<Float>::value ? vector_load<float, 1>(ghost_norm[2 * dim + dir], parity_off + x)[0] : 0.0;
...
#pragma unroll
      ...
      /**
         @brief Store a ghost color-spinor
         @tparam In Register scalar type of the incoming spinor
         @param[in] in Site spinor to store
         @param[in] x Checkerboarded ghost index
         @param[in] dim Ghost dimension
         @param[in] dir Ghost direction
         @param[in] parity Parity
       */
      template <typename In>
      __device__ __host__ inline void saveGhost(const quda::complex<In> in[length_ghost / 2], int x, int dim, int dir,
                                                int parity = 0) const
      {
        real v[length_ghost];
#pragma unroll
        for (int i = 0; i < length_ghost / 2; i++) {
          v[2 * i + 0] = store_cast<real>(in[i].real());
          v[2 * i + 1] = store_cast<real>(in[i].imag());
        }
...
#pragma unroll
          for (int i = 0; i < length_ghost / 2; i++)
            max_[i] = max((norm_type)abs((norm_type)v[i]), (norm_type)abs((norm_type)v[i + length_ghost / 2]));
#pragma unroll
          for (int i = 0; i < length_ghost / 2; i++) scale = max(max_[i], scale);
          ghost_norm[2 * dim + dir][static_cast<index_t>(parity) * faceVolumeCB[dim] + x]
            = scale * fixedInvMaxValue<Float>::value;
...
#pragma unroll
       ...
       */
      template <typename In>
      __device__ __host__ inline void save(const quda::complex<In> in[length / 2], int x, int parity = 0) const
      {
        real v[length];
        auto norm_offset = (volumeCB * 2 * Nc * Ns + parity * offset) * sizeof(Float) / sizeof(norm_t);
...
#pragma unroll
        ...
        if constexpr (isFixed<Float>::value) {
          ...
          // two-pass to increase ILP (assumes length divisible by two, e.g. complex-valued)
#pragma unroll
          for (int i = 0; i < length / 2; i++) max_[i] = max(abs((norm_t)v[i]), abs((norm_t)v[i + length / 2]));
#pragma unroll
...
#pragma unroll
    ...
    template <bool spin_project> struct GhostNOrder<short, 1, 3, spin_project, false> {
      ...
      {
        real v[length_ghost];
        auto vecTmp = vector_load<Float, 8>(ghost[2 * dim + dir], static_cast<index_t>(parity) * faceVolumeCB[dim] + x);
...
#pragma unroll
        ...
        vector_store(ghost[2 * dim + dir], static_cast<index_t>(parity) * faceVolumeCB[dim] + x, vecTmp);
      }
...
#pragma unroll
    ...
    template <typename Float, int Ns, int Nc> struct SpaceColorSpinorOrder {
        ...
        nParity(a.SiteSubset())
      {
        for (int i = 0; i < 4; i++) {
          ...
          faceVolumeCB[i] = static_cast<index_t>(a.SurfaceCB(i)) * nFace;
        }
      }
      ...
      template <typename In>
      __device__ __host__ inline void save(const quda::complex<In> v[length / 2], int x, int parity = 0) const
      {
        auto out = &field[(parity * volumeCB + x) * length];
        quda::complex<In> v_[length / 2];
        for (int s = 0; s < Ns; s++) {
      ...
      */
      ...
      __device__ __host__ inline void loadGhost(complex v[length / 2], int x, int dim, int dir, int parity = 0) const
      {
        for (int s = 0; s < Ns; s++) {
          for (int c = 0; c < Nc; c++) {
            v[s * Nc + c]
              = complex(static_cast<real>(ghost[2 * dim + dir][(((parity * faceVolumeCB[dim] + x) * Nc + c) * Ns + s) * 2 + 0]),
                        static_cast<real>(ghost[2 * dim + dir][(((parity * faceVolumeCB[dim] + x) * Nc + c) * Ns + s) * 2 + 1]));
          }
        }
      }

      template <typename In>
      __device__ __host__ inline void saveGhost(const quda::complex<In> v[length / 2], int x, int dim, int dir, int parity = 0) const
      {
        for (int s = 0; s < Ns; s++) {
          for (int c = 0; c < Nc; c++) {
            ghost[2 * dim + dir][(((parity * faceVolumeCB[dim] + x) * Nc + c) * Ns + s) * 2 + 0]
              = store_cast<Float>(v[s * Nc + c].real());
            ghost[2 * dim + dir][(((parity * faceVolumeCB[dim] + x) * Nc + c) * Ns + s) * 2 + 1]
              = store_cast<Float>(v[s * Nc + c].imag());
          }
        }
      }
      ...
      */
      ...
      __device__ __host__ inline void loadGhost(complex v[length / 2], int x, int dim, int dir, int parity = 0) const
      {
        for (int s = 0; s < Ns; s++) {
          for (int c = 0; c < Nc; c++) {
            v[s * Nc + c]
              = complex(static_cast<real>(ghost[2 * dim + dir][(((parity * faceVolumeCB[dim] + x) * Ns + s) * Nc + c) * 2 + 0]),
                        static_cast<real>(ghost[2 * dim + dir][(((parity * faceVolumeCB[dim] + x) * Ns + s) * Nc + c) * 2 + 1]));
          }
        }
      }
      ...
      __device__ __host__ inline void saveGhost(const quda::complex<In> v[length / 2], int x, int dim, int dir, int parity = 0) const
      {
        for (int s = 0; s < Ns; s++) {
          for (int c = 0; c < Nc; c++) {
            ghost[2 * dim + dir][(((parity * faceVolumeCB[dim] + x) * Ns + s) * Nc + c) * 2 + 0]
              = store_cast<Float>(v[s * Nc + c].real());
    ...
    template <typename Float, int Ns, int Nc> struct QDPJITDiracOrder {
      ...
      __device__ __host__ inline void load(complex v[Ns * Nc], int x, int parity = 0) const
      {
        for (int s = 0; s < Ns; s++) {
          for (int c = 0; c < Nc; c++) {
            v[s * Nc + c] = complex(static_cast<real>(field[(((0 * Nc + c) * Ns + s) * 2 + parity) * volumeCB + x]),
                                    static_cast<real>(field[(((1 * Nc + c) * Ns + s) * 2 + parity) * volumeCB + x]));
          }
        }
      }

      template <typename In>
      __device__ __host__ inline void save(const quda::complex<In> v[Ns * Nc], int x, int parity = 0) const
      {
        for (int s = 0; s < Ns; s++) {
          for (int c = 0; c < Nc; c++) {
            field[(((0 * Nc + c) * Ns + s) * 2 + parity) * volumeCB + x] = store_cast<Float>(v[s * Nc + c].real());
            field[(((1 * Nc + c) * Ns + s) * 2 + parity) * volumeCB + x] = store_cast<Float>(v[s * Nc + c].imag());
          }
        }
      }
    ...
    template <typename Float, int Ns, int Nc> struct OpenQCDDiracOrder {
      ...
      template <typename In>
      __device__ __host__ inline void save(const quda::complex<In> v[length / 2], int x_cb, int parity = 0) const
      {
