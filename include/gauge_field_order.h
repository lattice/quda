    ...
    */
    ...
    struct Accessor<Float, nColor, QUDA_MILC_GAUGE_ORDER, storeFloat> {
                                                 ...
                                                 const complex<theirFloat> &val) const
      {
        using vec2 = array<storeFloat, 2>;
        vec2 *u2
          = reinterpret_cast<vec2 *>(u + (((parity * volumeCB + x_cb) * geometry + dim) * nColor + row) * nColor + col);
    ...
    struct Accessor<Float, nColor, QUDA_FLOAT2_GAUGE_ORDER, storeFloat> {
      ...
      {
        using vec2 = array<storeFloat, 2>;
        vec2 *u2 = reinterpret_cast<vec2 *>(u + parity * offset_cb + dim * stride * nColor * nColor
                                            + (row * nColor + col) * stride + x_cb);
    ...
    struct FieldOrder {
       ...
       * @param x 1-d site index
       */
      __device__ __host__ auto wrap_ghost(int d, int parity, int x) const { return ghostAccessor(d, parity, x, 0, 0); }
       ...
       */
      __device__ __host__ inline auto operator()(int d, int parity, int x, int s_row, int s_col, int c_row, int c_col) const
      {
      ...
      {
        commGlobalReductionPush(global);
        double nrm2 = accessor.template transform_reduce<plus<double>>(location, dim,
                                                                       square_<double, storeFloat>(accessor.scale_inv));
        commGlobalReductionPop();
        return nrm2;
      }
...
#pragma unroll
      ...
      template <typename I>
      __device__ __host__ inline void Unpack(complex out[9], const real in[10], int, int, real, const I *, const int *) const
      {
...
#pragma unroll
      ...
      template <typename I>
      __device__ __host__ inline void Unpack(complex out[9], const real in[8], int idx, int dir, real phase, const I *X,
                                             const int *R) const
      {
...
#pragma unroll
    ...
    struct FloatNOrder {
      using Accessor = FloatNOrder<Float, length_, N, reconLenParam, stag_phase, huge_alloc, ghostExchange_, use_inphase>;
...
#undef FLOATN
  ...
  template <typename T, int Nc> struct gauge_order_mapper<T, QUDA_QDP_GAUGE_ORDER, Nc> {
    typedef gauge::QDPOrder<T, 2 * Nc * Nc> type;
  };
  template <typename T, int Nc> struct gauge_order_mapper<T, QUDA_QDPJIT_GAUGE_ORDER, Nc> {
    typedef gauge::QDPJITOrder<T, 2 * Nc * Nc> type;
  };
  template <typename T, int Nc> struct gauge_order_mapper<T, QUDA_MILC_GAUGE_ORDER, Nc> {
    typedef gauge::MILCOrder<T, 2 * Nc * Nc> type;
  };
  ...
  template <typename T, int Nc> struct gauge_order_mapper<T, QUDA_BQCD_GAUGE_ORDER, Nc> {
    typedef gauge::BQCDOrder<T, 2 * Nc * Nc> type;
  };
  template <typename T, int Nc> struct gauge_order_mapper<T, QUDA_TIFR_GAUGE_ORDER, Nc> {
    typedef gauge::TIFROrder<T, 2 * Nc * Nc> type;
  };
  ...
  template <typename T, int Nc> struct gauge_order_mapper<T, QUDA_FLOAT2_GAUGE_ORDER, Nc> {
    typedef gauge::FloatNOrder<T, 2 * Nc * Nc, 2, 2 * Nc * Nc> type;
  };
