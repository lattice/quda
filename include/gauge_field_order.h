...
#pragma unroll
        ...
        unpack(v, vecTmp, x, dir, parity, phase);
      }

      __device__ __host__ inline void unpack(complex v[length / 2], Vector vecTmp[], int x, int dir, int parity,
                                             real phase) const
      {
        const int M = reconLen / N;
        real tmp[reconLen];
...
#pragma unroll
	 ...
	 this field at the above coordinates.
      ...
      __device__ __host__ inline void cache(Cache &cache, int stage, Pipe &pipe, int dim, int x_cb, int parity,
...
#pragma unroll
        for (int i = 0; i < M; i++) {
          // first load from memory
          vector_load_async<Vector>(cache.template gauge<Vector>(i, stage), gauge,
                                    parity * offset + (dim * M + i) * stride + x_cb, pipe);
        }
      }
