...
#pragma unroll
		for(int j=0; j<nreduce; j++) {
			aggregate[j] = (do_sum ? BlockReduce(cub_tmp).Sum(in[j]) : BlockReduce(cub_tmp).Reduce(in[j], r)); // BlockReduce(cub_tmp).Sum(in[j]);
			__syncthreads();
		}
...
#pragma unroll
			for(int j=0; j<nreduce; j++) {
				...
				sum[j] = (do_sum ? BlockReduce(cub_tmp).Sum(sum[j]) : BlockReduce(cub_tmp).Reduce(sum[j],r));
				__syncthreads();
			}
...
#pragma unroll
	...
	template <int nreduce, int block_size, typename T, bool do_sum = true, typename Reducer = cub::Sum>
  __device__ inline void array_reduce(ReduceArg<T> arg, const T in[nreduce], const int idx=0) { array_reduce2d<nreduce, block_size, 1, T, do_sum, Reducer>(arg, in, idx); }
