...
namespace madwf_ml {
  ...
  public:
    ...
    TrainingParameter(std::vector<real>& host_vector): size(host_vector.size()) {
      ...
      if(!device_data){
        errorQuda("Unable to allocate a device buffer of %lu bytes.\n", m_size);
      }
    ...
    ~TrainingParameter(){
      if(device_data){
        device_free_(__func__, __FILE__, __LINE__, device_data);
      }
    }
    ...
    void from_host(const std::vector<real>& host_vector){
      if(host_vector.size() != get_size()){
        errorQuda("Size mismatch.\n");
      }
