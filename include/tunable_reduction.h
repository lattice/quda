...
namespace quda {
   ...
   */
  template <template <typename> class Transformer, int block_size_y = 2>
  ...
  protected:
    ...
    */
     ...
     */
    unsigned int maxBlockSize(const TuneParam &param) const { return max_block_size<block_size_y>(); }

    template <int block_size_x, template <typename> class Reducer, typename Arg>
      ...
      launch(Arg &arg, const TuneParam &tp, const qudaStream_t &stream)
    {
      if (tp.block.x == block_size_x)
        ...
        return launch<block_size_x - warp_size(), Reducer>(arg, tp, stream);
    }
  ...
  public:
    TunableReduction2D(const LatticeField &field) :
      field(field),
      kernel_file(std::string("kernels/") + Transformer<int>::filename())
    {
#ifdef JITIFY
