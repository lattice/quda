...
void display_driver_info() {
  printfQuda("Running the example driver:\n");
...
void exampleInvert(std::vector<ColorSpinorField*> &quarks, QudaInvertParam *inv_param)
{  
...
void exampleContract(std::vector<ColorSpinorField*> &quarks, void *host_corr)
{
  int n_quarks = (int)quarks.size();
  for(int i=0; i<n_quarks; i++) {
    exampleContractQuda(*quarks[i], (double*)host_corr);
  }
}

void programExample(void **host_quarks, QudaInvertParam inv_param, const int X[4])
{
  // Parameter object describing the sources/quarks
  ...
  // Create device vectors for sources/quarks
  ...
  // The sources are located in quda_quarks. We invert using those
  // sources and place the propagator from that solve back into quda_quarks.
  // The code in exampleInvert is very similar to the code found in the
  // invertQuda() function in interface_quda.cpp.
  double time_inv = -clock();
  exampleInvert(quda_quarks, &inv_param);
  time_inv += clock();
  
  // We now perfrom a contraction on the propagators, written using the
  // QUDA framework, but external to the QUDA library.
  // The code is an augmented version of QUDA's contraction code found
  // in lib/contrcat.cu and include/kernels/contraction.cuh.
  // Use it as demonstration to show that one can write GPU kernels,
  // autotuned by QUDA tuning.
  // It is a simple gamma5 insertion <phi| gamma_5 |phi> typical
  // for a pion correlation function. The correlation function is placed
  // in host_corr.
  
  double time_contract = -clock();
  exampleContract(quda_quarks, host_corr);
  time_lsp += clock();

  printfQuda("Invert time = %e\n", time_lsc/CLOCKS_PER_SEC);
  printfQuda("Contraction time = %e\n", time_lsi/CLOCKS_PER_SEC);
