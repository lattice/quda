...
// eigenvectors in eigen_vecs, and returns the smeared quarks in smeared_quark. Below is a description
// of the workflow.
//
...
// 
// 3. We pass each of these sources to the inverter, and then populated the apprpriate smeared_quark 
...
void laphQuarkSmearQuda(const void *noise_array, const void **eigen_vecs, void **smeared_quark, const int dilution_scheme, const int n_eigen_vecs, QudaInvertParam *inv_param)
 {
  profileContract.TPSTART(QUDA_PROFILE_TOTAL);
  profileContract.TPSTART(QUDA_PROFILE_INIT);
  ...
  // Parameter object descibing the eigenvectors
  ColorSpinorParam cpu_eigenvec_param(cpu_param);
  cpu_eigenvec_param.nSpin = 1;
  
  // Deduce the number of eigenvectors to be used per dilution.
  if (n_eigen_vecs%dilution_scheme != 0) {
    errorQuda("Number of eigen vectors passed %d is not a multiple of the dilution scheme %d", n_eigen_vecs, dilution_schem);
  }
  int n_dilution_vecs = n_eigen_vecs/dilution_scheme;
  
  // Loop over dilutions
  for(int i = 0; i<dilution_scheme; i++) {

    // Make QUDA host pointers for eigenvectors
    // This is an Nvec array of nColor * L^3 * T sized complex vectors
    std::vector<ColorSpinorField *> host_eigen_vecs;
    host_eigen_vecs.reserve(n_dilution_vecs);
    for (int j = 0; j < n_dilution_vecs; j++) {
      cpu_eigenvec_param.v = eigen_vecs[i + j * dilution_scheme];
      host_eigen_vecs.push_back(ColorSpinorField::Create(cpuParam));
    }
  
    // Make host pointer for the approriate stochastic coeffs    
    size_t source_data_bytes = n_dilution_vecs *  * cpu_param->Precision();
    void *host_source = pool_device_malloc(source_data_bytes);
  
  ColorSpinorParam cpu_param((void *)host_source, *inv_param, X, false, param->input_location);
  ColorSpinorField *host_source_ptr = ColorSpinorField::Create(cpu_param);
  ...
  cpu_eigenvec_param.nSpin = 1;
  std::vector<ColorSpinorField *> host_eigen_vecs;
  host_eigen_vecs.reserve(n_eigen_vecs);
  for (int i = 0; i < n_eigen_vecs; i++) {
    cpu_eigenvec_param.v = eigen_vecs[i];
    host_eigen_vecs.push_back(ColorSpinorField::Create(cpuParam));
  }

  // Loop over spin, eigen vector and dilution indices to compute the source
  for(int dil = 0; dil<dilution_scheme; dil++) {
    for(int spin = 0; spin<4; spin++) {
      for(int i = 0; i<n_eigen_vecs; i++) {
	int idx1 = 0;
	int idx2 = 0;
	((double*)host_source)[idx1] = ((double*)noise_array)[idx2] * eigen_vecs[i][idx3];
      }
    }
  }

  // Create device parameter
  ColorSpinorParam cuda_param(cpu_param);
  cuda_param.location = QUDA_CUDA_FIELD_LOCATION;
  cuda_param.create = QUDA_NULL_FIELD_CREATE;
  // Quda uses Degrand-Rossi gamma basis for contractions and will
  // automatically reorder data if necessary.
  cuda_param.gammaBasis = QUDA_DEGRAND_ROSSI_GAMMA_BASIS;
  cuda_param.setPrecision(cpu_param.Precision(), cpu_param.Precision(), true);

  std::vector<ColorSpinorField *> x, y;
  x.push_back(ColorSpinorField::Create(cuda_param));
  y.push_back(ColorSpinorField::Create(cuda_param));
