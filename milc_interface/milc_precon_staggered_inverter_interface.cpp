...
void qudaDDInvert(int external_precision, 
  ...
  milc_interface::Timer timer("qudaDDInvert");
  ...
  const QudaPrecision device_sloppy_precision = QUDA_SINGLE_PRECISION;
  const QudaPrecision device_precon_precision = QUDA_HALF_PRECISION;
  //const QudaPrecision device_precon_precision = QUDA_SINGLE_PRECISION;
  //const QudaPrecision device_precon_precision = QUDA_DOUBLE_PRECISION;


  printfQuda("link_size = %d\n", link_size);
  fflush(stdout);
  ...
  // set up the inverter
  {
    printfQuda("About to create invertParam\n");
    fflush(stdout);
    QudaInvertParam invertParam = newQudaInvertParam(); // Need to call this to set some default values


    printfQuda("inv_args.evenodd = %d\n", inv_args.evenodd);
    fflush(stdout);
    ...
    invertParam.maxiter_precondition = maxiter_precondition;
    ...
    for(int dir=0; dir<4; ++dir) invertParam.domain_overlap[dir] = domain_overlap[dir];

    printfQuda("invertParam.domain_overlap = %d, %d, %d, %d\n",
                                            invertParam.domain_overlap[0],
                                            invertParam.domain_overlap[1],
                                            invertParam.domain_overlap[2],
                                            invertParam.domain_overlap[3]);
