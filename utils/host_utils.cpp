				...
				const int dil)
{
  ...
  // Get local index
  int src_local[4];
  for (int d = 0; d < 4; d++) src_local[d] = prop_source_position[d] - comm_coord(d) * X[d];
  // Get linear index
  ...
  if ((comm_coord(0) * X[0] <= prop_source_position[0] && prop_source_position[0] < (comm_coord(0) + 1) * X[0])
      && (comm_coord(1) * X[1] <= prop_source_position[1] && prop_source_position[1] < (comm_coord(1) + 1) * X[1])
      && (comm_coord(2) * X[2] <= prop_source_position[2] && prop_source_position[2] < (comm_coord(2) + 1) * X[2])
      && (comm_coord(3) * X[3] <= prop_source_position[3] && prop_source_position[3] < (comm_coord(3) + 1) * X[3])) {
